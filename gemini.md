...
- **P-01: 소스 파일 인코딩 처리 (CRITICAL)**
    ...
    - **읽기 처리**: `PBMigrator`는 이 파일들을 `CP949` 인코딩으로 정확하게 읽어들여 한글 깨짐을 방지합니다. 이 과정은 `PBMigrator`의 파일 I/O 로직에 구현되어 있으며, 별도의 '깨진 한글 복원' 규칙(구 P-06)을 필요로 하지 않습니다.
    - **저장 인코딩**: 변환된 `target` 폴더의 모든 `.srw` 파일은 파워빌더 10.5와의 완벽한 호환성을 위해 **UTF-16 LE** 인코딩과 **CRLF** (`\r\n`) 줄바꿈으로 저장해야 합니다.
