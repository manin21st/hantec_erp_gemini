...
> **현재 모든 마이그레이션 로직은 P-01, P-02, P-03 세 가지 패턴의 조합으로 표준화되었습니다.**

-   **P-02: 이미지 버튼 이벤트 마이그레이션 (선행 작업)**
    -   **(목적)**: PowerBuilder 10의 구형 이미지 버튼(`p_retrieve` 등) `clicked` 이벤트를 10.5 환경에 맞는 표준 사용자 이벤트(`ue_retrieve` 등)로 변환합니다.
    -   **(처리 방안)**: 미리 정의된 매핑에 따라, 내용이 있는 이벤트 블록의 선언부(`event...;`)를 새로운 사용자 이벤트로 대체합니다. (내용이 없는 이벤트는 P-01이 처리하도록 남겨둡니다.)

-   **P-03: 오래된 이미지 버튼 참조 수정 (선행 작업)**
    -   **(목적)**: P-02 적용 후, 코드 내에 남아있는 구형 이미지 버튼의 속성 접근 구문(`p_print.Enabled` 등)을 수정하여 컴파일 오류를 방지합니다.
    -   **(처리 방안)**:
        -   `.Enabled` 속성 변경은 `of_SetEnabled()` 함수 호출로 **대체**합니다.
        -   `.PictureName` 속성 변경은 해당 라인을 **주석 처리**합니다.

-   **P-01: 부모-자식 윈도우 동기화 (마무리 작업)**
    -   **(목적)**: 자식 윈도우에 남아있는, 부모에 더 이상 존재하지 않는 '오래된 컨트롤'과 관련 이벤트들을 최종적으로 주석 처리하여 컴파일 오류를 방지합니다.
    -   **(처리 방안)**:
        1.  부모-자식 컨트롤 목록을 비교하여 '오래된 컨트롤'을 식별합니다.
        2.  해당 컨트롤의 정의 블록(`type...end type`)과 모든 관련 이벤트 블록(`event...end event`)을 찾아, 모든 라인 앞에 `//& `를 붙여 주석 처리합니다.
        3.  변환된 파일 상단에 `//& PBMigrator에 의해...` 설명 주석을 추가합니다.
    -   **(파일 포맷)**: 최종 저장되는 파일은 **U-DOS (UTF-16 LE with BOM)** 형식과 **CRLF** 줄바꿈을 사용해야 합니다.
    -   **(상태)**: 모든 규칙은 `pb_migrator/rules/` 폴더에 개별 파일로 구현되어 있으며, `engine.py`에서 **P-02 → P-03 → P-01** 순서로 적용됩니다.

-   **P-04: `dw_cond`를 `dw_input`으로 대체**
    -   **(목적)**: 구 버전의 `dw_cond` 데이터윈도우를 10.5 버전의 `dw_input`으로 안전하게 전환합니다.
    -   **(전제 조건)**: **소스 코드 내에 `dw_input`이라는 문자열이 존재하지 않을 경우에만 이 패턴을 적용합니다.**
    -   **(핵심 처리 방안)**:
        1.  `dw_cond`의 컨트롤 정의, 변수 선언, `create`/`destroy` 관련 라인을 모두 **삭제**합니다.
        2.  `create` 이벤트 내에서 `Control` 배열의 인덱스를 재정렬하여 빈틈이 없도록 **다시 채번**합니다.
        3.  위 작업들이 모두 끝난 후, 남아있는 코드 전체에서 `dw_cond`를 `dw_input`으로 **일괄 변경**합니다.
