...
type pb_1 from u_pic_cal within w_sal_06120
end type
type r_gubun from rectangle within w_sal_06120
end type
end forward
...
Else
	...
	If IsNull(sInsDat) or sInsDat = '' Then
		return 0
	Else
		MessageBox('확인','검사완료 자료는 삭제하실수 없습니다.!!')
		return -2
	End If	
End If
...
For ix = 1 To ds1.RowCount()
	...
	If sCvcod <> Trim(dw_input.GetItemString(1,'cvcod')) Then
		dw_insert.Reset()
		dw_cih.Reset()
		dw_cid.Reset()
		MessageBox('확 인','선택한 바이어가 일치하지 않습니다')
		Return -1
	End If
...
p_ins.Enabled = True
p_ins.PictureName = '..\image\송장선택_up.gif'
...
IF sModStatus = 'I' THEN
	...
	//dw_input.Modify("sudat.background.color = '" + String(Rgb(195,225,184))+"'")
...
this.Control[iCurrent+5]=this.dw_cid
this.Control[iCurrent+6]=this.dw_cih
this.Control[iCurrent+7]=this.pb_1
this.Control[iCurrent+8]=this.r_gubun
end on
...
type dw_datetime from w_inherite`dw_datetime within w_sal_06120
end type
...
ELSE
	...
	sCiNo	  = dw_input.GetItemString(1,"inv_no")
END IF
...
ELSE
	...
	For k = 1 TO dw_cid.RowCount()
		...
		dw_cid.SetItem(k,'ciqty', dCiqty*-1) // 송장수량
		dw_cid.SetItem(k,'ciamt', TrunCate(Round(dCiqty,2) * Round(dCiprc,5),2)*-1) // 송장금액
		dw_cid.SetItem(k,'itmamt',TrunCate(Round(dCiqty,2) * Round(dCiprc,5),2)*-1) // 품목금액
	Next
	
	/* CI header SETTING */
	dw_cih.SetItem(1, "sabu",     gs_sabu)
	dw_cih.SetItem(1, "cino",     sCino)
	dw_cih.SetItem(1, 'outcfdt',  sIoSuDate) /* 출고확정일 (매출확정일은 면장등록시) */
	...
	/* ci detail-> header update : trigger에서 원화계산및 끝전 처리 */
	...
	For k = 1 TO dw_insert.RowCount()
      If dw_insert.GetItemStatus(k,0,Primary!) = NewModified! Then
		...
		If nRow <= 0 Then
			RollBack;
			MessageBox('확인','송장의 품목을 찾을 수가 없습니다.!!')
			wf_init()
			Return
		End If
...
/* IMHIST 삭제 -> CI DETAIL 저장 -> CI HEADER 저장 */
...
IF sModStatus = 'M' THEN
	...
	/* CI DETAIL 먼저 저장하고 HEADER 저장해야함 */
	IF dw_cid.Update() <> 1 THEN
		ROLLBACK;
		Return
	END IF
...
IF dw_input.Retrieve(gs_sabu,sIoJpNo) > 0 THEN
	sCino = dw_input.GetItemString(1,'inv_no')
	...
	P_ins.Enabled = False //송장선택
	p_ins.PictureName = '..\image\송장선택_d.gif'
	ib_any_typing = False
ELSE
	wf_init()
	f_message_chk(50,'')
...
If Not IsNull(sSaleDt) Then
	...
	w_mdi_frame.sle_msg.text = '반품확정된 자료입니다.!!'
Else
	P_del.Enabled = True
...
string picturename = "..\image\송장선택_up.gif"
end type
...
sCvName  = Trim(dw_input.GetItemString(1,'cvname'))
...
IF il_functionvalue = 1 THEN
	...
	gs_code = 'A'
	gs_gubun = 'Y'
	OpenWithParm(w_expci_popup, sSaupj)	/* 부가사업장 */
...
event p_ins::ue_lbuttondown;PictureName = "..\image\송장선택_dn.gif"
end event

event p_ins::ue_lbuttonup;PictureName = "..\image\송장선택_up.gif"
end event

type p_new from w_inherite`p_new within w_sal_06120
integer y = 3200
end type

type dw_input from w_inherite`dw_input within w_sal_06120
...
type cb_delrow from w_inherite`cb_delrow within w_sal_06120
boolean visible = false
integer y = 3324
end type

type cb_addrow from w_inherite`cb_addrow within w_sal_06120
boolean visible = false
integer y = 3324
end type
...
If wf_chk(newrow) = 0 Then
	p_del.Enabled = True
	p_ins.PictureName = '..\image\삭제_d.gif'
Else
	p_del.Enabled = False
	p_ins.PictureName = '..\image\삭제_up.gif'
End If
...
type gb_10 from w_inherite`gb_10 within w_sal_06120
end type
...
type r_head from w_inherite`r_head within w_sal_06120
integer x = 416
integer y = 40
integer width = 2917
integer height = 284
...
type r_detail from w_inherite`r_detail within w_sal_06120
integer y = 348
integer width = 4576
integer height = 1952
end type
