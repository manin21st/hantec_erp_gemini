...
type pb_1 from u_pic_cal within w_mat_01350
end type
type pb_2 from u_pic_cal within w_mat_01350
end type
...
//dw_input.SetItem(1, 'stdy', String(TODAY(), 'yyyymm') + '01')
dw_input.SetItem(1, 'stdy', String(TODAY(), 'yyyymmdd'))
dw_input.SetItem(1, 'eddy', String(TODAY(), 'yyyymmdd'))
...
this.Control[iCurrent+5]=this.dw_csv
this.Control[iCurrent+6]=this.dw_hidden
this.Control[iCurrent+7]=this.cb_2
this.Control[iCurrent+8]=this.dw_prt
end on
...
event ue_preview;p_print.TriggerEvent(Clicked!)
end event

type cb_exit from w_inherite`cb_exit within w_mat_01350
end type

type sle_msg from w_inherite`sle_msg within w_mat_01350
end type

type dw_datetime from w_inherite`dw_datetime within w_mat_01350
end type
...
If dw_input.GetItemString(1, 'gbn') <> 'I' Then
...
If dw_input.GetItemString(1, 'gbn') <> 'D' Then
...
String  ls_saupj
ls_saupj = dw_input.GetItemString(row, 'saupj')
If IsNull(ls_saupj) OR Trim(ls_saupj) = '' Then ls_saupj = '%'

String  ls_sdat
ls_sdat = dw_input.GetItemString(row, 'stdy')
If IsNull(ls_sdat) OR Trim(ls_sdat) = '' Then
	MessageBox('확인', '시작일자를 입력 하십시오.')
	f_setfocus_dw(dw_input, row, 'stdy')
	Return
End If

String  ls_edat
ls_edat = dw_input.GetItemString(row, 'eddy')
If IsNull(ls_edat) OR Trim(ls_edat) = '' Then
	MessageBox('확인', '종료일자를 입력 하십시오.')
	f_setfocus_dw(dw_input, row, 'eddy')
	Return
End If

String  ls_cvcod
ls_cvcod = dw_input.GetItemString(row, 'tovnd')
If IsNull(ls_cvcod) Or Trim(ls_cvcod) = '' Then
//	ls_cvcod = '%'
	MessageBox('확인', '거래처를 입력 하십시오.')
	f_setfocus_dw(dw_input, row, 'tovnd')
	Return
End If

String  ls_ittyp
ls_ittyp = dw_input.GetItemString(row, 'ittyp')
...
event p_print::clicked;call super::clicked;If dw_input.GetItemString(1, 'gbn') <> 'D' Then
	MessageBox('확인', '삭제일 경우 가능 합니다.')
	Return
End If
...
type p_new from w_inherite`p_new within w_mat_01350
integer x = 1202
integer y = 3320
end type

type dw_input from w_inherite`dw_input within w_mat_01350
integer y = 56
integer width = 4576
integer height = 212
...
type cb_delrow from w_inherite`cb_delrow within w_mat_01350
boolean visible = false
integer y = 3216
end type

type cb_addrow from w_inherite`cb_addrow within w_mat_01350
boolean visible = false
integer y = 3216
end type
...
type gb_10 from w_inherite`gb_10 within w_mat_01350
end type
...
type r_head from w_inherite`r_head within w_mat_01350
integer width = 4585
integer height = 220
end type

type r_detail from w_inherite`r_detail within w_mat_01350
integer y = 308
integer width = 4585
integer height = 2000
end type
...
dw_input.SetColumn('stdy')
IF IsNull(gs_code) THEN Return
ll_row = dw_input.GetRow()
If ll_row < 1 Then Return
dw_input.SetItem(ll_row, 'stdy', gs_code)
...
dw_input.SetColumn('eddy')
IF IsNull(gs_code) THEN Return
ll_row = dw_input.GetRow()
If ll_row < 1 Then Return
dw_input.SetItem(ll_row, 'eddy', gs_code)
...
event clicked;If dw_input.GetItemString(1, 'gbn') <> 'I' Then
