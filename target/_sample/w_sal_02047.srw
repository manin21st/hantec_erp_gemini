...
type pb_1 from u_pic_cal within w_sal_02047
end type
...
IF sModStatus = 'I' THEN
	...
	dw_input.Modify('iojpno_img.visible = 0')
	dw_input.Modify('iogbn.protect = 0')
	dw_input.Modify('cvcod.protect = 0')
	dw_input.Modify('vndname.protect = 0')
	dw_input.Modify('pjt_cd.protect = 0')
//	dw_input.Modify('saupj.protect = 0')
	dw_input.Modify('sudat.protect = 0')
//	dw_input.Modify('io_empno.protect = 0')
	dw_input.Modify('depot_no.protect = 0')
	
	siogbn = dw_input.getitemstring(1, "iogbn")
	Select salegu,amtgu into :ssalegu, :samtgu from iomatrix where iogbn = :siogbn;
	If ssalegu = 'Y' then
		dw_input.object.t_gbn.text = "유상"
...
ELSE
   ...
   p_print.PictureName = "..\image\인쇄_up.gif"
	...
	dw_input.Modify('iojpno_img.visible = 1')
	dw_input.Modify('iogbn.protect = 1')
	dw_input.Modify('sudat.protect = 1')
	dw_input.Modify('cvcod.protect = 1')
	dw_input.Modify('vndname.protect = 1')
//	dw_input.Modify('io_empno.protect = 1')
	dw_input.Modify('depot_no.protect = 1')
	dw_input.Modify('pjt_cd.protect = 1')
...
If Trim(ls_saupj) = '' OR IsNull(ls_saupj) Then
	ls_saupj = gs_saupj
	dw_input.SetItem(1, 'saupj', ls_saupj)
End If
f_child_saupj(dw_input, 'depot_no', ls_saupj)
//f_child_saupj(dw_input, 'depot_no', gs_saupj)
//f_child_saupj(dw_input, 'iogbn', gs_saupj)

// 창고
select cvcod into :sdepot  from vndmst    where cvgu = '5' and jumaechul = '1'  and ipjogun = :gs_saupj and rownum = 1 ;
dw_input.SetItem(1, 'depot_no', sdepot)
...
//제품 출고 담당자
rtncode 	= dw_input.GetChild('io_empno', state_child)
...
LsIoJpNo    	= Left(dw_input.GetItemString(1,"iojpno"),12)
...
sIo_empno   	= Trim(dw_input.GetItemString(1,"io_empno"))
sDepot_no   	= Trim(dw_input.GetItemString(1,"depot_no"))
...
sdepot_no = dw_input.GetItemString(1,"depot_no")
...
For k = 1 TO nRcnt
	If dw_insert.GetItemStatus(k, 0, Primary!) = NewModified! Then 
	  ...
	  dw_insert.SetItem(k,"pjt_cd",	   dw_input.GetItemString(1,"pjt_cd")) /* 프로젝트 */
	  ...
	  dw_insert.SetItem(k,"outchk",     'N')
  	  dw_insert.SetItem(k,"depot_no",    dw_input.GetItemString(1, "depot_no"))
//	  dw_insert.SetItem(k,"io_empno",    dw_input.GetItemString(1, "io_empno"))
...
dw_input.SetItem(1,"iojpno",LsIoJpNo)
...
For k = 1 TO nRcnt
   ...
   dw_insert.SetItem(k,"pjt_cd",	    dw_input.GetItemString(1,"pjt_cd")) /* 프로젝트 */
	...
	/* ---------------------------------------------------- */		
//	If cbx_move.Checked = True And  dw_input.GetItemString(1,"saupj") = '20' Then
	If cbx_move.Checked = True Then
		String ls_autojpno
		ls_autojpno = wf_automove(k, dw_insert, LsSuBulDate)
		If ls_autojpno <> '-1' Then
...
/* 판매단가및 할인율 */
sOrderDate = dw_input.GetItemString(1,"sudat")
sCvcod = dw_input.GetItemString(1,"cvcod")
...
this.Control[iCurrent+9]=this.dw_autoimhist
this.Control[iCurrent+10]=this.cbx_move
end on
...
event ue_preview;call super::ue_preview;p_3.TriggerEvent(Clicked!)
end event
...
type sle_msg from w_inherite`sle_msg within w_sal_02047
integer x = 5230
integer y = 3228
end type

type dw_datetime from w_inherite`dw_datetime within w_sal_02047
integer x = 5230
integer y = 3228
end type
...
IF sModStatus = 'C' THEN
	...
	sIojpno = dw_input.getitemstring(1, "iojpno")
	lcnt = 0
...
//	dw_input.Retrieve(gs_sabu,LsIoJpNo)
...
// 수불구분 입력
ls_iogbn	= dw_input.getitemstring(1, "iogbn")
...
type p_new from w_inherite`p_new within w_sal_02047
integer y = 3200
end type

type dw_input from w_inherite`dw_input within w_sal_02047
...
Choose Case GetColumnName()
	Case 'saupj'
		...
		f_child_saupj(dw_input, 'depot_no', ssaupj)
...
type cb_delrow from w_inherite`cb_delrow within w_sal_02047
boolean visible = false
integer y = 3500
end type

type cb_addrow from w_inherite`cb_addrow within w_sal_02047
boolean visible = false
integer y = 3500
end type
...
type gb_10 from w_inherite`gb_10 within w_sal_02047
integer x = 5221
integer y = 3376
end type
...
type r_head from w_inherite`r_head within w_sal_02047
integer width = 2953
integer height = 240
end type

type r_detail from w_inherite`r_detail within w_sal_02047
integer y = 328
end type
...
boolean bringtotop = true
string picturename = "..\image\발주처품목선택_up.gif"
end type
...
dw_insert.Reset()
sCvcod 	= dw_input.getitemstring(1, "cvcod"    ) /* 거 래 처  */ 
Sempno   = dw_input.getitemstring(1, "io_empno" ) /* 담 당 자  */
SO_house = dw_input.getitemstring(1, "depot_no" ) /* 출고 창고 */
sOrderDate = dw_input.getitemstring(1, "sudat" ) /* 출고 창고 */
...
sArg_sdate = dw_input.getitemstring(1,'iojpno')
