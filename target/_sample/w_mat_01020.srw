...
type cb_3 from commandbutton within w_mat_01020
end type
type cb_delrow from commandbutton within w_mat_01020
end type
type cb_addrow from commandbutton within w_mat_01020
end type
...
type pb_1 from u_pic_cal within w_mat_01020
end type
...
type r_head from rectangle within w_mat_01020
end type
type r_detail from rectangle within w_mat_01020
end type
type istr_resize from structure within w_mat_01020
end type
end forward

type istr_resize from structure
	datawindow		idw_name
	string		boolean_x
	string		boolean_y
end type
...
Datawindow	idw_name //// 검색시 지정된 dw 이름  
...
event ue_append();p_addrow.TriggerEvent(Clicked!)
end event

event ue_retrieve();p_retrieve.TriggerEvent(Clicked!)
end event

event ue_update();p_save.TriggerEvent(Clicked!)
end event

event ue_delete();p_delete.TriggerEvent(Clicked!)
end event

event ue_cancel();p_cancel.TriggerEvent(Clicked!)
end event
...
this.cb_delrow=create cb_delrow
...
m_main2.m_window.m_addrow.enabled = false
m_main2.m_window.m_delrow.enabled = false
...
event clicked;p_2.TriggerEvent(Clicked!)
end event
