...
type pb_1 from u_pic_cal within w_sm10_0030_asan
end type
...
type pb_2 from u_pic_cal within w_sm10_0030_asan
end type
...
event ue_update();p_mod.TriggerEvent(Clicked!)
end event

public function integer wf_retrieve ();Long i ,ll_rcnt
string ls_gubun ,ls_saupj , ls_cvcod ,ls_sdate  ,ls_edate, ls_saupj_cust 
string ls_factory , ls_itnbr, ls_itnbr_from, ls_itnbr_to, ls_from, ls_to, ls_doccode
string ls_ipsource, ls_ipgubun
Integer li_no
DataWindow ldw_x
...
ls_doccode = Trim(dw_ip.Object.doccode[1])

/* D9 자료에 한해 입고구분, 입고소스 조회조건 추가 - 2025.09.09 by dykim */
ls_ipsource = Trim(dw_ip.Object.ipsource[1]) 
ls_ipgubun = Trim(dw_ip.Object.ipgubun[1])

If ls_itnbr = '' Or isNull(ls_itnbr) Then ls_itnbr = ''
If ls_factory = '' Or isNull(ls_factory) Or ls_factory = '.' Then ls_factory = '%'
If ls_doccode = '' Or isNull(ls_doccode) Then ls_doccode = '%'

ls_doccode = ls_doccode + '%'
...
Else
	...
	IF ls_gubun = 'D9' THEN
		ldw_x.Retrieve(ls_saupj_cust , ls_gubun+ls_sdate ,ls_gubun+ls_edate, ls_from, ls_to , ls_factory ,  li_no, ls_doccode, ls_ipsource, ls_ipgubun )		
	ELSE
		ldw_x.Retrieve(ls_saupj_cust , ls_gubun+ls_sdate ,ls_gubun+ls_edate, ls_from, ls_to , ls_factory ,  li_no, ls_doccode )
	END IF
...
dec {3}  ld_iptqty, ld_iptamt, ld_packtamt
...
Do While	FileRead(li_FileNum, ls_Input_Data) <> -100	
	...
	If li_rtn > 0 And li_cnt = 0 Then
		...
		ld_iptqty = dec(f_replace(ls_data[7], ',', ''))
		ld_iptamt = dec(f_replace(ls_data[8], ',', ''))
		ld_packtamt = dec(f_replace(ls_data[9], ',', ''))
		...
		VALUES    
		(:gs_sabu	,	:ls_data[1],		:ls_data[2],		:ls_data[3],		:ls_data[4],	:ls_data[5]	,		:ls_data[6]	,
		 :ld_iptqty,	:ld_iptamt,			:ld_packtamt,		:ls_crt_date	,	:ls_crt_time,	:ls_crt_user,		NULL	,
...
Do While	FileRead(li_FileNum, ls_Input_Data) <> -100	
	...
	If li_rtn > 0 And li_cnt = 0 Then
		...
		ld_iptqty = dec(f_replace(ls_data[9], ',', ''))
		ld_iptamt = dec(f_replace(ls_data[10], ',', ''))
		ld_packtamt = dec(f_replace(ls_data[11], ',', ''))
		...
		(:gs_sabu	,	:ls_data[1],		:ls_data[3],		:ls_data[2],		:ls_data[6],	:ls_data[4]	,		:ls_data[5]	,
		 :ls_data[7],	:ls_data[8],		:ld_iptqty,			:ld_iptamt,			:ld_packtamt,	:ls_crt_date	,	:ls_crt_time,
...
if gs_lang = "CH" then
	...
	w_mdi_frame.uo_toolbarstrip.of_SetEnabled("저장(&S)", true) //// 저장
	...
	//w_mdi_frame.uo_toolbarstrip.of_SetEnabled("미리보기(&R)", true)  //// 미리보기
...
tab_1.tabpage_1.rr_1.width = tab_1.width - 59
tab_1.tabpage_1.rr_1.height = tab_1.height - 195

tab_1.tabpage_2.rr_2.width = tab_1.width - 59
tab_1.tabpage_2.rr_2.height = tab_1.height - 195

dw_list.width = this.width - 154
dw_list.height = this.height - dw_list.y - 154
end event

event ue_excel;p_excel.TriggerEvent(Clicked!)
end event
...
type cb_print from w_standard_print`cb_print within w_sm10_0030_asan
end type

type cb_excel from w_standard_print`cb_excel within w_sm10_0030_asan
end type

type cb_preview from w_standard_print`cb_preview within w_sm10_0030_asan
end type

type cb_1 from w_standard_print`cb_1 within w_sm10_0030_asan
...
If  FileExists('..\bin\date_conv.xls') = False Then
	MessageBox('확인','date_conv.xls'+' 파일이 존재하지 않습니다.')
	Return
End If 

uo_xltemp = create uo_xlobject
uo_xltemp.uf_excel_connect('..\bin\date_conv.xls', false , 3)
...
type r_1 from w_standard_print`r_1 within w_sm10_0030_asan
...
type r_2 from w_standard_print`r_2 within w_sm10_0030_asan
integer width = 3301
integer height = 360
end type
...
if ldw_x.Update() = 1 then
	...
	commit ;
	w_mdi_frame.sle_msg.text =	"모든자료를 삭제하였습니다!!"	
else
	rollback ;
...
integer width = 178
boolean bringtotop = true
string picturename = "..\image\엑셀변환_up.gif"
end type
...
event clicked;uo_xlobject uo_xl , uo_xltemp
string ls_doccode, ls_ipsource, ls_ipgubun
string ls_mitnbr , ls_colitnbr, ls_mcvcod, ls_mdcvcod, ls_citnbr, ls_CRT_DATE, ls_CRT_TIME
String ls_saupj, ls_itnbr, ls_ymd, ls_plant, ls_factory, ls_cvcod, ls_btime, ls_arrymd, ls_arrtime
string ls_docname, ls_named[], ls_line 
Long   ll_xl_row, ll_r, ix, i, ll_cnt=0, ll_c, ll_value, iii=0
Long   ll_err, ll_succeed  
String ls_file, ls_orderno
Long ll_seq, ll_jpno, ll_count, ll_cc
String ls_custcd, ls_iojpno[]
...
//		현대/기아 최신 양식으로 변경 '25.02.28 BY BHKIM
//		ls_doccode = Trim(uo_xl.uf_gettext(ll_xl_row,2))
//		ls_custcd  = ls_custcd
//		ls_factory = Trim(uo_xl.uf_gettext(ll_xl_row,3))
//		ls_itnbr   = Trim(uo_xl.uf_gettext(ll_xl_row,4))
...
//		ls_ipgubun = 'A'
		...
		ls_doccode = Trim(uo_xl.uf_gettext(ll_xl_row,1))
		ls_custcd  = ls_custcd
		ls_factory = Trim(uo_xl.uf_gettext(ll_xl_row,2))
		ls_itnbr   = Trim(uo_xl.uf_gettext(ll_xl_row,3))
		
		if isNull(Trim(uo_xl.uf_gettext(ll_xl_row,4))) or Trim(uo_xl.uf_gettext(ll_xl_row,4)) = '' then
			ls_ipsource = 'V'
		elseif Trim(uo_xl.uf_gettext(ll_xl_row,4)) <> 'L' and Trim(uo_xl.uf_gettext(ll_xl_row,4)) <> 'V' then
			ls_ipsource = 'V'
		else
			ls_ipsource = Trim(uo_xl.uf_gettext(ll_xl_row,4))
		end if
		
		if isNull(Trim(uo_xl.uf_gettext(ll_xl_row,5))) or Trim(uo_xl.uf_gettext(ll_xl_row,5)) = '' then
			ls_ipgubun = 'A'
		else
			ls_ipgubun = Trim(uo_xl.uf_gettext(ll_xl_row,5))
		end if
		
		st_state.text =String(ll_xl_row) +" 행의 " +ls_itnbr+ ' 품번을 읽고 있습니다.'
		
		ld_iptqty  = Dec(f_replace(Trim(uo_xl.uf_gettext(ll_xl_row,6)), ',', ''))
		ld_iptamt  = Dec(f_replace(Trim(uo_xl.uf_gettext(ll_xl_row,7)), ',', ''))
		ld_packtamt= Dec(f_replace(Trim(uo_xl.uf_gettext(ll_xl_row,8)), ',', ''))
		
		//품번 등록 여부 확인 '25.06.23 BY BHKIM
		select count('x')
		  into :ll_cc
		  from itemas
		 where replace(itnbr,'-','') = :ls_itnbr and useyn = '0' ;
		
		if ll_cc <= 0 then
			MessageBox('확인', String(ll_xl_row) +" 행의 " +ls_itnbr+ " 품번이 등록되어 있지 않습니다.")
			rollback;
			uo_xltemp.uf_excel_Disconnect()
			uo_xl.uf_excel_Disconnect()
			st_state.visible = false
			return
		elseif ll_cc > 1 then
			select listagg(itnbr, ', ') within group (order by itnbr) as col_itnbr
			  into :ls_colitnbr
			  from itemas
			 where replace(itnbr, '-', '') = :ls_itnbr;
			
			MessageBox('확인', String(ll_xl_row) +" 행의 " +ls_itnbr+ " 품번이 다음과 같이 등록되어 있습니다." + "~n" + ls_colitnbr)
			rollback;
			uo_xltemp.uf_excel_Disconnect()
			uo_xl.uf_excel_Disconnect()
			st_state.visible = false
		...
		if wf_ref_code('ERPD9', ls_custcd, ls_factory, ls_itnbr, ls_mitnbr, ls_mcvcod, ls_mdcvcod) = -1 then
			wf_error(ls_file, ll_cnt, ls_doccode, ls_factory, ls_itnbr, '참조코드가 없습니다.')
			il_err++
			Continue ;
		end if
