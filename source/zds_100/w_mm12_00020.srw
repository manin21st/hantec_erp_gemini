$PBExportHeader$w_mm12_00020.srw
$PBExportComments$SPARE PART 입고
forward
global type w_mm12_00020 from window
end type
type dw_imhist from datawindow within w_mm12_00020
end type
type dw_imhist_mro from datawindow within w_mm12_00020
end type
type p_delrow from uo_picture within w_mm12_00020
end type
type p_addrow from uo_picture within w_mm12_00020
end type
type p_search from uo_picture within w_mm12_00020
end type
type p_exit from uo_picture within w_mm12_00020
end type
type p_can from uo_picture within w_mm12_00020
end type
type p_del from uo_picture within w_mm12_00020
end type
type p_mod from uo_picture within w_mm12_00020
end type
type p_inq from uo_picture within w_mm12_00020
end type
type cb_1 from commandbutton within w_mm12_00020
end type
type cb_delete from commandbutton within w_mm12_00020
end type
type cb_cancel from commandbutton within w_mm12_00020
end type
type rb_delete from radiobutton within w_mm12_00020
end type
type rb_insert from radiobutton within w_mm12_00020
end type
type dw_detail from datawindow within w_mm12_00020
end type
type cb_save from commandbutton within w_mm12_00020
end type
type cb_exit from commandbutton within w_mm12_00020
end type
type cb_retrieve from commandbutton within w_mm12_00020
end type
type rr_1 from roundrectangle within w_mm12_00020
end type
type rr_2 from roundrectangle within w_mm12_00020
end type
type rr_3 from roundrectangle within w_mm12_00020
end type
type dw_list from datawindow within w_mm12_00020
end type
end forward
...
global w_mm12_00020 w_mm12_00020
...
siogbn = 'I65'	/* SPARE PART 입고	*/
sJpno  = sDate + string(dSeq, "0000")


FOR lRow = 1	TO	dw_list.RowCount()
	dw_list.SetItem(lRow, "imhist_sabu",	gs_sabu)
	dw_list.SetItem(lRow, "imhist_jnpcrt",	'048')												// 전표생성구분
	...
	dw_list.SetItem(lRow, "imhist_cvcod",		dw_list.GetItemstring(lRow, "imhist_ioredept"))	// 거래처창고=거래처
...
if dw_list.update() <> 1 then
	rollback ;
	messagebox('확인','SPARE PART 입고 생성실패!!!')
	return -1
end if
...
if ic_status = '1' then
	dw_detail.dataobject = 'd_mm12_00020_1'
	dw_list.dataobject = 'd_mm12_00020_a'
	
	p_addrow.visible = true
	p_delrow.visible = true
	w_mdi_frame.sle_msg.text = "등록"
else
	dw_detail.dataobject = 'd_mm12_00020_2'
	dw_list.dataobject = 'd_mm12_00020_b'
...
boolean titlebar = true
string title = "none"
string dataobject = "d_mm12_00020_0a"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean livescroll = true
...
if messagebox('확인','선택된 입고자료를 삭제합니다.',question!,yesno!,1) = 2 then return
...
if ic_status = '1' then
	...
	FOR lRow = 1	TO	dw_list.RowCount()
		dw_list.SetItem(lRow, "imhist_field_cd", dw_list.GetItemString(lRow, "imhist_usage"))		// 사용처
		dw_list.SetItem(lRow, "imhist_iosuqty", dw_list.GetItemString(lRow, "imhist_ioqty"))		// 사용처
	NEXT
	if dw_list.update() <> 1 then
		rollback ;
		messagebox('확인','SPARE PART 저장실패!!!')
		return
	end if
...
if dw_list.retrieve(gs_sabu,sfdate,stdate,sempno,sdepot) < 1 then
	f_message_chk(50, '[SparePart 입고내역]')
	dw_detail.setcolumn("fdate")
	dw_detail.setfocus()
	return
end if
...
integer height = 160
integer taborder = 10
string dataobject = "d_mm12_00020_1"
boolean border = false
boolean livescroll = true
end type
...
string dataobject = "d_mm12_00020_b"
