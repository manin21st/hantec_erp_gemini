$PBExportHeader$w_mm12_00030.srw
$PBExportComments$** MRO 자재 불출
forward
global type w_mm12_00030 from window
end type
type dw_imhist from datawindow within w_mm12_00030
end type
type dw_imhist_mro from datawindow within w_mm12_00030
end type
type p_delrow from uo_picture within w_mm12_00030
end type
type p_addrow from uo_picture within w_mm12_00030
end type
type p_search from uo_picture within w_mm12_00030
end type
type p_exit from uo_picture within w_mm12_00030
end type
type p_can from uo_picture within w_mm12_00030
end type
type p_del from uo_picture within w_mm12_00030
end type
type p_mod from uo_picture within w_mm12_00030
end type
type p_inq from uo_picture within w_mm12_00030
end type
type cb_1 from commandbutton within w_mm12_00030
end type
type cb_delete from commandbutton within w_mm12_00030
end type
type cb_cancel from commandbutton within w_mm12_00030
end type
type rb_delete from radiobutton within w_mm12_00030
end type
type rb_insert from radiobutton within w_mm12_00030
end type
type dw_detail from datawindow within w_mm12_00030
end type
type cb_save from commandbutton within w_mm12_00030
end type
type cb_exit from commandbutton within w_mm12_00030
end type
type cb_retrieve from commandbutton within w_mm12_00030
end type
type rr_1 from roundrectangle within w_mm12_00030
end type
type rr_2 from roundrectangle within w_mm12_00030
end type
type rr_3 from roundrectangle within w_mm12_00030
end type
type dw_list from datawindow within w_mm12_00030
end type
end forward
...
global w_mm12_00030 w_mm12_00030
...
if ic_status = '1' then
	dw_detail.dataobject = 'd_mm12_00030_1'
	dw_list.dataobject = 'd_mm12_00030_a'
	
	p_addrow.visible = true
	p_delrow.visible = true
	w_mdi_frame.sle_msg.text = "등록"
else
	dw_detail.dataobject = 'd_mm12_00030_2'
	dw_list.dataobject = 'd_mm12_00030_b'
...
boolean titlebar = true
string title = "none"
string dataobject = "d_mm12_00030_0a"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean livescroll = true
...
//	NEXT
	if dw_list.update() <> 1 then
		rollback ;
		messagebox('확인','SparePart 불출자료 저장실패!!!')
		return
	end if
...
integer height = 160
integer taborder = 10
string dataobject = "d_mm12_00030_1"
boolean border = false
boolean livescroll = true
end type
...
if this.getcolumnname() = 'gubun' then
	...
	if ic_status = '1' then 		// 등록
		...
		if sgubun = 'OA1' then		// 개인불출
			dw_list.dataobject = 'd_mm12_00030_c'
		else
			dw_list.dataobject = 'd_mm12_00030_d'
		end if
	end if		
	dw_list.settransobject(sqlca)
end if
...
string dataobject = "d_mm12_00030_a"
...
elseif this.getcolumnname() = 'imhist_field_cd' then
	sdept = this.gettext()
	
	if isnull(sdept) or sdept = '' then 
		this.setitem(lrow,'field_cd_name',snull)
		return
	end if
		...
		select wcdsc into :sdeptname from wrkctr
		 where wkctr = :sdept ;
		if sqlca.sqlcode = 0 then
			this.setitem(lrow,'field_cd_name',sdeptname)
		else
			messagebox('확인','등록되지 않은 작업장코드입니다.')
			this.setitem(lrow,'imhist_field_cd',snull)
			this.setitem(lrow,'field_cd_name',snull)
			return 1
		end if
...
elseif this.getcolumnname() = 'imhist_field_cd' then
		...
		this.SetItem(1, 'imhist_field_cd', 	 gs_Code)
		this.triggerevent(itemchanged!)
