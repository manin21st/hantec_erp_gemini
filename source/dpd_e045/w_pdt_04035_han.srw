$PBExportHeader$w_pdt_04035_han.srw
$PBExportComments$생산투입등록 - 한텍
forward
global type w_pdt_04035_han from window
end type
type dw_hidden from datawindow within w_pdt_04035_han
end type
type p_print from uo_picture within w_pdt_04035_han
end type
type p_2 from uo_picture within w_pdt_04035_han
end type
type p_1 from uo_picture within w_pdt_04035_han
end type
type p_retrieve from uo_picture within w_pdt_04035_han
end type
type p_save from uo_picture within w_pdt_04035_han
end type
type p_delete from uo_picture within w_pdt_04035_han
end type
type p_cancel from uo_picture within w_pdt_04035_han
end type
type p_exit from uo_picture within w_pdt_04035_han
end type
type p_addrow from uo_picture within w_pdt_04035_han
end type
type p_delrow from uo_picture within w_pdt_04035_han
end type
type dw_print from datawindow within w_pdt_04035_han
end type
type cbx_1 from checkbox within w_pdt_04035_han
end type
type dw_imhist_in from datawindow within w_pdt_04035_han
end type
type rb_delete from radiobutton within w_pdt_04035_han
end type
type rb_insert from radiobutton within w_pdt_04035_han
end type
type dw_imhist from datawindow within w_pdt_04035_han
end type
type dw_list from datawindow within w_pdt_04035_han
end type
type gb_2 from groupbox within w_pdt_04035_han
end type
type rr_1 from roundrectangle within w_pdt_04035_han
end type
type dw_detail from datawindow within w_pdt_04035_han
end type
end forward
...
global w_pdt_04035_han w_pdt_04035_han
...
//		dw_imHist.SetItem(lRowHist, "pjt_cd",	  sProject)
		dw_imHist.SetItem(lRowHist, "pjt_cd",	  dw_list.GetItemString(lRow, "imhist_ijpno"))
...
//			dw_imHist_in.SetItem(lRowHist_in, "pjt_cd",	sProject)
			dw_imHist_in.SetItem(lRowHist_in, "pjt_cd",	dw_list.GetItemString(lRow, "imhist_ijpno"))
...
string	sHist_Jpno, sIodate, sDelgub, syebi5, sAuto, sError, sJpno
long		lRow, lRowCount, i, k

lRowCount = dw_list.RowCount()

select fun_get_syscnfg('Y', 25, '8') into :sAuto from dual ;
...
//	/* 생산부품입고에 대한 생산자동출고 생성  - 2007.03.29 - 송병호 */
//	if sAuto = 'N' Then
//		sError = 'X';
//		
//		sJpno = Left(sHist_Jpno, 12)
//		sqlca.MM10_IMHIST_O01_COIL(gs_sabu, sJpno, 'D', sError);
//		if sError = 'X' or sError = 'Y' then
//			f_message_chk(41, '[생산자동출고]')
//			Rollback;
//			return -1
//		end if
//	end if
...
//	
//////////////////////////////////////////////////////////////////
string	sHist_Jpno, sGubun, siodate, sioyn, sAuto, sError
...
//		if sAuto = 'N' Then
//			sError = 'X';
//			sqlca.MM10_IMHIST_O01_COIL(gs_sabu, sHist_Jpno, 'U', sError);
//			if sError = 'X' or sError = 'Y' then
//				f_message_chk(41, '[생산자동출고]')
//				Rollback;
//				return -1
...
//출고창고 
f_child_saupj(dw_detail, 'house', ls_saupj)
//입고창고
f_child_saupj(dw_detail, 'cvcod', ls_saupj)

/* 기본값 설정 - 사업장별 기본 입/출고 창고 지정 */
String  ls_istore, ls_ostore
SELECT MAX(DECODE(SOGUAN, '2', CVCOD, NULL)), MAX(DECODE(SOGUAN, '3', CVCOD, NULL)) INTO :ls_istore, :ls_ostore FROM VNDMST
 WHERE CVGU = '5' AND EMAIL = 'Y' AND HOMEPAGE = 'Y' AND IPJOGUN = :ls_saupj AND SOYOUJA IS NOT NULL ;
dw_detail.SetItem(1, 'house', ls_ostore)
dw_detail.SetItem(1, 'cvcod', ls_istore)
...
string dataobject = "d_pdt_04035_han_2"
...
event clicked;call super::clicked;if rb_delete.checked then
	messagebox('확인','등록모드에서만 생성 가능합니다!!!')
	return
end if
...
IF dw_hidden.Retrieve(gs_sabu, sdate) > 0 THEN
	dw_list.Reset()
	FOR k=1 TO dw_hidden.rowcount()
		lRow = dw_list.insertrow(0)
		dw_list.setitem(lRow, 'itnbr', dw_hidden.getitemstring(k, 'imhist_itnbr' ))
		dw_list.setitem(lRow, 'itdsc', dw_hidden.getitemstring(k, 'itemas_itdsc' ))
		dw_list.setitem(lRow, 'ispec', dw_hidden.getitemstring(k, 'itemas_ispec' ))
		dw_list.setitem(lRow, 'jijil', dw_hidden.getitemstring(k, 'itemas_jijil' ))
		dw_list.setitem(lRow, 'lotsno', dw_hidden.getitemstring(k, 'imhist_lotsno' ))
		// 전표번호 기록-중복생성방지
		dw_list.setitem(lRow, 'imhist_ijpno', dw_hidden.getitemstring(k, 'imhist_iojpno' ))	
		dw_list.setitem(lRow, 'outqty', dw_hidden.getitemnumber(k, 'imhist_ioqty' ))
	NEXT
END IF

/* 재고선택 기능 */
//if dw_detail.enabled then
//
//	p_addrow.triggerevent(clicked!)
//	
...
string	sArg_Jpno, sAuto, sError, sJpno
...
IF ic_status = '1'	THEN
	...
	IF wf_imhist_create(sArg_Jpno) = -1 THEN
		ROLLBACK;
		RETURN
	END IF
...
//	if sAuto = 'N' Then
//		sError = 'X';
//		sqlca.MM10_IMHIST_O01_COIL(gs_sabu, sArg_Jpno, 'I', sError);
//		if sError = 'X' or sError = 'Y' then
//			f_message_chk(41, '[생산자동출고]')
//			Rollback;
//			return 
...
//	end if
	...
	MessageBox("전표번호 확인", "전표번호 : " + left(sArg_Jpno, 8)+ '-' + right(sArg_Jpno,4)+		&
										 "~r~r생성되었습니다.")	
	
	if cbx_1.checked then 
		dw_print.retrieve(gs_sabu, sArg_Jpno, sArg_Jpno, gs_saupj)
		dw_print.print()
   end if      										 
ELSE
...
integer height = 1872
integer taborder = 30
string dataobject = "d_pdt_04035_han_1"
boolean hscrollbar = true
boolean vscrollbar = true
...
integer height = 312
integer taborder = 10
string dataobject = "d_pdt_04035_han"
boolean border = false
boolean livescroll = true
end type
