...
$PBExportComments$자동전표 관리 : 매출(계산서기준)
forward
...
IF sVatGbn <> '29' AND sVatGbn <> '24' THEN 
	...
	dw_junpoy.SetItem(iCurRow,"descr",   '예수부가세')	
...
//		sSaupj   = dw_rtv.GetItemString(k,"kif03ot0_sabu")
		...
		iLoopCnt = dw_group_detail.Retrieve(sChkNo)								/*처리할 물품 건수*/
		...
		FOR iCnt = 1 TO iLoopCnt
			...
			iItemDetailCnt = dw_detail.Retrieve(sChkNo,sAcc1_Dae+sAcc2_Dae)		/*물품 건수*/
			...
			dw_junpoy.SetItem(iCurRow,"descr",   dw_detail.GetItemString(1,"itdsc") + ' 외')	
		...
		sDcGbn = '1'										
		dw_group_detail.SetFilter("chgbn = '1'")						/*내수-차변계정(외상매출금,미수금)*/
		dw_group_detail.Filter()
		
		iLoopCnt = dw_group_detail.RowCount()
		IF iLoopCnt <=0 THEN
			IF sExpGbn = '1' THEN								/*내수-차변계정(외상매출금)*/
			...
			dw_junpoy.SetItem(iCurRow,"descr",   '제품매출 외')	
	...
	//		dw_junpoy.SetItem(iCurRow,"sdept_cd",dw_rtv.GetItemString(k,"cost_cd"))
		...
		ELSE
			...
			FOR iCnt = 1 TO iLoopCnt
				...
				IF sSaleGbn = '1' OR sSaleGbn = '2' OR sSaleGbn = '3' THEN									/*일반매출*/
					IF sExpGbn = '1' THEN								/*내수-차변계정(외상매출금)*/
						SELECT SUBSTR("SYSCNFG"."DATANAME",1,5), SUBSTR("SYSCNFG"."DATANAME",6,2)
							INTO :sAcc1_Cha,								:sAcc2_Cha
							FROM "SYSCNFG"
