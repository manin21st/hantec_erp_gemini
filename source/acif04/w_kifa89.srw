$PBExportHeader$w_kifa89.srw
$PBExportComments$본점 인수전표 처리
forward
global type w_kifa89 from w_inherite
end type
type rr_1 from roundrectangle within w_kifa89
end type
type gb_1 from groupbox within w_kifa89
end type
type rb_1 from radiobutton within w_kifa89
end type
type rb_2 from radiobutton within w_kifa89
end type
type dw_junpoy from datawindow within w_kifa89
end type
type dw_sungin from datawindow within w_kifa89
end type
type dw_print from datawindow within w_kifa89
end type
type dw_sang from datawindow within w_kifa89
end type
type dw_ip from u_key_enter within w_kifa89
end type
type cbx_all from checkbox within w_kifa89
end type
type dw_rtv from datawindow within w_kifa89
end type
type dw_delete from datawindow within w_kifa89
end type
end forward

global type w_kifa89 from w_inherite
integer height = 2468
string title = "본점 인수 전표 처리"
rr_1 rr_1
gb_1 gb_1
rb_1 rb_1
rb_2 rb_2
...
For k = 1 TO dw_delete.RowCount()
	IF dw_delete.GetItemString(k,"chk") = '1' THEN
		...
		ELSE
			update kfz10ot9
				set rcv_gbn = 'N',
					 saupj_rcv = null, baldate_rcv = null, upmugu_rcv = null, bjunno_rcv = null
				where saupj_rcv = :sSaupj and baldate_rcv = :sBalDate and upmugu_rcv = :sUpmuGu and bjunno_rcv = :lJunNo ;
		END IF
	END IF
NEXT
COMMIT;
...
public function integer wf_insert_kfz12ot0 (string ssaupj, string sbaldate);/************************************************************************************/
/* 본점 인수 자동전표를 처리한다.																	*/
/* 1. 채권 : 차변 - 선택된 자료의 계정과목														*/
/*           대변 - 보낸 사업장의 지점 계정과목													*/
/* 2. 채무 : 차변 - 보낸사업장의 지점 계정과목													*/
/*           대변 - 선택된 자료의 계정과목														*/
/************************************************************************************/
Integer k,iCurRow
String  sAcCd,sChaDae,sAcc1_Cha,sAcc2_Cha,sDcGbn,sAcc1_Dae,sAcc2_Dae,sSendSaupj
Long    lJunNo,lLinNo
Double  dAmount

dw_ip.AcceptText()
sAcCd      = dw_ip.GetItemString(1,"accd")
sSendSaupj = dw_ip.GetItemString(1,"send_saupj")

select dc_gu into :sChaDae from kfz01om0 where acc1_cd||acc2_cd = :sAcCd ;

w_mdi_frame.sle_msg.text =""

w_mdi_frame.sle_msg.text ="본점 인수 전표 처리 중 ..."
...
if sChaDae = '1' then							/*자산계정*/
	...
	FOR k = 1 TO dw_rtv.RowCount()	
		...
		sAcc1_Cha = Left(dw_rtv.GetItemString(k,"acno"),5)
		sAcc2_Cha = Right(dw_rtv.GetItemString(k,"acno"),2)
		...
		dw_junpoy.SetItem(iCurRow,"descr",   f_get_refferance('AD',sSendSaupj)+'에서 송부')
	...
	select substr(rfna3,1,5), substr(rfna3,6,2) into :sAcc1_Dae, :sAcc2_Dae
		from reffpf where rfcod = 'BR' and rfgub = :sSendSaupj ;
	...
	select substr(rfna3,1,5), substr(rfna3,6,2) into :sAcc1_Cha, :sAcc2_Cha
		from reffpf where rfcod = 'BR' and rfgub = :sSendSaupj ;
	...
	FOR k = 1 TO dw_rtv.RowCount()	
		...
		sAcc1_Dae = Left(dw_rtv.GetItemString(k,"acno"),5)
		sAcc2_Dae = Right(dw_rtv.GetItemString(k,"acno"),2)
...
IF dw_junpoy.Update() <> 1 THEN
	...
	Return -1
ELSE
	String sMain_saupj,sMain_AccDate,sMain_UpmuGu= 'S',sMain_BalDate
	Long   lMain_JunNo, lMain_bJunno
	
	w_mdi_frame.sle_msg.text = '본지점 내역 갱신 중...'
	/*본지점 대체 내역 갱신*/
	for k = 1 to dw_rtv.RowCount()
		sMain_Saupj   = dw_rtv.GetItemString(k,"saupj")
		sMain_AccDate = dw_rtv.GetItemString(k,"acc_date")
		lMain_JunNo	  = dw_rtv.GetItemNumber(k,"jun_no")
		sMain_BalDate = dw_rtv.GetItemString(k,"bal_date")
		lMain_bJunNo  = dw_rtv.GetItemNumber(k,"bjun_no")
		
		update kfz10ot9
			set rcv_gbn = 'Y',
				 saupj_rcv = :sSaupj, baldate_rcv = :sBalDate, upmugu_rcv = :sUpmuGbn, bjunno_rcv = :lJunNo
			where saupj = :sMain_Saupj and acc_date = :sMain_AccDate and upmu_gu = :sMain_UpmuGu and
					jun_no = :lMain_JunNo and bal_date = :sMain_BalDate and bjun_no = :lMain_BjunNo ;
	next
END IF
COMMIT;

w_mdi_frame.sle_msg.text ="본점 인수 전표 처리 완료!!"
...
type dw_insert from w_inherite`dw_insert within w_kifa89
...
type p_delrow from w_inherite`p_delrow within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_addrow from w_inherite`p_addrow within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_search from w_inherite`p_search within w_kifa89
boolean visible = false
integer x = 3776
integer y = 2940
integer taborder = 0
...
type p_ins from w_inherite`p_ins within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_exit from w_inherite`p_exit within w_kifa89
integer y = 16
integer taborder = 60
end type

type p_can from w_inherite`p_can within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_print from w_inherite`p_print within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type
...
type p_inq from w_inherite`p_inq within w_kifa89
integer x = 4096
integer y = 16
integer taborder = 20
end type
...
type p_del from w_inherite`p_del within w_kifa89
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_mod from w_inherite`p_mod within w_kifa89
integer x = 4270
integer y = 16
...
select nvl(rfgub,'10') into :sSaupj from reffpf where rfcod = 'BR' and rfna5  = 'Y' ;
if sqlca.sqlcode <> 0 then
	w_mdi_frame.sle_msg.text = '참조코드 "BR"을 확인하십시요.'
	MessageBox('확 인','본점코드가 설정되지 않았습니다.')
	Return
end if
...
type cb_exit from w_inherite`cb_exit within w_kifa89
boolean visible = false
integer x = 4302
integer y = 2752
end type

type cb_mod from w_inherite`cb_mod within w_kifa89
boolean visible = false
integer x = 3941
integer y = 2752
string text = "처리(&P)"
...
type cb_ins from w_inherite`cb_ins within w_kifa89
boolean visible = false
integer x = 2350
integer y = 2748
...
type cb_del from w_inherite`cb_del within w_kifa89
boolean visible = false
integer x = 2693
integer y = 2748
end type

type cb_inq from w_inherite`cb_inq within w_kifa89
boolean visible = false
integer x = 3579
integer y = 2752
end type

type cb_print from w_inherite`cb_print within w_kifa89
boolean visible = false
integer x = 2706
integer y = 2748
end type

type st_1 from w_inherite`st_1 within w_kifa89
boolean visible = false
end type

type cb_can from w_inherite`cb_can within w_kifa89
boolean visible = false
integer x = 2345
integer y = 2628
end type

type cb_search from w_inherite`cb_search within w_kifa89
boolean visible = false
integer x = 3113
integer y = 2756
integer width = 439
...
type dw_datetime from w_inherite`dw_datetime within w_kifa89
boolean visible = false
end type

type sle_msg from w_inherite`sle_msg within w_kifa89
boolean visible = false
integer height = 88
end type

type gb_10 from w_inherite`gb_10 within w_kifa89
boolean visible = false
end type

type gb_button1 from w_inherite`gb_button1 within w_kifa89
boolean visible = false
integer x = 2304
integer y = 2580
integer height = 308
...
type gb_button2 from w_inherite`gb_button2 within w_kifa89
boolean visible = false
integer x = 3067
integer y = 2696
integer width = 1609
