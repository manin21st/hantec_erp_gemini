$PBExportHeader$wp_pik2010.srw
$PBExportComments$근태 항목별 조회현황
forward
global type wp_pik2010 from w_standard_print
end type
type rr_2 from roundrectangle within wp_pik2010
end type
end forward

global type wp_pik2010 from w_standard_print
string title = "근태 항목별 조회현황"
rr_2 rr_2
end type
global wp_pik2010 wp_pik2010
...
public function integer wf_retrieve ();string vSaupcd,vKgubn,vJgubn,vSdate,vEdate,vEmpno,vKtae,vDate

dw_ip.AcceptText()

vSaupcd = dw_ip.GetItemString(1,'saupcd')
vKgubn = dw_ip.GetItemString(1,'kgubn')
...
vKtae = dw_ip.GetItemString(1,'ktae')
...
if isnull(vSaupcd) or vSaupcd = '' then
	vSaupcd = '%'
end if

if isnull(vKgubn) or vKgubn = '' then
	vKgubn = '%'
end if
...
if isnull(vKtae) or vKtae = '' then
	vKtae = '%'
end if

if dw_Print.Retrieve(vSaupcd, vKgubn, vJgubn, vSdate, vEdate, vEmpno, vKtae) < 1 then
...
type p_preview from w_standard_print`p_preview within wp_pik2010
end type

type p_exit from w_standard_print`p_exit within wp_pik2010
end type

type p_print from w_standard_print`p_print within wp_pik2010
end type

type p_retrieve from w_standard_print`p_retrieve within wp_pik2010
end type
...
type st_10 from w_standard_print`st_10 within wp_pik2010
end type



type dw_print from w_standard_print`dw_print within wp_pik2010
string dataobject = "dp_pik2010_2_p"
end type

type dw_ip from w_standard_print`dw_ip within wp_pik2010
integer y = 12
integer width = 3355
integer height = 288
string dataobject = "dp_pik2010_1"
end type
...
type dw_list from w_standard_print`dw_list within wp_pik2010
integer x = 32
integer y = 320
integer width = 4581
string dataobject = "dp_pik2010_2"
