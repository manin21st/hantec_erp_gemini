$PBExportHeader$w_cic01045.srw
$PBExportComments$부산물수불생성
forward
global type w_cic01045 from w_inherite
end type
type dw_2 from datawindow within w_cic01045
end type
type gb_4 from groupbox within w_cic01045
end type
type dw_1 from datawindow within w_cic01045
end type
type dw_print from datawindow within w_cic01045
end type
type rr_1 from roundrectangle within w_cic01045
end type
end forward
...
type dw_insert from w_inherite`dw_insert within w_cic01045
...
type p_delrow from w_inherite`p_delrow within w_cic01045
boolean visible = false
integer x = 5202
integer y = 184
integer taborder = 0
...
type p_addrow from w_inherite`p_addrow within w_cic01045
boolean visible = false
integer x = 5029
integer y = 184
integer taborder = 0
...
type p_search from w_inherite`p_search within w_cic01045
boolean visible = false
integer x = 3566
integer taborder = 20
...
//
//
//DECLARE SP_CREATE_CIC0110_SP PROCEDURE FOR CIC0125_SP (:sWorkym) USING SQLCA;
//EXECUTE SP_CREATE_CIC0110_SP;
////STRING sPrcRtn
////FETCH SP_CREATE_CIC0110_SP INTO :sPrcRtn ;
//CLOSE SP_CREATE_CIC0110_SP;
//If SQLCA.SQLCODE < 0 Then
//   f_message_chk(57, "~r~n~r~n[EXECUTE SP_CREATE_CIC0125_SP]~r~n~r~n" + SQLCA.SQLERRTEXT)	// 자료를 처리할 수 없습니다.
//else
...
type p_ins from w_inherite`p_ins within w_cic01045
integer x = 4087
integer taborder = 60
...
DELETE FROM cic0125 WHERE workym = :ls_workym;
IF SQLCA.SQLCODE = 0 THEN
	COMMIT;
...
type p_exit from w_inherite`p_exit within w_cic01045
integer x = 4434
end type

type p_can from w_inherite`p_can within w_cic01045
integer x = 4261
end type
...
type p_print from w_inherite`p_print within w_cic01045
boolean visible = false
integer x = 3397
integer taborder = 0
end type
...
type p_inq from w_inherite`p_inq within w_cic01045
integer x = 3739
integer taborder = 30
end type
...
type p_del from w_inherite`p_del within w_cic01045
integer x = 4855
integer y = 184
integer taborder = 0
end type
...
type p_mod from w_inherite`p_mod within w_cic01045
integer x = 3913
end type

event p_mod::clicked;call super::clicked;String sWorkym, sSaupj, sItnbr
...
type cb_exit from w_inherite`cb_exit within w_cic01045
integer x = 3026
integer y = 2752
end type

type cb_mod from w_inherite`cb_mod within w_cic01045
integer x = 2199
integer y = 2752
end type
...
type cb_ins from w_inherite`cb_ins within w_cic01045
integer x = 503
integer y = 2752
end type
...
type cb_del from w_inherite`cb_del within w_cic01045
integer x = 2551
integer y = 2752
end type
...
type cb_inq from w_inherite`cb_inq within w_cic01045
integer x = 142
integer y = 2752
end type
...
type cb_print from w_inherite`cb_print within w_cic01045
integer x = 1650
integer y = 2424
end type

type st_1 from w_inherite`st_1 within w_cic01045
integer width = 293
end type

type cb_can from w_inherite`cb_can within w_cic01045
integer x = 2907
integer y = 2752
end type
...
type cb_search from w_inherite`cb_search within w_cic01045
integer x = 2053
integer y = 2432
end type

type dw_datetime from w_inherite`dw_datetime within w_cic01045
integer height = 88
end type

type sle_msg from w_inherite`sle_msg within w_cic01045
integer x = 329
integer width = 2533
end type

type gb_10 from w_inherite`gb_10 within w_cic01045
integer width = 3607
end type

type gb_button1 from w_inherite`gb_button1 within w_cic01045
integer x = 110
integer y = 2696
end type

type gb_button2 from w_inherite`gb_button2 within w_cic01045
integer x = 2153
integer y = 2696
end type
...
integer taborder = 90
string title = "부산물수불조정"
string dataobject = "dw_cic01045_20"
...
/******************************************************************************
입고금액 또는 입고수량 변경시  [기초+입고 = 재고 + 출고]  --> 출고금액,    재고금액, 재고수량을 변경시킴
******************************************************************************/
if dwo.name='ip_amt' or dwo.name='ip_qty' then
	if dwo.name = 'ip_amt' then
	   ll_ip_amt = long(data)
	elseif dwo.name = 'ip_qty' then
	  ll_ip_qty = long(data)
    end if
	 ll_op_amt = ((ll_iwol_amt+ll_ip_amt) / (ll_iwol_qty+ll_ip_qty))  *  ll_op_qty    //출고금액 = (기초금액+입고금액) / (기초수량+입고수량) * 출고단가
	 dw_2.Object.op_amt[row] = ll_op_amt
	 
	 ll_wp_amt = ll_iwol_amt + ll_ip_amt //- ll_ot_amt                                       //재고금액 = 기초금액 + 입고금액
	 dw_2.Object.wp_amt[row] = ll_wp_amt
	 
	 ll_wp_qty = ll_iwol_qty + ll_ip_qty //- ll_ot_qty                                           //재고수량	  = 기초수량 + 입고수량
	 dw_2.Object.wp_qty[row] = ll_wp_qty	 
	 
	 dw_2.Object.ip_danga[row] = ROUND(ll_ip_amt / ll_ip_qty, 1)
/******************************************************************************
재고수량 변경시  [기초+입고 = 재고 + 출고]                   --> 재고금액,       출고수량, 출고금액을 변경시킴
******************************************************************************/
elseif dwo.name='wp_qty' then
      ll_wp_qty =  long(data)
	 //ll_op_qty = ll_iwol_qty + ll_ip_qty - ll_ot_qty - ll_wp_qty  //출고수량
	 ll_op_qty = ll_iwol_qty + ll_ip_qty - ll_wp_qty                                              //출고수량 = 기초수량 + 입고수량 - 재고수량
	 dw_2.Object.op_qty[row] = ll_op_qty          
	 
	 ll_wp_amt = ((ll_iwol_amt+ll_ip_amt) / (ll_iwol_qty+ll_ip_qty))  *  ll_wp_qty //재고금액 = (기초금액+입고금액) / (기초수량+입고수량) * 재고수량
	 dw_2.Object.wp_amt[row] = ll_wp_amt
	 
	 //ll_op_amt = ll_iwol_amt + ll_ip_amt - ll_ot_amt - ll_wp_amt //출고금액
	 ll_op_amt = ll_iwol_amt + ll_ip_amt  - ll_wp_amt                                       //출고금액 = 기초금액 + 입고금액 - 재고금액
	 dw_2.Object.op_amt[row] = ll_op_amt
	 
	 dw_2.Object.wp_danga[row] = ROUND(ll_wp_amt / ll_wp_qty, 1)
end if
...
event constructor;Object.DataWindow.HorizontalScrollSplit = 1883
end event
...
integer height = 156
integer taborder = 10
string dataobject = "dw_cic01045_10"
boolean border = false
boolean livescroll = true
end type
