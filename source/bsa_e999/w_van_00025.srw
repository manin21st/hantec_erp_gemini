$PBExportHeader$w_van_00025.srw
$PBExportComments$검수자료 등록- VAN
forward
global type w_van_00025 from w_inherite
end type
type dw_ip from u_key_enter within w_van_00025
end type
type dw_chulgo from u_key_enter within w_van_00025
end type
type dw_imhist from datawindow within w_van_00025
end type
type st_msg from statictext within w_van_00025
end type
type lb_1 from listbox within w_van_00025
end type
type cb_1 from commandbutton within w_van_00025
end type
type cb_2 from commandbutton within w_van_00025
end type
type pb_1 from u_pb_cal within w_van_00025
end type
type pb_2 from u_pb_cal within w_van_00025
end type
type gb_2 from groupbox within w_van_00025
end type
type gb_3 from groupbox within w_van_00025
end type
type gb_4 from groupbox within w_van_00025
end type
type gb_5 from groupbox within w_van_00025
end type
type rr_1 from roundrectangle within w_van_00025
end type
end forward

global type w_van_00025 from w_inherite
string title = " 납품검수 등록(후채무)"
dw_ip dw_ip
dw_chulgo dw_chulgo
dw_imhist dw_imhist
st_msg st_msg
...
global w_van_00025 w_van_00025
...
dw_imhist.SetItem(iCurRow, "FIELD_CD",   dw_chulgo.GetItemString(nRow, "FIELD_CD"))
...
dw_imhist.SetItem(iCurRow, "LCLGBN", 	lcgb)	
...
If dw_chulgo.GetItemString(nRow, 'chk') = 'Y' Then
	If IsNull(sDate) Or sdate = '' Then
		...
		dw_chulgo.SetItem(nRow, 'lclgbn', 'V')
		dw_chulgo.SetItem(nRow, 'field_cd', dw_chulgo.GetItemString(nRow, 'field_cd', Primary!, True))
	Else
		dw_chulgo.SetItem(nRow, 'yebi1',   sDate)
		dw_chulgo.SetItem(nRow, 'lclgbn', dw_insert.GetItemString(nSel, 'localyn'))
		dw_chulgo.SetItem(nRow, 'field_cd', dw_insert.GetItemString(nSel, 'baljuno'))
	End If
Else
	dw_chulgo.SetItem(nRow, 'yebi1',   sNull)
	dw_chulgo.SetItem(nRow, 'lclgbn', 'V')
...
type dw_insert from w_inherite`dw_insert within w_van_00025
integer x = 59
integer y = 368
integer width = 4434
integer height = 1216
string dataobject = "d_van_00025_2"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
end type

event dw_insert::clicked;String sBalNo, sPlnt, sDate, sSeq, sCvcod, sLocalYn
...
If dw_ip.GetItemString(1, 'crtgu') = '3' Then
	...
	dw_chulgo.Retrieve(gs_sabu, sBalNo, sCvcod, sDate, sPlnt)
Else
	sBalNo   = Trim(GetItemString(Row,'baljuno'))
	sLocalYn = Trim(GetItemString(Row,'localyn'))
	
	sSeq = Trim(dw_insert.GetItemString(Row, 'regseq'))
	If IsNull(sSeq) Or sSeq = '' Then sSeq = '0000'
	
	dw_chulgo.Retrieve(gs_sabu, sBalNo, sLocalYn)
End If
...
If dwo.name = 'b_del' Then
	If MessageBox(string(row) + '행','삭제하시겠습니까?', Information!, YesNo!, 2) = 2 Then Return
	
	Deleterow(row)
	
	w_mdi_frame.sle_msg.text = '삭제하였습니다.!! 저장버튼을 누르세요.'
End If
end event

type p_delrow from w_inherite`p_delrow within w_van_00025
...
type p_addrow from w_inherite`p_addrow within w_van_00025
boolean visible = false
integer x = 3474
integer y = 172
...
type p_search from w_inherite`p_search within w_van_00025
boolean visible = false
integer x = 3296
...
type p_ins from w_inherite`p_ins within w_van_00025
integer x = 3479
...
event p_ins::clicked;call super::clicked;String sOrderDate, sVndgu, sCvcod, senddate, sCrtgu, sItnbr
...
//프로시저 선언
DECLARE ERP_VAN_GUMSU_SEQ procedure for ERP_VAN_GUMSU_SEQ(:gs_sabu, :sOrderDate, :senddate, 'C') ;

Execute ERP_VAN_GUMSU_SEQ;
//If sqlca.sqlcode <> 0 Then
//	MessageBox(string(sqlca.sqlcode),sqlca.sqlerrtext)
//	ROLLBACK;	
//Else
...
type p_exit from w_inherite`p_exit within w_van_00025
end type

type p_can from w_inherite`p_can within w_van_00025
end type
...
type p_print from w_inherite`p_print within w_van_00025
boolean visible = false
integer x = 4430
integer y = 192
...
type p_inq from w_inherite`p_inq within w_van_00025
integer x = 3918
end type
...
type p_del from w_inherite`p_del within w_van_00025
integer x = 3653
string picturename = "C:\erpman\image\일괄삭제_up.gif"
end type

event p_del::clicked;call super::clicked;String sOrderDate, sVndgu, sCvcod, senddate, sCrtgu, sItnbr
...
IF MessageBox("확 인","일괄 삭제 처리를 하시겠습니까?",Question!,YesNo!) = 2 THEN RETURN -1

//프로시저 선언
DECLARE ERP_VAN_GUMSU_SEQ procedure for ERP_VAN_GUMSU_SEQ(:gs_sabu, :sOrderDate, :senddate, 'D') ;
...
event p_del::ue_lbuttondown;PictureName = "C:\erpman\image\일괄삭제_dn.gif"
end event

event p_del::ue_lbuttonup;PictureName = "C:\erpman\image\일괄삭제_up.gif"
end event

type p_mod from w_inherite`p_mod within w_van_00025
integer x = 4096
end type
...
type cb_exit from w_inherite`cb_exit within w_van_00025
integer x = 3182
integer y = 1924
end type

type cb_mod from w_inherite`cb_mod within w_van_00025
integer x = 2487
integer y = 1924
end type

type cb_ins from w_inherite`cb_ins within w_van_00025
integer x = 1413
integer y = 1924
boolean enabled = false
string text = "정산(&I)"
...
type cb_del from w_inherite`cb_del within w_van_00025
integer x = 933
integer y = 1924
boolean enabled = false
end type

type cb_inq from w_inherite`cb_inq within w_van_00025
integer x = 549
integer y = 1924
end type

type cb_print from w_inherite`cb_print within w_van_00025
integer x = 997
integer y = 1924
boolean enabled = false
end type

type st_1 from w_inherite`st_1 within w_van_00025
end type

type cb_can from w_inherite`cb_can within w_van_00025
integer x = 2834
integer y = 1924
end type

type cb_search from w_inherite`cb_search within w_van_00025
integer x = 105
integer y = 1924
integer width = 421
string text = "VAN조회(&W)"
...
type gb_button1 from w_inherite`gb_button1 within w_van_00025
end type

type gb_button2 from w_inherite`gb_button2 within w_van_00025
end type
...
integer height = 184
integer taborder = 10
string dataobject = "d_van_00025_1"
boolean border = false
end type
...
//		END IF
	Case 'crtgu'
		Choose Case GetText()
			Case '1'
				dw_insert.SetFilter("")
				dw_chulgo.DataObject = 'd_van_00025_3'
			Case '2'
				dw_insert.SetFilter("cnfirm = 'Y'")
				dw_chulgo.DataObject = 'd_van_00025_3'
			Case '3'
				dw_insert.SetFilter("cnfirm = 'N'")
				dw_chulgo.DataObject = 'd_van_00025_4'
		End Choose
...
string dataobject = "d_van_00025_4"
boolean vscrollbar = true
boolean border = false
end type
...
//					dNewVatAmt = Truncate(dChoiceQty * dIoPrc,0)
				...
				If ix = sRow Then
					...
					dw_chulgo.SetItem(1, 'lclgbn', sLocalYn)
