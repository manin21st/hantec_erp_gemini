$PBExportHeader$w_han_01070.srw
$PBExportComments$작업지시 취소현황
forward
global type w_han_01070 from w_standard_print
end type
type pb_1 from u_pb_cal within w_han_01070
end type
type pb_2 from u_pb_cal within w_han_01070
end type
type rr_1 from roundrectangle within w_han_01070
end type
end forward
...
integer width = 4667
integer height = 2596
string title = "작업지시 취소현황"
pb_1 pb_1
pb_2 pb_2
rr_1 rr_1
end type
global w_han_01070 w_han_01070
...
ls_stim = dw_ip.GetItemString(ll_row, 'stim') + '00'
If Trim(ls_stim) = '' OR IsNull(ls_stim) Then ls_stim = '080000'

String ls_etim

ls_etim = dw_ip.GetItemString(ll_row, 'etim') + '00'
If Trim(ls_etim) = '' OR IsNull(ls_etim) Then ls_etim = '063000'
...
ls_sdt = LEFT(ls_st, 4) + '-' + MID(ls_st, 5, 2) + '-' + RIGHT(ls_st, 2) + ' ' + &
         LEFT(ls_stim, 2) + ':' + MID(ls_stim, 3, 2) + ':' + RIGHT(ls_stim, 2)
ls_edt = LEFT(ls_ed, 4) + '-' + MID(ls_ed, 5, 2) + '-' + RIGHT(ls_ed, 2) + ' ' + &
         LEFT(ls_etim, 2) + ':' + MID(ls_etim, 3, 2) + ':' + RIGHT(ls_etim, 2)

dw_list.SetRedraw(False)
dw_list.Retrieve(ls_itnbr, ls_wkctr, ls_jocod, ls_sdt, ls_edt)
...
type p_xls from w_standard_print`p_xls within w_han_01070
...
type p_sort from w_standard_print`p_sort within w_han_01070
boolean visible = true
integer x = 4421
integer y = 164
...
type p_preview from w_standard_print`p_preview within w_han_01070
boolean visible = false
integer x = 4247
end type

type p_exit from w_standard_print`p_exit within w_han_01070
integer x = 4421
end type

type p_print from w_standard_print`p_print within w_han_01070
boolean visible = false
integer x = 3890
integer y = 20
end type

type p_retrieve from w_standard_print`p_retrieve within w_han_01070
integer x = 4073
end type
...
type st_10 from w_standard_print`st_10 within w_han_01070
end type



type dw_print from w_standard_print`dw_print within w_han_01070
integer x = 3749
string dataobject = "d_han_01070_201"
end type

type dw_ip from w_standard_print`dw_ip within w_han_01070
integer x = 37
integer width = 2565
integer height = 288
string dataobject = "d_han_01070_101"
end type
...
type dw_list from w_standard_print`dw_list within w_han_01070
integer y = 324
integer width = 4558
integer height = 1924
string dataobject = "d_han_01070_201"
