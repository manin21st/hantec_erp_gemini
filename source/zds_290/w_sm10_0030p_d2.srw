$PBExportHeader$w_sm10_0030p_d2.srw
$PBExportComments$MOBIS AUTO 주간 소요 현황
forward
global type w_sm10_0030p_d2 from w_inherite
end type
type p_1 from uo_picture within w_sm10_0030p_d2
end type
type p_dellrow_all from uo_picture within w_sm10_0030p_d2
end type
type rr_1 from roundrectangle within w_sm10_0030p_d2
end type
type dw_1 from datawindow within w_sm10_0030p_d2
end type
type pb_1 from u_pb_cal within w_sm10_0030p_d2
end type
type pb_2 from u_pb_cal within w_sm10_0030p_d2
end type
type p_preview from picture within w_sm10_0030p_d2
end type
type dw_print from datawindow within w_sm10_0030p_d2
end type
end forward

global type w_sm10_0030p_d2 from w_inherite
string title = "D2계획 일자별 변화추이 현황"
p_1 p_1
...
global w_sm10_0030p_d2 w_sm10_0030p_d2
...
string st_date, ed_date, tm_date

// 기준일자는 전주 수요일(4)
tm_date = f_afterday(is_today, -11)

//select min(cldate)
//  into :st_date
//  from p4_calendar
// where companycode = 'KN'
//   and cldate > :tm_date and cldate < :is_today
//   and to_char(to_date(cldate,'yyyymmdd'),'d') = 4 ;

select min(substr(doccode,3,8))
  into :st_date
  from van_hkcd2
 where sabu = '1' 
   and doccode > 'D2'||:tm_date and doccode < 'D2'||:is_today
   and to_char(to_date(substr(doccode,3,8),'yyyymmdd'),'d') = 4 ;

dw_1.Object.jisi_date[1] = st_date


// 비교일자는 현재일자 포함 최종접수일
select max(substr(doccode,3,8))
  into :ed_date
  from van_hkcd2
 where sabu = '1' and doccode <= 'D2'||:is_today ;

dw_1.Object.jisi_date2[1] = ed_date
...
type dw_insert from w_inherite`dw_insert within w_sm10_0030p_d2
integer x = 55
integer y = 360
integer width = 4530
integer height = 1900
string dataobject = "d_sm10_0030p_d2_2"
...
type p_delrow from w_inherite`p_delrow within w_sm10_0030p_d2
...
type p_addrow from w_inherite`p_addrow within w_sm10_0030p_d2
boolean visible = false
integer x = 3479
integer y = 212
end type

type p_search from w_inherite`p_search within w_sm10_0030p_d2
boolean visible = false
integer x = 3342
integer y = 56
end type

type p_ins from w_inherite`p_ins within w_sm10_0030p_d2
boolean visible = false
integer x = 3122
integer y = 0
end type

type p_exit from w_inherite`p_exit within w_sm10_0030p_d2
end type
...
type p_can from w_inherite`p_can within w_sm10_0030p_d2
boolean visible = false
end type

type p_print from w_inherite`p_print within w_sm10_0030p_d2
integer x = 4096
...
type p_inq from w_inherite`p_inq within w_sm10_0030p_d2
integer x = 3922
end type

event p_inq::clicked;call super::clicked;String ls_ymd , ls_ymd2 , ls_saupj , ls_saupj_cust, ls_factory ,ls_gubun, tx_saupj, tx_mcvcod
String ls_itnbr_from, ls_itnbr_to, ls_from, ls_to, ls_cvcod
Integer li_no, li_dgag
...
if f_datechk(ls_ymd) <> 1 then
	MessageBox('에러','기준일자가 잘못 지정되었습니다!!!')
	dw_1.SetColumn('jisi_date')
	dw_1.SetFocus()
	Return -1
end if
ls_ymd2 = Trim(dw_1.Object.jisi_date2[1])
if f_datechk(ls_ymd2) <> 1 then
	MessageBox('에러','비교일자가 잘못 지정되었습니다!!!')
	dw_1.SetColumn('jisi_date2')
	dw_1.SetFocus()
	Return -1
end if
li_dgag = f_dayterm(ls_ymd, ls_ymd2)
if li_dgag < 0 then
	MessageBox('에러','비교일자는 기준일자 이후로 지정하십시오!!!')
	dw_1.SetColumn('jisi_date2')
	dw_1.SetFocus()
	Return -1
end if
...
if dw_insert.Retrieve(ls_saupj_cust , ls_ymd , ls_ymd2, ls_cvcod ,ls_from, ls_to , ls_factory, li_no, li_dgag) < 1 then
...
if dw_print.Retrieve(ls_saupj_cust , ls_ymd , ls_ymd2, ls_cvcod ,ls_from, ls_to , ls_factory, li_no, li_dgag) < 1 then
	f_message_chk(50,"")
	return -1
end if
...
type p_del from w_inherite`p_del within w_sm10_0030p_d2
boolean visible = false
end type

type p_mod from w_inherite`p_mod within w_sm10_0030p_d2
boolean visible = false
end type

type cb_exit from w_inherite`cb_exit within w_sm10_0030p_d2
end type

type cb_mod from w_inherite`cb_mod within w_sm10_0030p_d2
end type

type cb_ins from w_inherite`cb_ins within w_sm10_0030p_d2
end type

type cb_del from w_inherite`cb_del within w_sm10_0030p_d2
end type

type cb_inq from w_inherite`cb_inq within w_sm10_0030p_d2
end type

type cb_print from w_inherite`cb_print within w_sm10_0030p_d2
end type

type st_1 from w_inherite`st_1 within w_sm10_0030p_d2
end type

type cb_can from w_inherite`cb_can within w_sm10_0030p_d2
end type

type cb_search from w_inherite`cb_search within w_sm10_0030p_d2
end type
...
type gb_button1 from w_inherite`gb_button1 within w_sm10_0030p_d2
end type

type gb_button2 from w_inherite`gb_button2 within w_sm10_0030p_d2
end type
...
integer taborder = 50
string title = "none"
string dataobject = "d_sm10_0030p_d2_1"
boolean border = false
