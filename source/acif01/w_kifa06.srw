$PBExportHeader$w_kifa06.srw
$PBExportComments$자동전표 관리 : 수금(반제를 회계에서 처리)
forward
global type w_kifa06 from w_inherite
end type
type rr_1 from roundrectangle within w_kifa06
end type
type gb_1 from groupbox within w_kifa06
end type
type rb_1 from radiobutton within w_kifa06
end type
type rb_2 from radiobutton within w_kifa06
end type
type dw_junpoy from datawindow within w_kifa06
end type
type dw_sungin from datawindow within w_kifa06
end type
type dw_group_detail_cause from datawindow within w_kifa06
end type
type dw_print from datawindow within w_kifa06
end type
type dw_rbill from datawindow within w_kifa06
end type
type dw_sang from datawindow within w_kifa06
end type
type dw_group_detail_type from datawindow within w_kifa06
end type
type dw_ip from u_key_enter within w_kifa06
end type
type cbx_all from checkbox within w_kifa06
end type
type dw_rtv from datawindow within w_kifa06
end type
type dw_delete from datawindow within w_kifa06
end type
type dw_sanglst from datawindow within w_kifa06
end type
end forward
...
dw_rtv dw_rtv
dw_delete dw_delete
dw_sanglst dw_sanglst
end type
global w_kifa06 w_kifa06
...
//		sSaupj   = dw_rtv.GetItemString(k,"subu")
		...
		FOR i = 1 TO iDetailChaCount
			...
			ELSEIF sInsType = '4' OR sInsType = '5' or sInsType = '6' or sInsType = '7' THEN	
				...
				IF sSangGbn = 'Y' and sDcGbn <> sChaDae THEN
					IF Wf_Insert_Sang(sAcc1_Cha,sAcc2_Cha,dw_group_detail_type.GetItemString(i,"cvcod"),&
						dw_group_detail_type.GetItemNumber(i,"ipgum_amt"),sSaupj,sBalDate,sUpmuGbn,lJunNo,lLinNo, False) > 0 THEN																		
		...
		FOR i = 1 TO iDetailDaeCount
			...
			END IF
			IF sSangGbn = 'Y' AND sDcGbn <> sChaDae THEN
				IF Wf_Insert_Sang(sAcc1_Dae,sAcc2_Dae,dw_group_detail_cause.GetItemString(i,"cvcod"),&
														dw_group_detail_cause.GetItemNumber(i,"ipgum_amt"),&
														sSaupj,sBalDate,sUpmuGbn,lJunNo,lLinNo, False) > 0 THEN
					dw_junpoy.SetItem(iCurRow,"cross_gu",'Y') 
				ELSE
					Return -1
				END IF		
...
//FOR k = 1 TO iCount
//
//	dRemainAmt = dw_sanglst.GetItemNumber(k,"remain_amt")
//	
//	IF dRemainAmt >= dTempAmt THEN
//		dCurAmt  = dAmount								/*현재 추가할 금액*/
//		dTempAmt = 0										/*처리후 남은 금액*/
//	ELSE
//		dCurAmt  = dRemainAmt
//		dTempAmt = dTempAmt - dRemainAmt
//	END IF
//	
//	iInsertRow = dw_sang.InsertRow(0)
//	
//	dw_sang.SetItem(iInsertRow,"saupj",    dw_sanglst.GetItemString(k,"saupj"))
//	dw_sang.SetItem(iInsertRow,"acc_date", dw_sanglst.GetItemString(k,"acc_date"))
//	dw_sang.SetItem(iInsertRow,"upmu_gu",  dw_sanglst.GetItemString(k,"upmu_gu"))
//	dw_sang.SetItem(iInsertRow,"jun_no",   dw_sanglst.GetItemNumber(k,"jun_no"))
//	dw_sang.SetItem(iInsertRow,"lin_no",   dw_sanglst.GetItemNumber(k,"lin_no"))
//	dw_sang.SetItem(iInsertRow,"jbal_date",dw_sanglst.GetItemString(k,"bal_date"))
//	dw_sang.SetItem(iInsertRow,"bjun_no",  dw_sanglst.GetItemNumber(k,"bjun_no"))
//	
...
//	dw_sang.SetItem(iInsertRow,"amt_s",    dCurAmt)
//	
//	IF dTempAmt = 0 THEN EXIT
//NEXT
Return 1
end function
...
this.dw_sanglst=create dw_sanglst
...
this.Control[iCurrent+16]=this.dw_sanglst
end on
...
dw_sanglst.SetTransObject(SQLCA)
...
type dw_insert from w_inherite`dw_insert within w_kifa06
...
type p_delrow from w_inherite`p_delrow within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_addrow from w_inherite`p_addrow within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_search from w_inherite`p_search within w_kifa06
integer x = 3922
integer y = 16
integer taborder = 70
...
type p_ins from w_inherite`p_ins within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_exit from w_inherite`p_exit within w_kifa06
integer y = 16
integer taborder = 60
end type

type p_can from w_inherite`p_can within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_print from w_inherite`p_print within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type
...
type p_inq from w_inherite`p_inq within w_kifa06
integer x = 4096
integer y = 16
integer taborder = 20
end type
...
type p_del from w_inherite`p_del within w_kifa06
boolean visible = false
integer y = 3120
integer taborder = 0
end type

type p_mod from w_inherite`p_mod within w_kifa06
integer x = 4270
integer y = 16
...
type cb_exit from w_inherite`cb_exit within w_kifa06
boolean visible = false
integer x = 4302
integer y = 2752
end type

type cb_mod from w_inherite`cb_mod within w_kifa06
boolean visible = false
integer x = 3941
integer y = 2752
string text = "처리(&P)"
...
type cb_ins from w_inherite`cb_ins within w_kifa06
boolean visible = false
integer x = 2350
integer y = 2748
...
type cb_del from w_inherite`cb_del within w_kifa06
boolean visible = false
integer x = 2693
integer y = 2748
end type

type cb_inq from w_inherite`cb_inq within w_kifa06
boolean visible = false
integer x = 3579
integer y = 2752
end type

type cb_print from w_inherite`cb_print within w_kifa06
boolean visible = false
integer x = 2706
integer y = 2748
end type

type st_1 from w_inherite`st_1 within w_kifa06
boolean visible = false
end type

type cb_can from w_inherite`cb_can within w_kifa06
boolean visible = false
integer x = 2345
integer y = 2628
end type

type cb_search from w_inherite`cb_search within w_kifa06
boolean visible = false
integer x = 3113
integer y = 2756
integer width = 439
...
type dw_datetime from w_inherite`dw_datetime within w_kifa06
boolean visible = false
end type

type sle_msg from w_inherite`sle_msg within w_kifa06
boolean visible = false
integer height = 88
end type

type gb_10 from w_inherite`gb_10 within w_kifa06
boolean visible = false
end type

type gb_button1 from w_inherite`gb_button1 within w_kifa06
boolean visible = false
integer x = 2304
integer y = 2580
integer height = 308
...
type gb_button2 from w_inherite`gb_button2 within w_kifa06
boolean visible = false
integer x = 3067
integer y = 2696
integer width = 1609
...
boolean bringtotop = true
boolean titlebar = true
string title = "계정별로 상계처리할 내역 표시"
string dataobject = "d_kifa056"
boolean controlmenu = true
boolean minbox = true
boolean maxbox = true
boolean hscrollbar = true
