...
type dw_jogun from datawindow within w_sal_06220
end type
type pb_1 from u_pb_cal within w_sal_06220
end type
type pb_2 from u_pb_cal within w_sal_06220
end type
type dw_list from datawindow within w_sal_06220
end type
type rr_1 from roundrectangle within w_sal_06220
end type
...
Choose Case GetColumnName()
	...
	Case 'sudat'
		...
		sRstan = GetItemNumber(nRow, 'expasn_han_rstan')
		...
		SELECT RSTAN 
			...
			IF SQLCA.SQLCODE <> 0 THEN
				SetItem(nRow,"expasn_han_rstan", 0)
				Return 1
			END IF	
			
			SetItem(nRow, "expasn_han_rstan", sRstan)
//			sCurramt = TrunCate(ROUND(dIoQty * dIoprc*sRstan,2),2)
			sCurramt = ROUND(dIoQty * dIoprc*sRstan,5)
//			Round(dNewCiQty,2)
			SetItem(nRow,"expasn_han_curramt",sCurramt)
...
Case 'ioqty'
		...
		IF dIoQty = 0 or isnull(dIoQty) then 
			SetItem(nRow,"ioamt",0)
			SetItem(nRow,"expasn_han_curramt",0)
			return 
		elseif  isnull(dIoprc) or isnull(dIoprc) then 
			SetItem(nRow,"ioamt",0)
			SetItem(nRow,"expasn_han_curramt",0)
...
is_today = f_today()   
		...
		IF SQLCA.SQLCODE <> 0 THEN
			dw_insert.SetItem(row,"expasn_han_rstan", 0)
			Return 1
		END IF	
		dw_insert.SetItem(row, "expasn_han_rstan", sRstan)
