...
type dw_cond from u_key_enter within w_sal_02030
end type
type st_title from statictext within w_sal_02030
end type
type rr_1 from roundrectangle within w_sal_02030
end type
type rr_2 from roundrectangle within w_sal_02030
end type
type pb_1 from u_pb_cal within w_sal_02030
end type
type pb_2 from u_pb_cal within w_sal_02030
end type
...
public function integer wf_requiredchk (integer icurrow);If dw_cond.AcceptText() <> 1 Then Return -1

LsDepotNo  	= Trim(dw_cond.GetItemString(1,"depot_no"))
LsHoldDate 	= Trim(dw_cond.GetItemString(1,"holddate"))
LsRqdat 		= Trim(dw_cond.GetItemString(1,"rqdat"))
...
IF rb_perform.Checked = True THEN
	...
	dw_cond.object.t_8.Visible = True
	
	w_mdi_frame.sle_msg.text = '할당 처리 작업'
	
	dw_cond.SetItem(1,"holddate",is_today)
//	dw_cond.Modify("saupj_t.visible = 1")
//	dw_cond.Modify("saupj.visible = 1")
ELSE
	ST_TITLE.Text = '할당 현황'
	...
	dw_cond.object.t_8.Visible = False
	
	w_mdi_frame.sle_msg.text = '할당 취소 작업'
	dw_cond.SetItem(1,"holddate",is_today)
//	dw_cond.Modify("saupj_t.visible = 0")
//	dw_cond.Modify("saupj.visible = 0") 
END IF
...
this.Control[iCurrent+1]=this.rb_perform
this.Control[iCurrent+2]=this.rb_cancel
...
this.Control[iCurrent+10]=this.dw_head
this.Control[iCurrent+11]=this.dw_cust_lst
this.Control[iCurrent+12]=this.dw_haldang
end on
...
sPlanDate	= Trim(dw_cond.GetItemString(1,'rqdat'))
...
sArea    = dw_cond.GetItemString(1,"sarea")
sCust    = Trim(dw_cond.GetItemString(1,"cust_cd"))
sItnbr    = Trim(dw_cond.GetItemString(1,"itnbr"))
//sOutGu    = Trim(dw_cond.GetItemString(1,"out_gu"))     //050719 OPTION 삭제 HJP
sHoldDate = Trim(dw_cond.GetItemString(1,"holddate"))
sSaupj    = Trim(dw_cond.GetItemString(1,"saupj"))
sSugugb	 = Trim(dw_cond.GetItemString(1,"sugugb"))    //050719 OPTION 삭제 HJP
...
IF rb_perform.Checked = True THEN
   ...
   nCnt = dw_cust_lst.RowCount()
	...
	/* 전표번호 채번 */
	sHoldStock_JpGbn = 'B0'
	sHoldDate = Trim(dw_cond.GetItemString(dw_cond.GetRow(),'holddate'))
   ...
   /* 창고 */
   sDepotNo = Trim(dw_cond.GetItemString(dw_cond.GetRow(),'depot_no'))
...
dw_cond.SetItem(1, 'rqdat', gs_code)
...
//Gs Code에 지정된 날짜 값 지정
dw_cond.SetItem(1, 'holddate', gs_code)
