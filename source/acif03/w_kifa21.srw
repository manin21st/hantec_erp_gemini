$PBExportHeader$w_kifa21.srw
$PBExportComments$자동전표 관리 : 매입(거래명세서기준)
forward
global type w_kifa21 from w_inherite
end type
type gb_1 from groupbox within w_kifa21
end type
type rb_1 from radiobutton within w_kifa21
end type
type rb_2 from radiobutton within w_kifa21
end type
type dw_ip from u_key_enter within w_kifa21
end type
type dw_junpoy from datawindow within w_kifa21
end type
type dw_sungin from datawindow within w_kifa21
end type
type dw_print from datawindow within w_kifa21
end type
type dw_group_detail from datawindow within w_kifa21
end type
type dw_detail from datawindow within w_kifa21
end type
type rr_1 from roundrectangle within w_kifa21
end type
type dw_rtv from datawindow within w_kifa21
end type
type dw_delete from datawindow within w_kifa21
end type
type cbx_all from checkbox within w_kifa21
end type
end forward

global type w_kifa21 from w_inherite
string title = "매입전표 처리(거래명세서 기준)"
gb_1 gb_1
rb_1 rb_1
rb_2 rb_2
dw_ip dw_ip
...
global w_kifa21 w_kifa21

type variables

String sUpmuGbn = 'B',LsAutoSungGbn
end variables
...
dw_detail.DataObject = 'd_kifa214'
...
For k = 1 TO dw_delete.RowCount()
	IF dw_delete.GetItemString(k,"chk") = '1' THEN
		...
		IF SQLCA.SQLCODE <> 0 THEN
			F_MessageChk(12,'[매입자료]'+sqlca.sqlerrtext)
			SetPointer(Arrow!)
			Return -1
		END IF
	END IF
...
public function integer wf_insert_kfz12ot0 (string ssaupj, string sdeptcode, string sbaldate);/************************************************************************************/
/* 매출자료를 자동으로 전표 처리한다.																*/
/* 1. 대변 : 외상매입금 계정과목으로 발생.(환경파일 A-1-28)									*/
/* 2. 차변 : 건별 상세의 품목에 대한 계정과목으로 발생.										*/
/************************************************************************************/
...
SELECT SUBSTR("SYSCNFG"."DATANAME",1,5), SUBSTR("SYSCNFG"."DATANAME",6,2)	/*차변계정(외상매입금)*/
	INTO :sAcc1_Dae,								:sAcc2_Dae
   FROM "SYSCNFG"  
   WHERE ( "SYSCNFG"."SYSGU" = 'A' ) AND ( "SYSCNFG"."SERIAL" = 1 ) AND  
         ( "SYSCNFG"."LINENO" = '28' )   ;
IF SQLCA.SQLCODE <> 0 THEN
	F_MessageChk(56,'[대변계정(A-1-28)]')
	RETURN -1
END IF
...
FOR k = 1 TO dw_rtv.RowCount()
	...
	IF dw_rtv.GetItemString(k,"chk") = '1' THEN	
		...
		sDcGbn = '1'											/*차변 전표*/
		FOR i = 1 TO iAccCnt
			sAcc1_Cha = Left(dw_group_detail.GetItemString(i,"itemacc"),5)
			sAcc2_Cha = Right(dw_group_detail.GetItemString(i,"itemacc"),2)
			...
			dw_junpoy.SetItem(iCurRow,"descr",   '매입 '+dw_group_detail.GetItemString(i,"min_itnbr")+ ' 외')	
			...
			IF sCusGbn = 'Y' THEN
				IF sGbn1 = '8' THEN									/*품번*/
					...
					dw_junpoy.SetItem(iCurRow,"in_nm",   dw_group_detail.GetItemString(i,"itdsc"))	
				ELSE
...
//			dw_junpoy.SetItem(iCurRow,"k_qty",   dw_group_detail.GetItemNumber(i,"ioqty"))
		...
		dw_junpoy.SetItem(iCurRow,"descr",   '외상매입금 ')	
...
type dw_insert from w_inherite`dw_insert within w_kifa21
...
type p_delrow from w_inherite`p_delrow within w_kifa21
boolean visible = false
integer x = 2121
integer y = 3148
integer taborder = 0
...
type p_addrow from w_inherite`p_addrow within w_kifa21
boolean visible = false
integer x = 1947
integer y = 3148
integer taborder = 0
...
type p_search from w_inherite`p_search within w_kifa21
integer x = 3922
integer y = 12
integer taborder = 70
...
IF rb_1.Checked = True THEN
	...
	OpenWithParm(w_kifa21a,'I'+dw_ip.GetItemString(1,"datef")+dw_ip.GetItemString(1,"datet"))
ELSE
	...
	OpenWithParm(w_kifa21a,'D'+dw_delete.GetItemString(iSelectRow,"checkno"))
END IF
end event
...
type p_ins from w_inherite`p_ins within w_kifa21
boolean visible = false
integer x = 1774
integer y = 3148
integer taborder = 0
...
type p_exit from w_inherite`p_exit within w_kifa21
integer y = 12
integer taborder = 60
end type

type p_can from w_inherite`p_can within w_kifa21
boolean visible = false
integer x = 2642
integer y = 3148
integer taborder = 0
...
type p_print from w_inherite`p_print within w_kifa21
boolean visible = false
integer x = 1426
integer y = 3148
integer taborder = 0
...
type p_inq from w_inherite`p_inq within w_kifa21
integer x = 4096
integer y = 12
integer taborder = 20
end type
...
type p_del from w_inherite`p_del within w_kifa21
boolean visible = false
integer x = 2469
integer y = 3148
integer taborder = 0
...
type p_mod from w_inherite`p_mod within w_kifa21
integer x = 4270
integer y = 12
...
type cb_exit from w_inherite`cb_exit within w_kifa21
boolean visible = false
integer x = 4165
integer y = 3180
end type

type cb_mod from w_inherite`cb_mod within w_kifa21
boolean visible = false
integer x = 3813
integer y = 3180
string text = "처리(&P)"
...
type cb_ins from w_inherite`cb_ins within w_kifa21
boolean visible = false
integer x = 2514
integer y = 2588
...
type cb_del from w_inherite`cb_del within w_kifa21
boolean visible = false
integer x = 2514
integer y = 2444
end type

type cb_inq from w_inherite`cb_inq within w_kifa21
boolean visible = false
integer x = 3451
integer y = 3180
end type

type cb_print from w_inherite`cb_print within w_kifa21
boolean visible = false
integer x = 3625
integer y = 2972
end type

type st_1 from w_inherite`st_1 within w_kifa21
boolean visible = false
end type

type cb_can from w_inherite`cb_can within w_kifa21
boolean visible = false
integer x = 3218
integer y = 2444
end type

type cb_search from w_inherite`cb_search within w_kifa21
boolean visible = false
integer x = 2926
integer y = 3184
integer width = 498
...
type dw_datetime from w_inherite`dw_datetime within w_kifa21
boolean visible = false
end type

type sle_msg from w_inherite`sle_msg within w_kifa21
boolean visible = false
end type

type gb_10 from w_inherite`gb_10 within w_kifa21
boolean visible = false
end type

type gb_button1 from w_inherite`gb_button1 within w_kifa21
boolean visible = false
integer x = 2802
integer y = 2384
end type

type gb_button2 from w_inherite`gb_button2 within w_kifa21
boolean visible = false
integer x = 2875
integer y = 3124
integer width = 1659
