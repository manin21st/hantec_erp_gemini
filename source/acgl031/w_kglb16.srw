...
$PBExportComments$받을어음 할인전표 처리-할인적용(일반/우대/전자)
forward
	...
	if sJpyCreateGbn = '1' then		/*개별*/
		...
		FOR i = 1 TO dw_bill.RowCount()	
			IF dw_bill.GetItemString(i, "sun") = '1' THEN
				...
				dw_junpoy.SetItem(iCurRow,"amt",     dAmount)
				dw_junpoy.SetItem(iCurRow,"descr",    '어음 할인이자')		
		...
		dw_junpoy.SetItem(iCurRow,"amt",      dAmount)
		dw_junpoy.SetItem(iCurRow,"descr",    '어음 할인 이자')		
...
w_mdi_frame.sle_msg.text ="받을어음 할인 전표 처리 완료!!"
...
Do
	...
	if sDayGbn <> '1' and sDayGbn <> '7' THEN 			/*요일구분 <> '일요일' and 요일구분 <> '토요일'*/
		iValidChk = 1
	else
...
IF this.GetColumnName() = "hal_bank" THEN
	...
	IF LsCheckLimitGbn = 'Y' THEN								/*금융기관별 할인한도 사용*/	
		...
		dLimitRemain = F_credit_check(scust,LsHalGb,sBalDate)	//금융기관&어음구분
...
IF this.GetColumnName() ="hal_bank" THEN
   ...
   END IF
	...
	IF LsCheckLimitGbn = 'Y' THEN		
		...
		dLimitRemain = F_credit_check(lstr_custom.code,lstr_custom.gubun,sBalDate)	//금융기관&어음구분
...
IF this.GetColumnName() = "halin_amt" THEN
	...
	dBillAmt = this.GetItemNumber(this.GetRow(),"bill_amt")	//어음금액
	dHalNuAmt = this.GetItemNumber(this.GetRow(),"hal_amt")	//할인누계액
	IF IsNull(dHalNuAmt) THEN dHalNuAmt = 0
	
	IF dBillAmt - dHalNuAmt < dHalInAmt THEN
		MessageBox("확인","어음 잔액보다 할인금액이 큽니다.")
		this.SetItem(this.GetRow(),"halin_amt",0)
		Return 1
	END IF
