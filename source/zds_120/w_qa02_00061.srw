$PBExportHeader$w_qa02_00061.srw
$PBExportComments$** 매입클레임 승인
forward
global type w_qa02_00061 from window
end type
type dw_1 from datawindow within w_qa02_00061
end type
type st_1 from statictext within w_qa02_00061
end type
type p_delrow from uo_picture within w_qa02_00061
end type
type p_addrow from uo_picture within w_qa02_00061
end type
type p_search from uo_picture within w_qa02_00061
end type
type p_exit from uo_picture within w_qa02_00061
end type
type p_can from uo_picture within w_qa02_00061
end type
type p_del from uo_picture within w_qa02_00061
end type
type p_mod from uo_picture within w_qa02_00061
end type
type p_inq from uo_picture within w_qa02_00061
end type
type cb_1 from commandbutton within w_qa02_00061
end type
type cb_delete from commandbutton within w_qa02_00061
end type
type cb_cancel from commandbutton within w_qa02_00061
end type
type rb_delete from radiobutton within w_qa02_00061
end type
type rb_insert from radiobutton within w_qa02_00061
end type
type dw_detail from datawindow within w_qa02_00061
end type
type cb_save from commandbutton within w_qa02_00061
end type
type cb_exit from commandbutton within w_qa02_00061
end type
type cb_retrieve from commandbutton within w_qa02_00061
end type
type rr_1 from roundrectangle within w_qa02_00061
end type
type rr_2 from roundrectangle within w_qa02_00061
end type
type rr_3 from roundrectangle within w_qa02_00061
end type
type dw_list from datawindow within w_qa02_00061
end type
type rr_4 from roundrectangle within w_qa02_00061
end type
end forward
...
global w_qa02_00061 w_qa02_00061
...
string		is_sempno			// 검토자 사번
string		is_tempno			// 팀장 사번
end variables
...
// 검토자, 팀장 사번을 환경설정에서..
select dataname into :is_sempno from syscnfg
 where sysgu = 'Y' and serial = 40 and lineno = '2' ;

select dataname into :is_tempno from syscnfg
 where sysgu = 'Y' and serial = 40 and lineno = '3' ;


wf_initial()
end event

on w_qa02_00061.create
...
integer taborder = 70
string title = "none"
string dataobject = "w_qa02_00061_a"
...
if this.getcolumnname() = "chk" then
	...
	if sgubun = 'N' then  // 미승인자료 조회인 경우
		if schk = 'Y' then
			this.setitem(row,'okyn','1')  // 승인
			this.setitem(row,'okdate',f_today())
			this.setitem(row,'sempno',is_sempno)
			this.setitem(row,'tempno',is_tempno)
		else
			this.setitem(row,'okyn','0')  // 미승인
			this.setitem(row,'okdate',snull)
			this.setitem(row,'sempno',snull)
			this.setitem(row,'tempno',snull)
		end if
	else						 // 승인자료 조회인 경우
		if schk = 'Y' then
			this.setitem(row,'okyn','0')  // 미승인
...
ELSE
	...
	dw_list.retrieve(gs_saupj,this.getitemstring(currentrow,'qajpno'))
END IF
end event
...
for lrow = 1 to dw_1.rowcount()
	if dw_1.getitemstring(lrow,'okyn') > '1' then continue	// 승인,미승인이 아닌경우는 skip
	if dw_1.getitemstring(lrow,'chk') = 'Y' then continue
	dw_1.setitem(lrow,'chk','Y')
	if sgubun = 'N' then  // 미승인자료 조회인 경우
		dw_1.setitem(lrow,'okyn','1')  // 승인
		dw_1.setitem(lrow,'okdate',f_today())
	else						 // 승인자료 조회인 경우
		dw_1.setitem(lrow,'okyn','0')  // 미승인
		dw_1.setitem(lrow,'okdate',snull)
	end if
next
dw_1.setredraw(true)
end event
...
if sokyn = 'N' then  // 미승인
	dw_1.dataobject = 'w_qa02_00061_a'
else
	dw_1.dataobject = 'w_qa02_00061_b'
end if
dw_1.settransobject(sqlca)

setpointer(hourglass!)
if dw_1.retrieve(gs_saupj,sdate1,sdeptcode1,sokyn) < 1 then
	f_message_chk(50, '[매입 클레임 승인]')
	dw_detail.setfocus()
	return
end if
...
integer height = 92
integer taborder = 10
string dataobject = "w_qa02_00061_1"
boolean border = false
boolean livescroll = true
end type
...
string dataobject = "w_qa02_00061_c"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
