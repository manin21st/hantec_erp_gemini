...
dw_2.setitem(arg_row, "momast_copy_pdprc", sqlca.fun_erp100000012(f_today(), arg_itnbr, '.'))	/* 생산기본단가 산출 */
...
elseif sittyp = '2' then
	...
	Select Min(cvcod) Into :sIpchango from Vndmst where cvgu = '5' and juprod = '2' and jumaechul = '1' and jumaeip = :spdtgu and ipjogun = :ls_saupj; // 생산창고
end if
...
public function integer wf_convert_qty (string arg_itnbr, decimal arg_qty, string arg_gubun);// arg_gubun = 'M' : m  -> 매
// arg_gubun = 'Q' : 매 -> M
Dec dstdqty, dvalue
String sPdtgu
