$PBExportHeader$w_itemas_waiju_popup.srw
$PBExportComments$** 품목코드 조회 선택(F1 KEY)완료처리된 품목
forward
global type w_itemas_waiju_popup from w_inherite_popup
end type
type rr_2 from roundrectangle within w_itemas_waiju_popup
end type
end forward
...
global w_itemas_waiju_popup w_itemas_waiju_popup

type variables
string is_itcls
end variables

on w_itemas_waiju_popup.create
int iCurrent
call super::create
...
on w_itemas_waiju_popup.destroy
call super::destroy
destroy(this.rr_2)
end on
...
type dw_jogun from w_inherite_popup`dw_jogun within w_itemas_waiju_popup
integer x = 0
integer y = 32
integer width = 2482
integer height = 400
string dataobject = "d_itemas_ittyp_waiju"
end type
...
type p_exit from w_inherite_popup`p_exit within w_itemas_waiju_popup
integer x = 3465
end type
...
type p_inq from w_inherite_popup`p_inq within w_itemas_waiju_popup
integer x = 3118
...
if gs_saupj = '10' or gs_saupj = '%' then
	sold_sql = "SELECT A.ITNBR, A.ITDSC, A.ISPEC, A.JIJIL, A.ITCLS AS ITCLS, B.TITNM, B.PDTGU, A.USEYN, " + &  
							" A.FILSK, FUN_GET_BOMCHK2(A.ITNBR, '1') AS EUSE_YN,	FUN_GET_BOMCHK2(A.ITNBR, '2') AS PUSE_YN, " + &  
							" A.GRITU, A.MDL_JIJIL, NVL(C.CVCOD, '.') AS CVCOD, FUN_GET_CVNAS(C.CVCOD) AS CVNAS, " +&
							" C.BUNBR BUNBR, C.BUDSC , '@@@@@@' AS ITEM_SHTNM, " + &
							...
							" CASE WHEN NVL(D.CNT, 0) > 0 THEN 'Y' ELSE 'N' END AS CNT " + &
				  " FROM ITEMAS A, ITNCT  B, ITMBUY C," + & 
				  " (  SELECT ITNBR, COUNT('X') AS CNT FROM ROUTNG GROUP BY ITNBR ) D" + &
				  " WHERE A.ITTYP = B.ITTYP (+) " + &
					 ...
					 " AND A.GBWAN = 'Y' "         + &
					 " AND A.ITGU = '6' "         + &
					 " AND (( B.PORGU LIKE '" + ls_porgu + "%' ) OR ( B.PORGU = '%' ))"
...
type p_choose from w_inherite_popup`p_choose within w_itemas_waiju_popup
integer x = 3291
end type
...
gs_codename = dw_1.GetItemString(ll_row,"itemas_jijil")
...
type dw_1 from w_inherite_popup`dw_1 within w_itemas_waiju_popup
integer x = 27
integer y = 460
integer width = 3589
integer height = 1336
integer taborder = 100
string dataobject = "d_itemas_popup_2"
...
type sle_2 from w_inherite_popup`sle_2 within w_itemas_waiju_popup
end type

type cb_1 from w_inherite_popup`cb_1 within w_itemas_waiju_popup
end type

type cb_return from w_inherite_popup`cb_return within w_itemas_waiju_popup
end type

type cb_inq from w_inherite_popup`cb_inq within w_itemas_waiju_popup
boolean default = false
end type

type sle_1 from w_inherite_popup`sle_1 within w_itemas_waiju_popup
end type

type st_1 from w_inherite_popup`st_1 within w_itemas_waiju_popup
end type
