$PBExportHeader$w_itemas_popup.srw
$PBExportComments$** 품목코드 조회 선택(F1 KEY)완료처리된 품목
forward
global type w_itemas_popup from w_inherite_popup
end type
type rr_2 from roundrectangle within w_itemas_popup
end type
type rr_1 from roundrectangle within w_itemas_popup
end type
end forward
...
global w_itemas_popup w_itemas_popup
...
dw_jogun.SetItem(1, 'porgu', gs_saupj)
...
type dw_jogun from w_inherite_popup`dw_jogun within w_itemas_popup
integer x = 0
integer y = 32
integer width = 2816
integer height = 400
...
type p_exit from w_inherite_popup`p_exit within w_itemas_popup
integer x = 3365
integer y = 68
end type
...
type p_inq from w_inherite_popup`p_inq within w_itemas_popup
integer x = 3017
integer y = 68
...
SELECT RFGUB
  INTO :ls_porgu
  FROM REFFPF
 WHERE SABU = :gs_sabu AND RFCOD = '03' AND RFNA2 = :ls_porgu ;
...
if gs_saupj = '%' then
	sold_sql = "SELECT A.ITNBR, A.ITDSC, A.ISPEC, A.JIJIL,A.ITCLS AS ITCLS, " + &
				  ...
				  " WHERE A.ITTYP = B.ITTYP (+) " + &
					 ...
					 " AND A.ITNBR = C.ITNBR(+) " + &
					 " AND A.GBWAN = 'Y' "         + &
					 " AND A.PDTGU LIKE '" + ls_porgu + "%'"
/*					 " AND (( B.PORGU LIKE '" + ls_porgu + "%' ) OR ( B.PORGU = 'ALL' ))" 사업장 조회조건 변경(품목마스터의 생산팀으로 구분) - by shingoon 2015.10.26 */
...
type p_choose from w_inherite_popup`p_choose within w_itemas_popup
integer x = 3191
integer y = 68
end type
...
type dw_1 from w_inherite_popup`dw_1 within w_itemas_popup
integer x = 27
integer y = 460
integer width = 3525
integer height = 1320
...
type sle_2 from w_inherite_popup`sle_2 within w_itemas_popup
end type

type cb_1 from w_inherite_popup`cb_1 within w_itemas_popup
end type

type cb_return from w_inherite_popup`cb_return within w_itemas_popup
end type

type cb_inq from w_inherite_popup`cb_inq within w_itemas_popup
boolean default = false
end type

type sle_1 from w_inherite_popup`sle_1 within w_itemas_popup
end type

type st_1 from w_inherite_popup`st_1 within w_itemas_popup
end type
