$PBExportHeader$w_pdm_01490.srw
$PBExportComments$기술bom등록
forward
global type w_pdm_01490 from w_inherite
end type
type tv_1 from treeview within w_pdm_01490
end type
type st_splitbar from u_st_splitbar within w_pdm_01490
end type
type cbx_1 from checkbox within w_pdm_01490
end type
type p_1 from uo_picture within w_pdm_01490
end type
type p_2 from uo_picture within w_pdm_01490
end type
type p_3 from uo_picture within w_pdm_01490
end type
type p_4 from uo_picture within w_pdm_01490
end type
type p_5 from uo_picture within w_pdm_01490
end type
type cbx_2 from checkbox within w_pdm_01490
end type
type p_6 from picture within w_pdm_01490
end type
type rr_2 from roundrectangle within w_pdm_01490
end type
type rr_1 from roundrectangle within w_pdm_01490
end type
type dw_head from u_key_enter within w_pdm_01490
end type
end forward
...
global w_pdm_01490 w_pdm_01490
...
ds_bom.dataobject = "d_pdm_01490_2"
ds_bom.settransobject(sqlca)
If ds_bom.retrieve(sItem) < 1 Then 
	Return
End if
...
public function integer wf_treeview_seek (treeview ltvi, long ahandle, string old_pinbr, string old_usseq, string new_pinbr, string new_cinbr, string arg_cinbr);
 ...
 SELECT "ESTRUC"."PINBR",   
    ...
    INTO :estruc_str.sPinbr,   
         ...
         :estruc_str.gubun2  
    FROM "ESTRUC"  
   WHERE ( "ESTRUC"."PINBR" = :old_pinbr ) AND  
         ( "ESTRUC"."CINBR" = :arg_cinbr ) AND  
         ( "ESTRUC"."USSEQ" = :old_usseq )   ;
 ...
 Delete From ESTRUC where pinbr = :Old_pinbr and cinbr = :arg_cinbr and usseq = :old_usseq;
...
For Lrow = 1 to dw_insert.rowcount()
	 If Len(dw_insert.getitemstring(Lrow, "estruc_usseq")) <> 5 then
		 MessageBox("사용번호", "사용번호는 다섯자리입니다(ex:00030)", stopsign!)
		 dw_insert.setcolumn("estruc_usseq")
	 ...
	 If isnull(dw_insert.getitemstring(Lrow, "estruc_cinbr")) or &
	    Trim(dw_insert.getitemstring(Lrow, "estruc_cinbr"))  = '' then
		 MessageBox("품목번호", "품목번호는 필수입니다.", stopsign!)
		 dw_insert.setcolumn("estruc_cinbr")
		 dw_insert.setrow(Lrow)
		 dw_insert.scrolltorow(Lrow)
		 dw_insert.setfocus()
		 return -1		
	 ...
	 If dw_insert.getitemdecimal(Lrow, "estruc_qtypr")  = 0 then
		 MessageBox("구성수량", "구성수량은 필수입니다.", stopsign!)
		 dw_insert.setcolumn("estruc_qtypr")
		 dw_insert.setrow(Lrow)
		 dw_insert.scrolltorow(Lrow)
		 dw_insert.setfocus()
		 return -1		
	 ...
	 If f_datechk(dw_insert.getitemstring(Lrow, "estruc_efrdt")) = -1  then
		 MessageBox("적용시작일", "일자가 부정확합니다..", stopsign!)
		 dw_insert.setcolumn("estruc_efrdt")
		 dw_insert.setrow(Lrow)
		 dw_insert.scrolltorow(Lrow)
		 dw_insert.setfocus()
		 return -1		
	 ...
	 If f_datechk(dw_insert.getitemstring(Lrow, "estruc_eftdt")) = -1  then
		 IF dw_insert.getitemstring(Lrow, "estruc_eftdt") <> '99999999' THEN		 
			 MessageBox("적용완료일", "일자가 부정확합니다..", stopsign!)
			 dw_insert.setcolumn("estruc_eftdt")
			 dw_insert.setrow(Lrow)
			 dw_insert.scrolltorow(Lrow)
			 dw_insert.setfocus()
			 return -1
	 ...
	 if isnull(dw_insert.getitemstring(Lrow, "estruc_opsno")) or &
	    Trim(dw_insert.getitemstring(Lrow, "estruc_opsno")) = '' then
	 	 dw_insert.setitem(Lrow, "estruc_opsno", '9999')
...
type dw_insert from w_inherite`dw_insert within w_pdm_01490
integer x = 1595
integer y = 280
integer width = 3013
integer height = 1984
string dataobject = "d_pdm_01490_4"
...
Choose Case GetcolumnName() 
	Case "estruc_cinbr"
		...
		SetItem(nRow,"estruc_cinbr",gs_code)
		PostEvent(ItemChanged!)
	 Case "estruc_opsno"
			OpenWithParm(w_routng_popup,is_masterno)
			IF IsNull(Gs_Code) THEN RETURN
			this.SetItem(nrow,"estruc_opsno",Gs_Code)
			this.setcolumn("estruc_opsno")
...
Choose Case GetColumnName() 
	Case	"estruc_cinbr" 
		...
		IF SQLCA.SQLCODE <> 0 THEN
			...
			SetItem(nRow,"estruc_cinbr",   snull)			
		...
		If sitnbr = this.getItemString(nrow, "estruc_pinbr") Then
	...
	Case "itemas_itemas_itdsc"
		...
		If IsNull(sItnbr) Then
			Return 1
		ElseIf sItnbr <> '' Then
			SetItem(nRow,"estruc_cinbr",sItnbr)
			SetColumn("estruc_cinbr")
			SetFocus()
			TriggerEvent(ItemChanged!)
			Return 1
		ELSE
	...
	Case	"estruc_opsno" 
		...
		IF SQLCA.SQLCODE <> 0 THEN
			...
			SetItem(nRow,"estruc_opsno",  '9999')
	...
	Case	"estruc_efrdt" 
		...
		if f_datechk(sopsno) = -1 then
			...
			setitem(nrow, "estruc_efrdt", f_today())
			return 1
		End if
	Case	"estruc_eftdt" 
		sopsno = gettext()
		if f_datechk(sopsno) = -1 then
			MessageBox("확인", "시작일자가 부정확합니다", stopsign!)
			setitem(nrow, "estruc_eftdt", '99991231')
			return 1
		End if
	Case	"estruc_gubun" 
		If Nrow = 1 then
			MessageBox("대체", "첫번째 행은 대체품목으로 지정될 수 없읍니다", stopsign!)
			setitem(nrow, "estruc_gubun", '1')
			return 1
		End if
		sopsno = gettext()
		if sopsno = '2' then
			For Lprv = nrow - 1 to 1 step -1
				if getitemstring(Lprv, "estruc_gubun") = '1' then
					setitem(nrow, "estruc_usseq", getitemstring(Lprv, "estruc_usseq"))
					setitem(nrow, "estruc_dcinbr", getitemstring(Lprv, "estruc_cinbr"))
					Exit
				End if
			Next
		Else	
			setitem(nrow, "estruc_dcinbr", snull)
		End if		
END Choose

end event

type p_delrow from w_inherite`p_delrow within w_pdm_01490
integer x = 3881
integer y = 52
...
sitnbr = dw_insert.getitemstring(Lrow, "estruc_cinbr")
susseq = dw_insert.getitemstring(Lrow, "estruc_usseq")
...
if MessageBox("삭제확인", "품번 : " + sitnbr + '~n' + &
	...
	if sitnbr <> '.' then
		Do while true
			...
			If sitnbr = dw_insert.getitemstring(Lrow, "estruc_dcinbr") And &
				susseq = dw_insert.getitemstring(Lrow, "estruc_usseq")  then
				dw_insert.deleterow(Lrow)
				Continue
			End if
			Lrow++
...
type p_addrow from w_inherite`p_addrow within w_pdm_01490
integer x = 3707
integer y = 52
boolean enabled = false
end type

event p_addrow::clicked;call super::clicked;long lRow, lIn, lusseq
String sUsseq
...
If dw_insert.RowCount() <= 0 Then
	sUsseq = '00000'
Else
	sUsseq = dw_insert.GetItemString(dw_insert.Rowcount(), 'estruc_usseq')
End If

lrow = dw_insert.insertrow(Lin+1)

dw_insert.setitem(lRow, "estruc_pinbr", is_masterno)
dw_insert.setitem(lRow, "estruc_efrdt", f_today())

sUsseq = String(lONG(sUsseq) + 10,'00000')
dw_insert.SEtItem(lrow, 'estruc_usseq', susseq)
...
type p_search from w_inherite`p_search within w_pdm_01490
boolean visible = false
integer x = 3369
integer y = 2428
end type

type p_ins from w_inherite`p_ins within w_pdm_01490
boolean visible = false
integer x = 3991
integer y = 2432
end type

type p_exit from w_inherite`p_exit within w_pdm_01490
integer x = 4402
integer y = 52
end type

type p_can from w_inherite`p_can within w_pdm_01490
integer x = 4229
integer y = 52
end type
...
type p_print from w_inherite`p_print within w_pdm_01490
boolean visible = false
integer x = 3584
integer y = 2428
end type

type p_inq from w_inherite`p_inq within w_pdm_01490
boolean visible = false
integer x = 3781
integer y = 2428
end type

type p_del from w_inherite`p_del within w_pdm_01490
integer x = 2807
integer y = 52
end type
...
type p_mod from w_inherite`p_mod within w_pdm_01490
integer x = 4055
integer y = 52
boolean enabled = false
end type
...
for i = 1 to dw_insert.rowcount()
	if dw_insert.GetItemStatus(i, 0, Primary!) = New! or dw_insert.GetItemStatus(i, 0, Primary!) = NewModified! then
		dw_insert.SetItem(i, "estruc_crt_user", gs_userid)
	elseif dw_insert.GetItemStatus(i, 0, Primary!) = DataModified! then
		dw_insert.SetItem(i, "estruc_upd_user", gs_userid)
	end if
next
...
type cb_exit from w_inherite`cb_exit within w_pdm_01490
end type

type cb_mod from w_inherite`cb_mod within w_pdm_01490
end type

type cb_ins from w_inherite`cb_ins within w_pdm_01490
end type

type cb_del from w_inherite`cb_del within w_pdm_01490
end type

type cb_inq from w_inherite`cb_inq within w_pdm_01490
end type

type cb_print from w_inherite`cb_print within w_pdm_01490
end type

type st_1 from w_inherite`st_1 within w_pdm_01490
integer x = 1422
integer y = 2484
integer width = 1582
string text = ""
...
type cb_can from w_inherite`cb_can within w_pdm_01490
integer x = 2117
integer y = 2784
end type

type cb_search from w_inherite`cb_search within w_pdm_01490
end type
...
type gb_button1 from w_inherite`gb_button1 within w_pdm_01490
integer x = 910
integer y = 3244
end type

type gb_button2 from w_inherite`gb_button2 within w_pdm_01490
end type
...
event clicked;if cbx_1.checked  then
   ...
   dw_insert.modify("estruc_bomend.protect=0")
	dw_insert.modify("estruc_pcbloc.protect=0")	
//	dw_insert.modify("estruc_rmks.protect=0")
else
	cbx_1.text = '상세'
	dw_insert.Modify("DataWindow.Header.Height=68")		
	dw_insert.Modify("DataWindow.Detail.Height=72")	
	dw_insert.modify("estruc_bomend.protect=1")
	dw_insert.modify("estruc_pcbloc.protect=1")
//	dw_insert.modify("estruc_rmks.protect=1")	
End if
...
event clicked;call super::clicked;open(w_pdm_01480)
...
event clicked;call super::clicked;open(w_pdm_01470)
...
event clicked;call super::clicked;open(w_pdm_01460)
...
event clicked;call super::clicked;open(w_pdm_01465)
...
event clicked;call super::clicked;open(w_pdm_01475)
...
event clicked;open(w_pdm_01577)
wf_refresh()
end event
...
integer height = 252
integer taborder = 11
string dataobject = "d_pdm_01490_1"
...
Long Lrow
Lrow = 0  
Select count(*) into :Lrow from estruc where pinbr = :sItnbr;
If Lrow < 1 then
	p_mod.enabled    = true
	p_addrow.enabled = true
	p_delrow.enabled = true
