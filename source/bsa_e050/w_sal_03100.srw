$PBExportHeader$w_sal_03100.srw
$PBExportComments$소급적용 등록(매출)
forward
global type w_sal_03100 from window
end type
type pb_3 from u_pb_cal within w_sal_03100
end type
type pb_2 from u_pb_cal within w_sal_03100
end type
type pb_1 from u_pb_cal within w_sal_03100
end type
type p_exit from uo_picture within w_sal_03100
end type
type p_can from uo_picture within w_sal_03100
end type
type p_del from uo_picture within w_sal_03100
end type
type p_mod from uo_picture within w_sal_03100
end type
type p_inq from uo_picture within w_sal_03100
end type
type st_2 from statictext within w_sal_03100
end type
type st_1 from statictext within w_sal_03100
end type
type rb_2 from radiobutton within w_sal_03100
end type
type rb_1 from radiobutton within w_sal_03100
end type
type cb_delete from commandbutton within w_sal_03100
end type
type cb_cancel from commandbutton within w_sal_03100
end type
type rb_delete from radiobutton within w_sal_03100
end type
type rb_insert from radiobutton within w_sal_03100
end type
type dw_imhist from datawindow within w_sal_03100
end type
type dw_detail from datawindow within w_sal_03100
end type
type cb_save from commandbutton within w_sal_03100
end type
type cb_exit from commandbutton within w_sal_03100
end type
type cb_retrieve from commandbutton within w_sal_03100
end type
type gb_1 from groupbox within w_sal_03100
end type
type rr_1 from roundrectangle within w_sal_03100
end type
type rr_3 from roundrectangle within w_sal_03100
end type
type rr_4 from roundrectangle within w_sal_03100
end type
type rr_5 from roundrectangle within w_sal_03100
end type
type dw_list from u_d_popup_sort within w_sal_03100
end type
end forward
...
global w_sal_03100 w_sal_03100
...
String     is_iogbn = 'OY3'		// 매출단가 소급 수불구분
end variables
...
FOR	lRow = 1		TO		dw_list.RowCount()
	...
	IF dQty > 0	and dAmt <> 0 and sopt = 'Y'	THEN
		...
		dw_imhist.SetItem(lRowHist, "jnpcrt",	'047')			// 전표생성구분
		...
		dw_imhist.SetItem(lRowHist, "dyebi3",	dw_list.GetItemDecimal(lRow,"iovat")) 	// 부가세=소급적용금액
		dw_imhist.SetItem(lRowHist, "bigo",		dw_list.GetItemString(lRow, "bigo"))   // 비고=사유
		dw_imhist.SetItem(lRowHist, "lclgbn",	Left(dw_list.GetItemString(lRow, "lclgbn"),1)) // 내수로칼 유무
		dw_imhist.SetItem(lRowHist, "dyebi1",  1)			// 적용환율
		dw_imhist.SetItem(lRowHist, "yebi2",  'WON')		// 통화단위
		dw_imhist.SetItem(lRowHist, "dyebi2",  dw_list.GetItemDecimal(lRow,"ioprc"))			// 외화단가
		dw_imhist.SetItem(lRowHist, "foramt",  dw_list.GetItemDecimal(lRow,"ioamt"))			// 외화금액
		...
		dw_imhist.setitem(lrowhist, "yebi1",   sToday) // 검수일자
...
//   dw_detail.Object.saupj.Background.Color= 12639424
	...
	dw_detail.setitem(1, "iogbn", is_iogbn)	
...
ELSE
	...
	For lrow = 1 to dw_list.rowcount()
		 ...
		 dw_list.setitem(lrow, "dyebi3", dw_list.getitemdecimal(lrow,"iovat"))
...
string  	sJpno,		&
			...
			sEmpno,		&
			sFrom, sTo,	&
			sNull, siogbn, sgubun, smadat, ssaupj, sidate, slclgbn, sSagub
long		lRow
SetNull(sNull)
...
slclgbn = Left(dw_detail.GetItemString(1, "lclgbn"),1)

// 유상사급여부
If Mid(dw_detail.GetItemString(1, "lclgbn"),2,1) = 'S' Then
	sSagub = 'Y'
Else
	sSagub = 'N'
End If
...
IF ic_status = '1'	THEN
	...
	IF isnull(sidate) or sidate = "" 	THEN
		f_message_chk(30, '[소급일자]')
		dw_detail.SetColumn("idate")
		dw_detail.SetFocus()
		RETURN
...
//	select waigu into :sgubun from iomatrix where sabu = :gs_sabu and iogbn = :siogbn;
	...
	IF	dw_list.Retrieve(gs_sabu, sVendor, sFrom, sTo, sidate, ssaupj, slclgbn, sSagub) <	1		THEN
		f_message_chk(50, '')
		dw_detail.setcolumn("jpno")
		dw_detail.setfocus()
		RETURN
...
dw_list.DataObject = 'd_sal_03100_3'
dw_list.SetTransObject(sqlca)
...
dw_list.DataObject = 'd_sal_03100_2'
dw_list.SetTransObject(sqlca)
...
dw_list.DataObject = 'd_sal_03100_4'
dw_list.SetTransObject(sqlca)
...
string dataobject = "d_sal_03100_5"
...
integer height = 176
integer taborder = 10
string dataobject = "d_sal_03100_1"
...
integer height = 1884
integer taborder = 30
string dataobject = "d_sal_03100_2"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
...
gs_gubun = dw_detail.GetItemString(1, "lclgbn")
OpenWithParm(w_sal_03100_popup, str_parm)


end event

event editchanged;call super::editchanged;dec{3} dQty
dec{5} dPrice
long	lRow
String slclgbn
...
IF this.getcolumnname() = "ioqty"		or		&
	...
	sLclgbn = Left(this.GetItemString(lRow, 'lclgbn'),1)
	
	this.SetItem(lRow, "ioprc", dPrice)
	this.SetItem(lRow, "ioamt", truncate(dPrice * dQty, 0))
	
	If sLclgbn = 'L' then
		this.SetItem(lRow, "iovat", 0)
	Else
		this.SetItem(lRow, "iovat", truncate(truncate(dPrice * dQty, 0) * 0.1 , 0))
	End If
END IF
end event
