$PBExportHeader$w_sal_06040.srw
$PBExportComments$C/I 등록(내수)
forward
global type w_sal_06040 from w_inherite
end type
type tab_1 from tab within w_sal_06040
end type
...
type dw_key from datawindow within w_sal_06040
end type
type rb_new from radiobutton within w_sal_06040
end type
type rb_upd from radiobutton within w_sal_06040
end type
type rr_1 from roundrectangle within w_sal_06040
end type
end forward

global type w_sal_06040 from w_inherite
string title = "INVOICE 등록(내수)"
tab_1 tab_1
dw_key dw_key
rb_new rb_new
rb_upd rb_upd
...
global w_sal_06040 w_sal_06040
...
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_wrate',wrate)
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_urate',urate)
...
//
///* 원화금액,외화금액 계산 */
//wrate = tab_1.tabpage_1.dw_insert_1.GetItemNumber(1,'domcih_wrate')
//urate = tab_1.tabpage_1.dw_insert_1.GetItemNumber(1,'domcih_urate')
...
/* Buyer의 기본정보를 가져온다 */
  ...
  SELECT "DOMCIH"."SELLER",         "DOMCIH"."CONSIGNEE",   
         "DOMCIH"."VF_FROM",        "DOMCIH"."VF_TO",       
			"DOMCIH"."BUYER",          "DOMCIH"."OTHER_REF",	"DOMCIH"."REMARK", "DOMCIH"."SHIPMARK", "DOMCIH"."CINOTES"
    INTO :sSeller,                     :sConSignee,   
	      :sVf_From,                    :sVf_To,            
		   :sBuyer,                      :sOther_ref,		:sRemark, :sshipmark, :scinotes
    FROM "DOMCIH"  
   WHERE "DOMCIH"."CINO"  = ( SELECT MAX("DOMCIH"."CINO") 
			                       FROM "DOMCIH" 
								       WHERE "DOMCIH"."CVCOD" = :sCvcod AND
											    "DOMCIH"."CIDATE" = ( SELECT MAX("DOMCIH"."CIDATE") 
												                         FROM "DOMCIH"
																				WHERE "DOMCIH"."CVCOD" = :sCvcod )) ;

tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_seller',sSeller)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_consignee',sConsignee)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_vf_from',sVf_From)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_vf_to',sVf_From)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_buyer',sBuyer)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_other_ref',sOther_ref)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_remark',sRemark)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_shipmark',sShipmark)
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_cinotes',scinotes)
...
ds_multi.DataObject = 'd_sal_06040_1'
ds_multi.SetTransObject(sqlca)
ds_multi.Retrieve(gs_sabu, s_cino+'%')
rtn = ds_multi.RowCount()
...
tab_1.tabpage_1.dw_insert_1.SetItem(1,'domcih_ctqty', i_sum_ctqty)
...
type dw_insert from w_inherite`dw_insert within w_sal_06040
...
type p_delrow from w_inherite`p_delrow within w_sal_06040
boolean visible = false
integer x = 3863
integer y = 2444
...
type p_addrow from w_inherite`p_addrow within w_sal_06040
boolean visible = false
integer x = 3689
integer y = 2444
boolean enabled = false
...
type p_search from w_inherite`p_search within w_sal_06040
boolean visible = false
integer x = 3758
integer y = 2444
boolean enabled = false
...
type p_ins from w_inherite`p_ins within w_sal_06040
boolean visible = false
integer x = 3054
integer y = 16
end type
...
type p_exit from w_inherite`p_exit within w_sal_06040
integer y = 16
end type

type p_can from w_inherite`p_can within w_sal_06040
integer y = 16
end type
...
type p_print from w_inherite`p_print within w_sal_06040
boolean visible = false
integer x = 3931
integer y = 2444
boolean enabled = false
...
type p_inq from w_inherite`p_inq within w_sal_06040
integer x = 3575
integer y = 16
end type
...
/* 출고확정 여부 */
sts = Trim(tab_1.tabpage_1.dw_insert_1.GetItemstring(1,'domcih_cists'))
...
/* L/C 잔액 */
sExplcno = Trim(tab_1.tabpage_1.dw_insert_1.GetItemstring(1,'domcih_explcno'))
...
type p_del from w_inherite`p_del within w_sal_06040
integer y = 16
end type
...
Choose Case tab_1.selectedtab
	...
	Case 1
		...
		sCino = Trim(tab_1.tabpage_1.dw_insert_1.GetItemString(nRow,'domcih_cino'))
		If IsNull(sCino) Or sCino = '' Then
			f_message_chk(1400,' C/I NO')
			Return 1
		End If
     ...
     // Packing List 삭제
     delete domcipl  where sabu = :gs_sabu and   cino = :sCino ;
     If sqlca.sqlcode <> 0 Then 
...
type p_mod from w_inherite`p_mod within w_sal_06040
integer y = 16
end type

event p_mod::clicked;call super::clicked;string sCino, sCiDate, sCvcod, sPino, sCurr, sLocalYn, sSaupj, sIpjpno, sShipdate, sPacNo
...
sCurr = Trim(tab_1.tabpage_1.dw_insert_1.GetItemString(1,'domcih_curr'))
If IsNull(sCurr) Or sCurr = '' then
	f_message_chk(40,'[통화]')
	tab_1.tabpage_1.dw_insert_1.SetColumn('domcih_curr')
	Return
End If

sShipDate = Trim(tab_1.tabpage_1.dw_insert_1.GetItemString(1,'domcih_shipdat'))
If IsNull(sShipDate) Or sShipDate = '' then
	f_message_chk(40,'[선적예정일]')
	tab_1.tabpage_1.dw_insert_1.SetColumn('domcih_shipdat')
	Return
End If
...
If rb_new.Checked = True Then
	tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_sabu', gs_sabu)
	tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_cino', sCino)
End If

tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_cidate',  sCiDate)
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_cvcod',   sCvcod)
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_localyn', sLocalYN)
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_saupj',   sSaupj)
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_yebi1',   dw_key.GetItemString(1, 'yebi1'))
...
ciamt = ciamt + chramt
tab_1.tabpage_1.dw_insert_1.SetItem(nRow,'domcih_expamt',ciamt)
...
type cb_exit from w_inherite`cb_exit within w_sal_06040
end type

type cb_mod from w_inherite`cb_mod within w_sal_06040
end type

type cb_ins from w_inherite`cb_ins within w_sal_06040
end type

type cb_del from w_inherite`cb_del within w_sal_06040
end type

type cb_inq from w_inherite`cb_inq within w_sal_06040
end type

type cb_print from w_inherite`cb_print within w_sal_06040
integer x = 1367
integer y = 2440
integer width = 498
boolean enabled = false
...
type st_1 from w_inherite`st_1 within w_sal_06040
end type

type cb_can from w_inherite`cb_can within w_sal_06040
end type

type cb_search from w_inherite`cb_search within w_sal_06040
integer x = 1413
integer y = 2376
integer width = 498
integer taborder = 50
...
type gb_button1 from w_inherite`gb_button1 within w_sal_06040
end type

type gb_button2 from w_inherite`gb_button2 within w_sal_06040
end type
...
integer height = 1704
integer taborder = 10
string dataobject = "d_sal_06040_h"
...
Choose Case GetColumnName()
	Case "domcih_explcno"                              // lc 접수번호 선택 popup 
      ...
      End If
   	...
   	Open(w_explc_popup)
		...
		this.SetItem(row,"domcih_explcno",gs_code)
...
//		sCists = GetItemString(nRow,'domcih_cists')
//		If SaleConfirm = '2' and sCists = '2' Then
//			If IsNull(sDate) or sDate = ''  Then
//				MessageBox('확 인','매출확정된 송장입니다')			
//				Return 2
//			Else
//				SetItem(nRow,'domcih_saledt',sDate) // 매출확정일자 수정
//			End If
//		End If
		
		If f_datechk(sdate) <> 1 Then
			f_message_chk(35,'')
			SetItem(nRow,'domcih_shipdat',sNull)
      	setcolumn('domcih_shipdat')
	      Return 1
      END IF
	...
	Case 'domcih_explcno'
      ...
      If sqlca.sqlcode <> 0 Then
		...
		this.SetItem(nrow,"domcih_curr",s_curr)
...
event ue_pressenter;Choose Case GetColumnName() 
	Case "domcih_seller" ,"domcih_consignee","domcih_shipmark","domcih_cinotes","domcih_buyer","domcih_other_ref","domcih_remark"
		return 0
	Case Else
...
integer height = 1712
integer taborder = 10
string dataobject = "d_sal_06040_d"
...
string dataobject = "d_sal_06040_p"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
end type
...
event itemchanged;double sizex,sizey,sizez,size
...
//			Return 2
	...
	Case 'qtypct'
	   ...
	   return 2
   Case 'cbmsizex','cbmsizey','cbmsizez'
		  Accepttext()
        ...
        sizez = GetItemNumber(nRow,'cbmsizez')
		  ...
		  size = Round((sizex/1000) * (sizey/1000) * (sizez/1000),3)
...
integer taborder = 10
boolean bringtotop = true
string dataobject = "d_sal_06040_key"
boolean border = false
...
String sCvnas, steam, sname1, sarea, sSudat
Long   nRow,   nCnt
...
Choose Case GetColumnName()
	Case 'cino'
		...
		SELECT COUNT(*)
		  INTO :nCnt  
		  FROM "DOMCIH"
		 WHERE ( "DOMCIH"."SABU" = :gs_sabu ) AND  
				 ( "DOMCIH"."CINO" = :sCino ) ;
			
		IF nCnt <=0 THEN
			SELECT A.CVCOD, A.SUDAT, V.CVNAS2, A.SAUPJ, A.YEBI2
			  INTO :scvcod, :ssudat, :scvnas, :sSaupj, :sCurr
			  FROM IMHIST A, VNDMST V
			 WHERE A.SABU = :gs_sabu
			   AND A.IOJPNO LIKE :sCino||'%'
				AND A.CVCOD = V.CVCOD
				AND ROWNUM = 1;
				
			SetItem(1, 'cvcod',   scvcod)
			SetItem(1, 'cidate',  ssudat)
			SetItem(1, 'cvcodnm', scvnas)
			SetItem(1, 'saupj', 	 ssaupj)
			tab_1.tabpage_1.dw_insert_1.SetItem(1, 'domcih_shipdat', ssudat)
			tab_1.tabpage_1.dw_insert_1.SetItem(1, 'domcih_curr', scurr)
			
			wf_copy_buyer(scvcod)
	...
	Case 'cidate'                    // ci 일자
      ...
      END IF
		...
		/* 환율 계산 */
		sCurr = tab_1.tabpage_1.dw_insert_1.GetItemString(1,'domcih_curr')
...
Choose Case GetColumnName() 
	Case "cino" 
		...
		this.SetItem(1,"cino",left(gs_code,12))
