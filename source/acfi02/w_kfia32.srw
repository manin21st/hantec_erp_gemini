$PBExportHeader$w_kfia32.srw
$PBExportComments$받을어음 예정결제 현황 조회 출력
forward
global type w_kfia32 from w_standard_print
end type
type gb_5 from groupbox within w_kfia32
end type
type gb_4 from groupbox within w_kfia32
end type
type rr_1 from roundrectangle within w_kfia32
end type
end forward
...
integer y = 0
integer height = 2444
string title = "받을어음 예정결제현황 조회 출력"
gb_5 gb_5
gb_4 gb_4
rr_1 rr_1
end type
global w_kfia32 w_kfia32
...
sbillnof = Trim(dw_ip.GetItemString(dw_ip.Getrow(),'billnof'))
sbillnot = Trim(dw_ip.GetItemString(dw_ip.Getrow(),'billnot'))
...
IF Isnull(sbillnof) or sbillnof = "" Then sbillnof = '0'
IF Isnull(sbillnot) or sbillnot = "" Then sbillnot = '하'

ll_retrow =dw_print.Retrieve( sabu_f,sabu_t,sdatef,sdatet,sbillnof,sbillnot )
...
dw_ip.SetItem(dw_ip.GetRow(),"gubun", '2')
...
type p_preview from w_standard_print`p_preview within w_kfia32
integer x = 4059
integer taborder = 60
end type

type p_exit from w_standard_print`p_exit within w_kfia32
integer x = 4407
integer taborder = 80
end type

type p_print from w_standard_print`p_print within w_kfia32
integer x = 4233
end type

type p_retrieve from w_standard_print`p_retrieve within w_kfia32
integer x = 3886
end type

type st_window from w_standard_print`st_window within w_kfia32
boolean visible = false
end type

type sle_msg from w_standard_print`sle_msg within w_kfia32
boolean visible = false
end type

type dw_datetime from w_standard_print`dw_datetime within w_kfia32
boolean visible = false
integer width = 754
end type

type st_10 from w_standard_print`st_10 within w_kfia32
boolean visible = false
end type

type gb_10 from w_standard_print`gb_10 within w_kfia32
boolean visible = false
end type

type dw_print from w_standard_print`dw_print within w_kfia32
integer x = 4517
integer y = 84
string dataobject = "d_kfia32_1_p"
end type

type dw_ip from w_standard_print`dw_ip within w_kfia32
integer x = 32
integer y = 32
integer width = 3854
integer height = 136
string dataobject = "d_kfia32_0"
end type
...
IF dwo.name = "gubun" THEN
	...
	IF data = '1' THEN
		...
		dw_print.DataObject ="d_kfia32_2_p"
	ELSEIF data = '2' THEN
		dw_list.Title ="받을어음 예정결제현황(상세)"
		dw_list.DataObject ="d_kfia32_1"
		dw_print.DataObject ="d_kfia32_1_p"
	ELSE
...
type dw_list from w_standard_print`dw_list within w_kfia32
integer x = 50
integer y = 192
integer width = 4503
integer height = 2004
string dataobject = "d_kfia32_1"
