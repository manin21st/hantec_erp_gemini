$PBExportHeader$w_mat_w01400.srw
$PBExportComments$세트품 분해처리 - 한텍미적용
forward
global type w_mat_w01400 from window
end type
type rb_1 from radiobutton within w_mat_w01400
end type
type p_3 from uo_picture within w_mat_w01400
end type
type p_exit from uo_picture within w_mat_w01400
end type
type p_cancel from uo_picture within w_mat_w01400
end type
type p_delete from uo_picture within w_mat_w01400
end type
type p_save from uo_picture within w_mat_w01400
end type
type p_inq from uo_picture within w_mat_w01400
end type
type p_delrow from uo_picture within w_mat_w01400
end type
type p_addrow from uo_picture within w_mat_w01400
end type
type rb_delete from radiobutton within w_mat_w01400
end type
type dw_detail from datawindow within w_mat_w01400
end type
type gb_2 from groupbox within w_mat_w01400
end type
type rr_1 from roundrectangle within w_mat_w01400
end type
type dw_list from datawindow within w_mat_w01400
end type
end forward
...
global w_mat_w01400 w_mat_w01400
...
dw_list.SetItem(lrow, "imhist_iogbn",    	dw_detail.GetItemString(1, 'gubun'))		//수불구분 - 분해입고
...
dw_list.SetItem(lrow, "pspec",				'.')			//품명
dw_list.SetItem(lrow, "opseq",				'9999')		//공정순서
dw_list.SetItem(lrow, "depot_no",			sin_house) 	//기준창고=입고처
...
DO WHILE sqlca.sqlcode = 0
	...
	dw_list.SetItem(lrow, "imhist_iojpno",		sIojpno)		//입고번호
	...
	select fun_get_cvnas(:sin_house) into :ls_cvnas from dual;//거래처 
	dw_list.SetItem(lrow, "cvnas",				ls_cvnas) 	//거래처창고
	dw_list.SetItem(lrow, "ioqty",				ld_reqty * ld_qtypr) //수불수량=입고수량 
	dw_list.SetItem(lrow, "ioprc",	   		0)				//공정단가
	dw_list.SetItem(lrow, "ioamt", 				0)				//수불금액
	dw_list.SetItem(lrow, "ioreqty",				ld_reqty * ld_qtypr)	//공정수량
	dw_list.SetItem(lrow, "insdat",				sDate)		//검사일자=입고의뢰일자
	dw_list.SetItem(lrow, "qcgub",				'1')			//검사구분-무검사
...
integer height = 268
integer taborder = 10
string dataobject = "d_mat_w01400"
...
IF this.GetColumnName() = 'empno'	THEN
   gs_gubun = '1'
	gs_code  = this.GetItemString(1,'in_house')
...
ELSEIF this.GetColumnName() = 'jpno'	THEN
	...
	If ls_bigo <> '세트품분해출고' and ls_bigo <> '세트품분해입고' then
		MessageBox("세트품 분해처리", "세트품 분해처리관려 자료가 아닙니다.", information!)
		this.setitem(1, "jpno", sNull)
		RETURN 1
	End If
...
ELSEIF this.GetColumnName() = 'itnbr'	THEN
	...
	Else
		If sgubun = '3' Then
			This.SetItem(1, 'gubun', 'O69')
		Else
			This.SetItem(1, 'gubun', 'O97')
		End If
	End If
END IF
...
integer height = 1932
integer taborder = 20
string dataobject = "d_mat_w01403_1"
