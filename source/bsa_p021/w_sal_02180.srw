$PBExportHeader$w_sal_02180.srw
$PBExportComments$수주 취소 집계현황 - 제품별
forward
global type w_sal_02180 from w_standard_print
end type
type pb_1 from u_pb_cal within w_sal_02180
end type
type pb_2 from u_pb_cal within w_sal_02180
end type
type rr_1 from roundrectangle within w_sal_02180
end type
end forward

global type w_sal_02180 from w_standard_print
string title = "수주 취소집계 현황 - 제품별"
boolean maxbox = true
pb_1 pb_1
pb_2 pb_2
rr_1 rr_1
end type
global w_sal_02180 w_sal_02180
...
public function integer wf_retrieve ();String sIttyp, sItcls, sItnbr, tx_name, sOrdCancelCause,ls_steamcd,ls_sarea,ssaupj
...
if isnull(ls_steamcd) then ls_steamcd ='%'
if isnull(ls_sarea)   then ls_sarea ='%'
if ls_emp_id = "" or isnull(ls_emp_id) then ls_emp_id = '%'

//IF dw_list.Retrieve(gs_sabu, sFrom, sTo, sIttyp+'%',sItcls+'%',sItnbr+'%', sOrdCancelCause+'%',ls_steamcd,ls_sarea,ssaupj,ls_emp_id) <=0 THEN
//	f_message_chk(50,'')
//   dw_ip.setcolumn('sdatef')
//	dw_ip.SetFocus()
//	Return -1
...
IF dw_print.Retrieve(gs_sabu, sFrom, sTo, sIttyp+'%',sItcls+'%',sItnbr+'%', sOrdCancelCause+'%',ls_steamcd,ls_sarea,ssaupj,ls_emp_id) <=0 THEN
	f_message_chk(50,'')
	dw_list.Reset()
   dw_ip.setcolumn('sdatef')
	dw_ip.SetFocus()
...
type p_preview from w_standard_print`p_preview within w_sal_02180
end type

type p_exit from w_standard_print`p_exit within w_sal_02180
end type

type p_print from w_standard_print`p_print within w_sal_02180
end type

type p_retrieve from w_standard_print`p_retrieve within w_sal_02180
end type
...
type dw_print from w_standard_print`dw_print within w_sal_02180
string dataobject = "d_sal_02180_p"
end type

type dw_ip from w_standard_print`dw_ip within w_sal_02180
integer x = 46
integer y = 24
integer width = 3543
integer height = 460
string dataobject = "d_sal_02180_1"
end type
...
String  sItemCls, sItemGbn, sItemClsName,sIoCustArea,sDept, sJijil, sIspeccode
Long    nRow
...
type dw_list from w_standard_print`dw_list within w_sal_02180
integer x = 55
integer y = 504
integer width = 4521
integer height = 1816
string dataobject = "d_sal_02180"
