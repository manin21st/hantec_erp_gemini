$PBExportHeader$w_sal_07022.srw
$PBExportComments$연동판매계획 - Level (1-2) - 중분류별
forward
global type w_sal_07022 from w_standard_print
end type
type shl_1 from statichyperlink within w_sal_07022
end type
type shl_2 from statichyperlink within w_sal_07022
end type
type st_1 from statictext within w_sal_07022
end type
type rr_1 from roundrectangle within w_sal_07022
end type
end forward

global type w_sal_07022 from w_standard_print
string title = "연동판매계획 - 중분류별"
string menuname = ""
boolean maxbox = true
shl_1 shl_1
shl_2 shl_2
...
global w_sal_07022 w_sal_07022
...
type p_preview from w_standard_print`p_preview within w_sal_07022
boolean visible = false
integer x = 2843
integer y = 12
end type

type p_exit from w_standard_print`p_exit within w_sal_07022
end type

type p_print from w_standard_print`p_print within w_sal_07022
boolean visible = false
integer x = 3017
integer y = 12
end type

type p_retrieve from w_standard_print`p_retrieve within w_sal_07022
integer x = 4265
end type
...
type st_10 from w_standard_print`st_10 within w_sal_07022
end type



type dw_print from w_standard_print`dw_print within w_sal_07022
integer x = 3255
string dataobject = "d_pdt_09020_1"
end type

type dw_ip from w_standard_print`dw_ip within w_sal_07022
integer x = 14
integer y = 96
integer width = 2318
integer height = 152
string dataobject = "d_sal_07022_a"
end type

type dw_list from w_standard_print`dw_list within w_sal_07022
event ue_mousemove pbm_mousemove
integer x = 46
integer y = 268
integer width = 4549
integer height = 2040
string dataobject = "d_sal_07022"
...
If mid(ls_Object, 1, 9)  = 'plan_amt_' Then
	ll_Row = long(mid(ls_Object, 12, 3))
	If ll_Row < 1 or isnull(ll_Row) Then Return 
	This.SetRow(ll_row)
	This.SetItem(ll_row, 'opt', mid(ls_Object, 10, 2))
Else
...
IF mid(ls_Object, 1, 9)  = 'plan_amt_' THEN 
	...
	gs_code = dw_ip.GetItemString(1, 'large')
	gs_codename = trim(dw_ip.GetItemString(1, 'large_nm'))
	gs_codename2 = this.GetItemString(row, 'itcls') + '~t' +&
						trim(this.GetItemString(row, 'titnm')) + '~t' +&
						mid(ls_Object, 10, 2) + '~t' + this.GetItemString(row, 'ittyp')
	
	// 윈도우가 이미 열려있으면 닫는다.
	lb_isopen = FALSE
	lw_window = parent.GetFirstSheet()
	DO WHILE IsValid(lw_window)
		if ClassName(lw_window) = 'w_sal_07024' then
			lb_isopen = TRUE
			Exit
		else		
	...
	OpenSheet(w_sal_07024, w_mdi_frame, 0, Layered!)
