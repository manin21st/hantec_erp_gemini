$PBExportHeader$w_kglc30a.srw
$PBExportComments$지급결제 전표-어음결제 상세
forward
global type w_kglc30a from window
end type
type p_exit from uo_picture within w_kglc30a
end type
type p_can from uo_picture within w_kglc30a
end type
type p_del from uo_picture within w_kglc30a
end type
type p_add from uo_picture within w_kglc30a
end type
type dw_find from u_key_enter within w_kglc30a
end type
type tab_bill from tab within w_kglc30a
end type
...
type dw_head from datawindow within w_kglc30a
end type
end forward
...
global w_kglc30a w_kglc30a

type variables

DataWindow    Idw_Bill
Integer       iCur_TabPage,LiSeq
String        LsGyelDate
end variables
...
IF sFlag = 'BILL' THEN
	...
	Idw_bill.SetItem(Idw_bill.GetRow(),"bbaldate",LsGyelDate)
...
LsGyelDate = Left(sMsg,8)
LiSeq      = Long(Mid(sMsg,9,4))
dAmount    = Double(Mid(sMsg,13,12))
...
dw_head.SetItem(1,"billamt",dAmount)

dw_find.SetTransObject(Sqlca)
dw_find.Reset()
dw_find.InsertRow(0)

iPCnt = tab_bill.tabpage_paybill.dw_paybil.Retrieve(LsGyelDate,LiSeq)
...
event clicked;call super::clicked;Integer iPayRow
Double  dAmount = 0,dSumAmt = 0

dAmount = dw_head.GetItemNumber(1,"billamt")
if IsNull(dAmount) then dAmount = 0

iPayRow = tab_bill.tabpage_paybill.dw_paybil.RowCount()
IF iPayRow > 0 THEN
	if Wf_RequiredChk(iPayRow) = -1 then Return
	
	dSumAmt = tab_bill.tabpage_paybill.dw_paybil.GetItemNumber(1,"sum_payamt")
ELSE
	dSumAmt = 0
END IF
if dAmount <> dSumAmt then
	F_MessageChk(47,'')
	Return
end if

IF iPayRow <=0 THEN
	CloseWithReturn(w_kglc30a,'2')	
ELSE
	tab_bill.tabpage_paybill.dw_paybil.SaveAs('',Clipboard!,False)
	CloseWithReturn(w_kglc30a,'1')
END IF
...
event clicked;tab_bill.tabpage_paybill.dw_paybil.Retrieve(LsGyelDate,LiSeq)

tab_bill.tabpage_paybill.dw_paybil.SaveAs('',Clipboard!,False)

CloseWithReturn(w_kglc30a,'0')
end event
...
event clicked;call super::clicked;IF ICur_TabPage = 1 THEN
	...
	IF iFunctionValue = 1 THEN
		...
		Idw_Bill.SetItem(iCurRow,'gyel_date',LsGyelDate)
		Idw_Bill.SetColumn("billno")
		Idw_Bill.SetFocus()
	END IF
ELSE
