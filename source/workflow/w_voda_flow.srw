$PBExportHeader$w_voda_flow.srw
$PBExportComments$Work Flow(Child Window)
forward
global type w_voda_flow from window
end type
type st_box_red_1 from statictext within w_voda_flow
end type
type st_box_red_2 from statictext within w_voda_flow
end type
type st_box_white_2 from statictext within w_voda_flow
end type
type st_box_white_1 from statictext within w_voda_flow
end type
type st_box_white_3 from statictext within w_voda_flow
end type
type st_box_white_4 from statictext within w_voda_flow
end type
type st_box_white_8 from statictext within w_voda_flow
end type
type st_box_white_7 from statictext within w_voda_flow
end type
type st_box_white_5 from statictext within w_voda_flow
end type
type st_box_white_6 from statictext within w_voda_flow
end type
type dw_2 from datawindow within w_voda_flow
end type
type dw_1 from datawindow within w_voda_flow
end type
type dw_summary from datawindow within w_voda_flow
end type
end forward
...
global w_voda_flow w_voda_flow
...
u_voda_proj_object	luo_new[]
u_voda_proj_object iuo_selected_box
datastore ids_2[] 

w_voda_flow  iw_flow_activity
w_bubblehelp iw_help

INT ii_USER_BOX_COUNT = 0 
end variables
...
public subroutine wf_retrieve_page ()
public function integer wf_save_page ()
public subroutine wf_set_mode_select_box (u_voda_proj_object auo_box)
...
for i = 1 to ii_box_count
	if IsValid(luo_new[i]) Then
//		luo_new[i].DragAuto = true
		luo_new[i].Pointer = "Arrow!"
//		luo_new[i].dw_info.enabled = true
	end if
next
...
public function integer wf_reset_page ();LONG ll_i

dw_1.visible = false
dw_2.reset()
//dw_2.settransobject(sqlca) 
//ll_return = dw_2.retrieve(gs_code)

//Summary 정보 조회
dw_summary.reset() 
dw_summary.insertrow(0)
//dw_1.settransobject(sqlca)
//dw_summary.settransobject(sqlca) 
//dw_summary.retrieve(gs_code) 

//기존에 조회가 되었으면 초기화 후 다시 조회
if ii_box_count > 0 then 
	For ll_i = 1 to ii_box_count 
		 Closeuserobject(luo_new[ll_i] )
		 destroy  ids_2[ll_i]
	Next
	ii_box_count = 0 
End if 
...
public subroutine wf_retrieve_page ();long 	 ll_return , ll_i, ll_x = 328, ll_y, ll_PROJ_SEQ, ll_GATEWAY_SEQ  , ll_ACTIVITY_SEQ, ll_DISPLAY_SEQ, ll_OBJECT_X,  ll_OBJECT_Y  
String ls_PROJ_CODE, ls_ACTIVITY_NAME, ls_DEPTCODE, ls_DEPTNAME, ls_CO_DEPTCODE, ls_CO_DEPTNAME, ls_REMARKS  
String ls_PRODUCT_FILE1, ls_PRODUCT_FILE2, ls_PRODUCT_FILE3, ls_PRODUCT_FILE4, ls_PRODUCT_FILE5  
String ls_ATTACH_FILE1, ls_ATTACH_FILE2, ls_ATTACH_FILE3, ls_ATTACH_FILE4, ls_ATTACH_FILE5  
...
for ll_i = 1 to ll_return 
	 //변수
	 ls_PROJ_CODE  	= dw_2.object.proj_code[ll_i]
	 ll_PROJ_SEQ   	= dw_2.object.PROJ_SEQ[ll_i] 
	 ll_GATEWAY_SEQ   = dw_2.object.GATEWAY_SEQ[ll_i]
	 ll_ACTIVITY_SEQ  = dw_2.object.ACTIVITY_SEQ[ll_i]
	 ll_DISPLAY_SEQ   = dw_2.object.DISPLAY_SEQ[ll_i]
	 
	 //Create Datastore
	 ids_2[ll_i] = create datastore 
	 ids_2[ll_i].dataobject = 'd_voda_flow_activity_line'
	 ids_2[ll_i].SetTransobject(sqlca)
	 
	 //OBJECT 호출
	 OpenUserObject(luo_new[ll_i], ll_x,  96	 )
	 
	 luo_new[ll_i].DW_1.INSERTROW(0)
	 luo_new[ll_i].DW_1.settransobject(sqlca)
	 ll_x = ll_x + luo_new[ll_i].width  
	 luo_new[ll_i].dw_1.retrieve(ls_PROJ_CODE, ll_PROJ_SEQ, ll_GATEWAY_SEQ, ll_ACTIVITY_SEQ)
	 /////////////////////////////////////////////////////////////
//	 string ls_str 
//	 ls_str = luo_new[ll_i].dw_1.object.notice[1] 
//	 messagebox('ggg', ls_str)
//	 
	 
	 
	 //Share Data
	 luo_new[ll_i].dw_1.sharedata(ids_2[ll_i])
...
public function integer wf_save_page ();long ll_i
...
public subroutine wf_set_mode_select_box (u_voda_proj_object auo_box);// BOX 선택 상태
wf_set_mode(MODE_SELECT_BOX)

iuo_selected_box = auo_box
...
public function integer wf_save ();long ll_i, ll_i2, ll_return  
string ls_1
...
//ACTIVITY 저장
for ll_i = 1 to dw_2.rowcount()
//	 ls_1 = ids_2[ll_i].object.deptname[1]
	 ll_return = ids_2[ll_i].update()
	 if ll_return = -1 then
//		 messagebox('에러', '에러발생')
...
gs_code = dw_summary.object.proj_code[1]
gs_codename = dw_summary.object.proj_name[1]
return 1
end function

public function integer wf_insert ();wstr_parm lstr_parm 


//해당 프로젝트가 조회가 되지 않았을 때는 리턴
if dw_2.rowcount() < 1 then return 1

lstr_parm.s_parm[1] = dw_2.object.proj_code[1] 

openwithparm(w_voda_activity_popup, lstr_parm )
...
public function integer wf_delete ();wstr_parm lstr_parm 
...
public function integer wf_retrieve ();long 	 ll_return , ll_i, ll_x = 328, ll_y, ll_PROJ_SEQ, ll_GATEWAY_SEQ  , ll_ACTIVITY_SEQ, ll_DISPLAY_SEQ, ll_OBJECT_X,  ll_OBJECT_Y  
String ls_PROJ_CODE, ls_ACTIVITY_NAME, ls_DEPTCODE, ls_DEPTNAME, ls_CO_DEPTCODE, ls_CO_DEPTNAME, ls_REMARKS, ls_chk
String ls_PRODUCT_FILE1, ls_PRODUCT_FILE2, ls_PRODUCT_FILE3, ls_PRODUCT_FILE4, ls_PRODUCT_FILE5  
String ls_ATTACH_FILE1, ls_ATTACH_FILE2, ls_ATTACH_FILE3, ls_ATTACH_FILE4, ls_ATTACH_FILE5, ls_month  
DATETIME lD_S_DATE, lD_E_DATE
...
dw_2.settransobject(sqlca) 
ll_return = dw_2.retrieve(gs_code)
ls_month = mid(f_today(), 1, 4) + '/' + mid(f_today(), 5, 2) + '%'
...
for ll_i = 1 to ll_return 
	 ...
	 //OBJECT 호출
	 OpenUserObject(luo_new[ll_i], ll_x,  120)
	 
	 luo_new[ll_i].DW_1.INSERTROW(0)
	 luo_new[ll_i].DW_1.settransobject(sqlca)
	 ll_x = ll_x + luo_new[ll_i].width  
	 luo_new[ll_i].dw_1.retrieve(ls_PROJ_CODE, ll_PROJ_SEQ, ll_GATEWAY_SEQ, ll_ACTIVITY_SEQ, gs_empno, gs_username, ls_month)
	 
	 //확정여부체크 
	 ls_chk = luo_new[ll_i].dw_1.object.finish_yn[1]
	 if ls_chk = 'Y' then
		luo_new[ll_i].dw_1.Modify("deptname.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("co_deptname.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("co_deptname2.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("co_deptname3.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("s_date.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("e_date.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("product_file1.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("product_file2.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("product_file3.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("product_file4.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("product_file5.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("attach_file1.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("attach_file2.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("attach_file3.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("attach_file4.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("attach_file5.TabSequence='0'")
		luo_new[ll_i].dw_1.Modify("remarks.TabSequence='0'")
...
//iw_help.Move(this.X + 29, this.Y + 250)
//iw_help.Resize(this.Width - 68, this.Height - 350) // - dw_emplist.Height - 40)
...
gnv_help.iw_child = this
gnv_help.iw_parent = iw_parent_voda
...
this.Control[]={this.st_box_red_1,&
this.st_box_red_2,&
this.st_box_white_2,&
this.st_box_white_1,&
this.st_box_white_3,&
...
//destroy im_popup
//IF (IsValid(gnv_Help)) THEN gnv_Help.of_unregister(This)
end event
...
integer taborder = 40
string title = "none"
string dataobject = "d_voda_flow_activity_line"
...
integer taborder = 20
string title = "none"
string dataobject = "d_voda_proj_user_object"
boolean border = false
boolean livescroll = true
end type
...
integer height = 80
integer taborder = 10
string dataobject = "dm_voda_flow_activity_summary"
end type
...
event rbuttondown;IF getcolumnname() = 'itnbr' then 
	open(w_itemas_popup)
...
event itemchanged;string sitem , sname, sspec, sJijil, sispec_code
int    ireturn
