$PBExportHeader$w_wflow_file_copy.srw
$PBExportComments$참조파일 서버로 등록
forward
global type w_wflow_file_copy from window
end type
type cb_2 from commandbutton within w_wflow_file_copy
end type
type sle_src_nm from singlelineedit within w_wflow_file_copy
end type
type st_2 from statictext within w_wflow_file_copy
end type
type st_1 from statictext within w_wflow_file_copy
end type
type cb_1 from commandbutton within w_wflow_file_copy
end type
type rb_3 from radiobutton within w_wflow_file_copy
end type
type st_tar from statictext within w_wflow_file_copy
end type
type st_src from statictext within w_wflow_file_copy
end type
type cb_cancel from commandbutton within w_wflow_file_copy
end type
type cb_ok from commandbutton within w_wflow_file_copy
end type
type sle_tar from singlelineedit within w_wflow_file_copy
end type
type cb_tar from commandbutton within w_wflow_file_copy
end type
type cb_src from commandbutton within w_wflow_file_copy
end type
type sle_src from singlelineedit within w_wflow_file_copy
end type
type rb_2 from radiobutton within w_wflow_file_copy
end type
type rb_1 from radiobutton within w_wflow_file_copy
end type
type browseinfo from structure within w_wflow_file_copy
end type
end forward
...
global w_wflow_file_copy w_wflow_file_copy
...
//
//st_1.text = '파일을 선택해 주십시오!'
sle_src_nm.text = is_filepath

if trim(is_filepath) = '' or isnull(is_filepath) then return 
//파일명
...
event clicked;lONG ll_return , ls_chk, ls_product_file[10] 
wstr_parm ls_str_parm

ll_return = Messagebox('확인','삭제하시겠습니까?', Exclamation!, okcancel!, 2) 

if ll_return = 2 then return 

if is_column = 'PRODUCT_FILE1' then 
	
	UPDATE Flow_ACTIVITY_BLOB 
		SET PRODUCT_FILE1 = '', 
			 MOD_DT 			= SYSDATE  
...
//파일유무체크(파일 등록// 변경시에는 'Y', 파일 삭제시에는 'N')
ls_str_parm.s_parm[1] = ' '
ls_str_parm.s_parm[2] = 'N'
gs_code = 'Y'
...
ls_str_parm.s_parm[1] = ' '
ls_str_parm.s_parm[2] = is_file_chk
setnull(gs_code) 
closewithreturn(parent, ls_str_parm)
...
string 	ls_src_path, ls_tar_path, ls_file_nm
string	ls_src_path_temp
wstr_parm ls_str_parm
...
elseif rb_3.checked then 
	...
	select to_char(sysdate,'YYYYMMDDHH24MISS') into :ls_dtms from dual;
		...
		if ll_count = 0 or isnull(ll_count) then 
			...
			if is_column = 'PRODUCT_FILE1' then 
				...
				INSERT INTO Flow_ACTIVITY_BLOB 
								( PROJ_CODE ,     
								PROJ_SEQ    ,   
								GATEWAY_SEQ ,   
								ACTIVITY_SEQ, 
	...
	IF ls_file = '' or isnull(ls_file) then 	
   	ls_str_parm.s_parm[1] = is_filepath
		ls_str_parm.s_parm[2] = 'Y'
		setnull(gs_code) 
	else 
...
if GetFileOpenName('원본 파일을 선택하세요', ls_path, ls_file) = 1 then
	...
	if trim(is_filepath) = '' or isnull(is_filepath) then  
