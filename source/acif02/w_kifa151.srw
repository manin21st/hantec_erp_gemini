...
$PBExportComments$자동전표 관리 : NEGO(동일어음 건)
forward
...
if sFlag = 'L' then
	IF dTotalMaiChul + dTotalSunSu > dTotalIpGum + dTotalBiYong + dTotalSang + dTotalBill THEN							/*외상매출 > 입금 + 비용 + 상계금액=외환차손*/
		dChaSon = dChaSon + ((dTotalMaiChul + dTotalSunSu) - (dTotalIpGum + dTotalBiYong + dTotalSang + dTotalBill))
	ELSEIF dTotalMaiChul + dTotalSunSu < dTotalIpGum + dTotalBiYong + dTotalSang + dTotalBill THEN							/*외상매출 < 입금 + 비용=외환차익*/
...
//		
//		IF sSonIkGbn = '1' AND dAmount < 0 THEN								/*외환차익*/
//			sDcGbn = '2'
//					
//			sAcc1_SonIk = Left(sAcc_ChaSon,5)
//			sAcc2_SonIk = Mid(sAcc_ChaSon,6,2)
//		ELSEIF sSonIkGbn = '1' AND dAmount > 0 THEN							/*외환차손*/
//			sDcGbn = '1'
//		
//			sAcc1_SonIk = Mid(sAcc_ChaSon,8,5)
//			sAcc2_SonIk = Mid(sAcc_ChaSon,13,2)
//		ELSEIF sSonIkGbn = '2' AND dAmount < 0 THEN							/*외환차손*/
//			sDcGbn = '1'
//		
//			sAcc1_SonIk = Mid(sAcc_ChaSon,8,5)
//			sAcc2_SonIk = Mid(sAcc_ChaSon,13,2)
//		ELSEIF sSonIkGbn = '2' AND dAmount > 0 THEN							/*외환차익*/
//			sDcGbn = '2'
//					
//			sAcc1_SonIk = Left(sAcc_ChaSon,5)
//			sAcc2_SonIk = Mid(sAcc_ChaSon,6,2)
...
//		END IF
		...
		dw_junpoy.SetItem(iCurRow,"amt",     dTotalSonIk)	
		dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 환 차' )	
...
//		dCurMaichul = dw_detail.GetItemNumber(k,'calc_wonamt')
		...
		dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 외화외상매출금 ' + dw_rtv.GetItemString(iRow,"curr") )	
...
IF dSunSuAmt <> 0 THEN
	...
	dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 선수금 발생' + dw_rtv.GetItemString(iRow,"explcno"))	
...
//		dCurMaichul = dw_detail.GetItemNumber(k,'calc_wonamt')
		...
		dw_junpoy.SetItem(iCurRow,"amt",     dCurMaichul)
		dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 외화외상매출금 ' + dw_samebill_ngno.GetItemString(iRow,"curr"))	
		IF sRemark1 = 'Y' AND sDcGbn = sAccDcr THEN				
...
IF dSunSuAmt <> 0 THEN
	...
	dw_junpoy.SetItem(iCurRow,"amt",     dSunSuAmt)
	dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 선수금 발생' + dw_samebill_ngno.GetItemString(iRow,"explcno"))	
...
//		dRate2 = Wf_Get_Rate(sBalDate, dw_rtv.GetItemString(k,"curr"))
		...
		iDetailCnt = dw_cha_banje.Retrieve(sNgNo)						/*외상매입금,미지급금 상계:2002.01.07*/
		IF iDetailCnt > 0 THEN
			FOR i = 1 TO iDetailCnt
				...
				dw_junpoy.SetItem(iCurRow,"amt",     dAmount)	
				dw_junpoy.SetItem(iCurRow,"descr",   dw_cha_banje.GetItemString(i,"saupname") + ' 상계')	
		...
		iDetailCnt = dw_bill.Retrieve(sNgNo)						/*어음 추가 : 2001.12.26*/
		IF iDetailCnt > 0 THEN
...
IF dMaiChul + dSunSuAmt > dIpGum + dBiYong + dSangAmt + dBillAmt THEN							/*외상매출 > 입금 + 비용 + 상계금액=외환차손*/
	dChaSon = dChaSon + ((dMaiChul + dSunSuAmt) - (dIpGum + dBiYong + dSangAmt + dBillAmt))
ELSEIF dMaiChul + dSunSuAmt < dIpGum + dBiYong + dSangAmt + dBillAmt THEN							/*외상매출 < 입금 + 비용=외환차익*/
...
FOR k = 1 TO dw_rtv.RowCount()
	IF dw_rtv.GetItemString(k,"chk") = '1' THEN	
		...
		iNgCnt = dw_samebill_ngno.Retrieve(sSaupj,sBalDate)							/*처리할 nego건수*/
		...
		FOR i = 1 TO iNgCnt
			sNgNo    = dw_samebill_ngno.GetItemString(i,"ngno")								/*nego번호*/
			...
			FOR m = 1 TO iIpGumCnt
				sDepot  = dw_ipgum.GetItemString(m,"dptno")										/*입금계좌코드*/
				IF sDepot = '' OR IsNull(sDepot) THEN Continue
				...
				dw_junpoy.SetItem(iCurRow,"amt",     dAmount)	
				dw_junpoy.SetItem(iCurRow,"descr",   sNgNo + ' 입금')	
			...
			IF iDetailCnt > 0 THEN
				FOR m = 1 TO iDetailCnt
					...
					dw_junpoy.SetItem(iCurRow,"amt",     dAmount)	
					dw_junpoy.SetItem(iCurRow,"descr",   dw_cha_banje.GetItemString(m,"saupname") + ' 상계')	
			...
			END IF
			if i = iNgCnt then
				Wf_Calculation_ChaIk_SameBill('L',i,sNgNo,iWeight,dWuiChaSonAmt,dWuiChaIkamt)			/*외환차손/익*/	
			else
				Wf_Calculation_ChaIk_SameBill('C',i,sNgNo,iWeight,dWuiChaSonAmt,dWuiChaIkamt)			/*외환차손/익*/	
			end if
