...
$PBExportComments$구매 계획대 실적 총괄표
forward
...
IF mid(ls_Object, 1, 6)  = 'cvnas2' THEN 
	...
	ls_Sort = Trim(this.Object.ssort[ll_row]) //거래처 혹은 대분류인지 구분값
	IF ls_Sort = '1' Then
...
IF mid(ls_Object, 1, 6)  = 'cvnas2' THEN 
   ...
   gs_codename = This.GetItemString(row, 'ittyp')
	...
	ls_Sort = Trim(this.Object.ssort[row]) //거래처 인지 대분류인지 구분값
