$PBExportHeader$w_pdt_01016.srw
$PBExportComments$** 주간 생산계획(외주 발주 생성)
forward
global type w_pdt_01016 from window
end type
type p_create from picture within w_pdt_01016
end type
type p_exit from picture within w_pdt_01016
end type
type st_1 from statictext within w_pdt_01016
end type
type dw_update from datawindow within w_pdt_01016
end type
type dw_ip from datawindow within w_pdt_01016
end type
end forward
...
global w_pdt_01016 w_pdt_01016
...
if isnull(get_seq) or get_seq = 0 then 
	messagebox("확 인", "주간 외주 계획 자료가" + &
	                    " 존재하지 않습니다. ~n~n 외주생산 계획 자료를 생성 후 작업하십시요."  )
	p_exit.triggerevent(clicked!)
	return 
end if

dw_ip.setitem(1, 'symd', symd)

dw_ip.setfocus()

end event

on w_pdt_01016.create
...
event clicked;String symd, sempno, steam, sfitnbr, stitnbr, siempno, sdeptno, errchk, sdelgub, sestno, ssaupj
...
symd  = dw_ip.getitemstring(1, "symd")
...
sqlca.erp000000401(gs_sabu, symd, sempno, steam, sfitnbr, stitnbr, sdeptno, siempno, sdelgub, ssaupj, errchk)
