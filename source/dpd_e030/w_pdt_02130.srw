$PBExportHeader$w_pdt_02130.srw
$PBExportComments$작업실적등록(작업장별)
forward
global type w_pdt_02130 from w_inherite
end type
type ds_update from datawindow within w_pdt_02130
end type
type dw_hist from datawindow within w_pdt_02130
end type
type dw_head from datawindow within w_pdt_02130
end type
type p_1 from uo_picture_01 within w_pdt_02130
end type
type p_nttabl from uo_picture_02 within w_pdt_02130
end type
type p_holdstock from uo_picture_03 within w_pdt_02130
end type
type pb_1 from u_pb_cal within w_pdt_02130
end type
type rr_3 from roundrectangle within w_pdt_02130
end type
type rr_33 from roundrectangle within w_pdt_02130
end type
end forward

global type w_pdt_02130 from w_inherite
string title = "작업실적등록(작업장별)"
ds_update ds_update
dw_hist dw_hist
dw_head dw_head
p_1 p_1
...
global w_pdt_02130 w_pdt_02130
...
dw_insert.accepttext()
lrow = dw_insert.rowcount()
scdate = dw_head.getitemstring(1, 'sidat')
...
For lcnt = 1 to lrow
		...
		ds_update.setitem(lirow, "silgbn", '1')
...
End if
dw_insert.setitem(1, "empno", dw_head.getitemstring(1, "empno"))			
dw_insert.setitem(1, "empname", dw_head.getitemstring(1, "empnm"))			
...
dw_head.setitem(1, "itdsc", snull)
dw_head.setitem(1, "ispec", snull)
...
this.Control[iCurrent+5]=this.p_nttabl
this.Control[iCurrent+6]=this.p_holdstock
this.Control[iCurrent+7]=this.pb_1
this.Control[iCurrent+8]=this.rr_3
this.Control[iCurrent+9]=this.rr_33
end on

on w_pdt_02130.destroy
call super::destroy
...
dw_head.insertrow(0)
dw_head.object.sidat[1] = f_today()
...
type dw_insert from w_inherite`dw_insert within w_pdt_02130
integer x = 37
integer y = 1632
integer width = 4567
integer height = 696
...
//	end if
	...
	sEdate = this.getitemstring(Lrow, "edate")
...
event dw_insert::clicked;//sle_msg.text = '작업지시번호 별 할당조회 => Double Click 하세요!'
end event
...
type p_delrow from w_inherite`p_delrow within w_pdt_02130
...
type p_addrow from w_inherite`p_addrow within w_pdt_02130
boolean visible = false
integer x = 3877
integer y = 1304
integer taborder = 0
...
type p_search from w_inherite`p_search within w_pdt_02130
boolean visible = false
integer x = 4411
integer y = 1316
integer taborder = 0
...
type p_ins from w_inherite`p_ins within w_pdt_02130
boolean visible = false
integer x = 4069
integer y = 1300
integer taborder = 0
...
type p_exit from w_inherite`p_exit within w_pdt_02130
end type

type p_can from w_inherite`p_can within w_pdt_02130
...
type p_print from w_inherite`p_print within w_pdt_02130
boolean visible = false
integer x = 4283
integer y = 1308
integer taborder = 0
...
type p_inq from w_inherite`p_inq within w_pdt_02130
integer x = 3749
end type

event p_inq::clicked;call super::clicked;string swkctr, strdate, enddate, sItnbr, sPdtgu, sigbn, swcdsc, get_data, shpsigbn
long   k, lCount
...
if dw_head.getitemstring(1, "jagbn") = 'Y' then		/* 신규인 경우 */
   sigbn = dw_head.object.sigbn[1]
	...
	IF sigbn = '1'  then /* 정상 */
		dw_hist.dataobject = "d_pdt_02130_1_hist"
		dw_hist.settransobject(sqlca)		
	   ...
	   SELECT DATANAME  
		...
		dw_insert.Filter()
	ELSE
		dw_hist.dataobject = "d_pdt_02130_11_hist"
		dw_hist.settransobject(sqlca)				
	...
	sitnbr = trim(dw_head.object.itnbr[1])	
	...
	dw_hist.dataobject = "d_pdt_02130_3_hist"
	dw_hist.settransobject(sqlca)			
	...
	strdate = trim(dw_head.object.sidat[1])
	if strdate = '' or isnull(strdate) then 
		f_message_chk(30,'[완료일자]')
		dw_head.setcolumn('sidat')
		dw_head.SetFocus()
		setpointer(arrow!)
		dw_insert.setredraw(true)
		return	
...
type p_del from w_inherite`p_del within w_pdt_02130
boolean enabled = false
...
type p_mod from w_inherite`p_mod within w_pdt_02130
boolean enabled = false
string picturename = "C:\erpman\image\저장_d.gif"
end type

event p_mod::clicked;call super::clicked;string scolumn, spordno
...
spordno = dw_insert.getitemstring(1, "morout_pordno")
...
Lfind = dw_hist.find("morout_pordno = '"+ spordno +"'", 0, dw_hist.rowcount())
...
type cb_exit from w_inherite`cb_exit within w_pdt_02130
integer x = 2363
integer y = 2708
end type

type cb_mod from w_inherite`cb_mod within w_pdt_02130
integer x = 2510
integer y = 2564
boolean enabled = false
end type

type cb_ins from w_inherite`cb_ins within w_pdt_02130
integer x = 832
integer y = 2576
boolean enabled = false
end type

type cb_del from w_inherite`cb_del within w_pdt_02130
integer x = 2839
integer y = 2576
boolean enabled = false
end type

type cb_inq from w_inherite`cb_inq within w_pdt_02130
integer x = 2176
integer y = 2564
end type

type cb_print from w_inherite`cb_print within w_pdt_02130
integer x = 1253
integer y = 2556
end type

type st_1 from w_inherite`st_1 within w_pdt_02130
end type

type cb_can from w_inherite`cb_can within w_pdt_02130
integer x = 2807
integer y = 2700
boolean enabled = false
end type

type cb_search from w_inherite`cb_search within w_pdt_02130
integer x = 1687
integer y = 2592
boolean enabled = false
end type
...
type gb_button1 from w_inherite`gb_button1 within w_pdt_02130
end type

type gb_button2 from w_inherite`gb_button2 within w_pdt_02130
integer x = 1710
integer width = 1486
end type
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "d_pdt_02130_1_hist"
...
event itemchanged;string   sCode, sName, sName2, sNull, sitnbr, sitdsc, sispec
int      ireturn 
Setnull(sNull)
...
if this.getcolumnname() = 'sidat' then
   sCode = trim(this.gettext())
	...
	IF f_datechk(sCode) = -1	then
		...
		this.setitem(1, "sidat", f_today())
		dw_insert.reset()
		return 1
	END IF
elseif this.getcolumnname() = 'sfrom' then
   sCode = trim(this.gettext())
...
event clicked;call super::clicked;dw_head.SetColumn('sidat')
IF IsNull(gs_code) THEN Return 
dw_head.SetItem(1, 'sidat', gs_code)
