$PBExportHeader$w_pdt_02136.srw
$PBExportComments$작업실적등록-수리재작업
forward
global type w_pdt_02136 from w_inherite
end type
type ds_update from datawindow within w_pdt_02136
end type
type dw_setup from datawindow within w_pdt_02136
end type
type dw_head from datawindow within w_pdt_02136
end type
type dw_hist from datawindow within w_pdt_02136
end type
type p_5 from uo_picture_01 within w_pdt_02136
end type
type p_nttabl from uo_picture_02 within w_pdt_02136
end type
type p_holdstock from uo_picture_03 within w_pdt_02136
end type
type p_4 from uo_picture_04 within w_pdt_02136
end type
type p_1 from uo_picture_01 within w_pdt_02136
end type
type gb_4 from groupbox within w_pdt_02136
end type
type gb_1 from groupbox within w_pdt_02136
end type
type rr_1 from roundrectangle within w_pdt_02136
end type
type rr_2 from roundrectangle within w_pdt_02136
end type
end forward

global type w_pdt_02136 from w_inherite
integer height = 2392
string title = "작업실적등록[재검사]"
ds_update ds_update
dw_setup dw_setup
dw_head dw_head
dw_hist dw_hist
...
global w_pdt_02136 w_pdt_02136
...
For Lcnt = 1 to dw_insert.rowcount()
	...
	sreff = dw_insert.getitemstring(lcnt, "de_opseq")
	if IsNull(sReff) or trim(sReff) = '' then
		sColumn = "de_opseq"
		Messagebox("다음공정", "다음공정이 없읍니다", stopsign!)
		return -1
	End if
...
For lcnt = 1 to lrow
		...
		ds_update.setitem(lirow, "yebiv2",  dw_insert.getitemstring(lcnt, "de_opseq"))
		...
		ds_update.setitem(lirow, "silgbn", '5')		// 재작업 실적보고로 작성
...
//	dw_insert.setitem(Lrow, "waqty", dw_insert.getitemdecimal(Lrow, "janqty") * dsilrate)	
Next
...
p_5.PictureName = "c:\erpman\image\전체삭제_d.gif"
...
string spordno, sopseq, scdate, strdate, enddate, stime, etime, sNull
Long Lrow, Linsert, Lcnt
Decimal  {2} dtotman, dtotset, dtotal, dremman, dremset, desset, desman, desmch, dsetrate, dmanrate, dacqty, dijun_qty, dsil_qty
...
For Lrow = 1 to dw_hist.rowcount()
	...
	if Lrow > dw_hist.rowcount() then exit
	...
	if dw_head.getitemstring(1, "jagbn") = 'Y' then		/* 신규인 경우 */
		...
		dijun_qty  = dw_hist.getitemdecimal(Lrow, "ijun_qty")
		dsil_qty  = dw_hist.getitemdecimal(Lrow, "sil_qty")		
		
		if dw_setup.retrieve(gs_sabu, spordno, sopseq) > 0 then
			dw_setup.setitem(1, "waqty", dacqty)
		   dw_setup.setitem(1, "ijun_qty", dijun_qty)
		   dw_setup.setitem(1, "sil_qty",  dsil_qty)			
...
dw_head.insertrow(0)
dw_head.object.sfrom[1] = f_today()
...
type dw_insert from w_inherite`dw_insert within w_pdt_02136
integer x = 82
integer y = 1464
integer width = 4489
integer height = 784
integer taborder = 50
string dataobject = "d_pdt_02136_3"
...
elseif this.getcolumnname() = "morout_mchcod" then
	...
	return ireturn	
elseif this.getcolumnname() = "de_opseq" then
	sPordno = getitemstring(Lrow, "morout_pordno")
	sreff = this.gettext()	
   Select opdsc into :sReff1
	  From Morout
	 Where sabu = :gs_sabu And pordno = :sPordno And opseq = :sreff;
	If sqlca.sqlcode <> 0 then
		Messagebox("다음 공정", "다음공정이 존재하지 않읍니다", stopsign!)
		setitem(Lrow, "de_opseq", sNull)
		setitem(Lrow, "de_opnam", sNull)		
		return 1
	End if
	setitem(Lrow, "de_opnam", sReff1)
...
type p_delrow from w_inherite`p_delrow within w_pdt_02136
integer y = 5000
end type

type p_addrow from w_inherite`p_addrow within w_pdt_02136
integer y = 5000
end type

type p_search from w_inherite`p_search within w_pdt_02136
integer y = 5000
end type

type p_ins from w_inherite`p_ins within w_pdt_02136
integer y = 5000
end type

type p_exit from w_inherite`p_exit within w_pdt_02136
integer y = 0
end type

type p_can from w_inherite`p_can within w_pdt_02136
integer y = 0
end type
...
type p_print from w_inherite`p_print within w_pdt_02136
integer y = 5000
end type

type p_inq from w_inherite`p_inq within w_pdt_02136
integer x = 3749
integer y = 0
end type
...
if dw_head.getitemstring(1, "jagbn") = 'Y' then		/* 신규인 경우 */
   sigbn = dw_head.object.sigbn[1]
	...
	dw_hist.dataobject = "d_pdt_02136_2"
	dw_hist.settransobject(sqlca)		
	dw_insert.dataobject = "d_pdt_02136_3"
	dw_insert.settransobject(sqlca)
	dw_setup.dataobject = "d_pdt_02136_3"
	dw_setup.settransobject(sqlca)
	...
	dw_hist.dataobject = "d_pdt_02136_4"
	dw_hist.settransobject(sqlca)			
	dw_insert.dataobject = "d_pdt_02136_5"
	dw_insert.settransobject(sqlca)
	dw_setup.dataobject = "d_pdt_02136_5"
	dw_setup.settransobject(sqlca)
...
type p_del from w_inherite`p_del within w_pdt_02136
integer y = 0
end type
...
type p_mod from w_inherite`p_mod within w_pdt_02136
integer y = 0
end type
...
type cb_exit from w_inherite`cb_exit within w_pdt_02136
integer x = 3278
integer y = 5000
integer taborder = 120
end type

type cb_mod from w_inherite`cb_mod within w_pdt_02136
integer x = 2235
integer y = 5000
integer taborder = 90
...
type cb_ins from w_inherite`cb_ins within w_pdt_02136
integer x = 832
integer y = 2576
boolean enabled = false
end type

type cb_del from w_inherite`cb_del within w_pdt_02136
integer x = 2583
integer y = 5000
...
type cb_inq from w_inherite`cb_inq within w_pdt_02136
integer x = 1870
integer y = 5000
integer taborder = 20
end type

type cb_print from w_inherite`cb_print within w_pdt_02136
integer x = 1253
integer y = 2556
end type

type st_1 from w_inherite`st_1 within w_pdt_02136
end type

type cb_can from w_inherite`cb_can within w_pdt_02136
integer x = 2930
integer y = 5000
...
type cb_search from w_inherite`cb_search within w_pdt_02136
integer x = 1687
integer y = 2592
boolean enabled = false
end type
...
type gb_button1 from w_inherite`gb_button1 within w_pdt_02136
end type

type gb_button2 from w_inherite`gb_button2 within w_pdt_02136
end type
...
integer height = 316
integer taborder = 10
string dataobject = "d_pdt_02136_1"
boolean border = false
boolean livescroll = true
end type
...
integer height = 972
integer taborder = 40
string dataobject = "d_pdt_02136_2"
boolean vscrollbar = true
boolean border = false
boolean livescroll = true
end type
...
event ue_lbuttondown;picturename = "C:\ERPMAN\image\전체삭제_dn.gif"
end event

event ue_lbuttonup;picturename = "C:\ERPMAN\image\전체삭제_up.gif"
end event
