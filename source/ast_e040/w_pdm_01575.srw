$PBExportHeader$w_pdm_01575.srw
$PBExportComments$생산bom(대량 공정복사)
forward
global type w_pdm_01575 from window
end type
type cbx_1 from checkbox within w_pdm_01575
end type
type p_save from uo_picture within w_pdm_01575
end type
type dw_1 from datawindow within w_pdm_01575
end type
type dw_list from datawindow within w_pdm_01575
end type
type rr_1 from roundrectangle within w_pdm_01575
end type
type p_exit from uo_picture within w_pdm_01575
end type
end forward
...
global w_pdm_01575 w_pdm_01575
...
If this.Checked = True Then
	...
	For Lrow = 1 to lcount
		 spdtgu = dw_list.getitemstring(Lrow, 'pstruc_opsno')	 
...
FOR	lRow = lRowCount	TO		1		STEP  -1
		if dw_list.getitemstring(lrow, "scheck") = 'Y' then
			dw_list.setitem(lrow, 'pstruc_opsno', sopseq)
//			IF trim(em_date.text) = ''		or  em_date.text = '0000.00.00'		THEN
//			ELSE
// 				dw_list.setitem(lrow, 'pstruc_efrdt', sdate )
//			END IF
		end if
NEXT
...
integer height = 212
integer taborder = 10
string dataobject = "d_pdm_01575_1"
...
string dataobject = "d_pdm_01575_2"
...
for ii = 1 to lcount
    spdtgu = this.getitemstring(ii, 'pstruc_opsno')	 
...
IF sOpt = 'Y' THEN 
	spdtgu = this.getitemstring(row, 'pstruc_opsno')
