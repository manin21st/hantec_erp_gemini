$PBExportHeader$w_pdm_11712w.srw
$PBExportComments$생산bom-정전개/역전개현황
forward
global type w_pdm_11712w from w_standard_print
end type
type rb_single from radiobutton within w_pdm_11712w
end type
type rb_prtgu from radiobutton within w_pdm_11712w
end type
type cbx_date from checkbox within w_pdm_11712w
end type
type rb_opseq from radiobutton within w_pdm_11712w
end type
type rb_usseq from radiobutton within w_pdm_11712w
end type
type cbx_qty from checkbox within w_pdm_11712w
end type
type cbx_1 from checkbox within w_pdm_11712w
end type
type rb_2 from radiobutton within w_pdm_11712w
end type
type rb_1 from radiobutton within w_pdm_11712w
end type
type rb_drilldown from radiobutton within w_pdm_11712w
end type
type rb_drillup from radiobutton within w_pdm_11712w
end type
type cbx_bomend from checkbox within w_pdm_11712w
end type
type pb_1 from u_pb_cal within w_pdm_11712w
end type
type gb_5 from groupbox within w_pdm_11712w
end type
type gb_2 from groupbox within w_pdm_11712w
end type
type gb_3 from groupbox within w_pdm_11712w
end type
type gb_4 from groupbox within w_pdm_11712w
end type
type gb_1 from groupbox within w_pdm_11712w
end type
end forward

global type w_pdm_11712w from w_standard_print
string title = "생산BOM-정전개(외주용)"
rb_single rb_single
rb_prtgu rb_prtgu
cbx_date cbx_date
rb_opseq rb_opseq
...
global w_pdm_11712w w_pdm_11712w
...
if rb_1.checked then
	...
	if rb_drilldown.checked then
		ds_dan.dataobject  = "d_pdm_11712_2"	
	elseif rb_drilldown.checked then
		ds_dan.dataobject  = "d_pdm_11712_2_1"
	end if
...
type p_xls from w_standard_print`p_xls within w_pdm_11712w
end type

type p_sort from w_standard_print`p_sort within w_pdm_11712w
end type

type p_preview from w_standard_print`p_preview within w_pdm_11712w
end type

type p_exit from w_standard_print`p_exit within w_pdm_11712w
end type

type p_print from w_standard_print`p_print within w_pdm_11712w
end type

type p_retrieve from w_standard_print`p_retrieve within w_pdm_11712w
end type
...
type st_10 from w_standard_print`st_10 within w_pdm_11712w
end type



type dw_print from w_standard_print`dw_print within w_pdm_11712w
integer x = 219
integer y = 800
integer width = 4366
integer height = 724
string dataobject = "d_pdm_11712_p_single_1"
boolean hscrollbar = true
boolean vscrollbar = true
end type

type dw_ip from w_standard_print`dw_ip within w_pdm_11712w
integer x = 32
integer y = 188
integer width = 4594
integer height = 148
string dataobject = "d_pdm_11712w_1"
end type

event dw_ip::itemchanged;call super::itemchanged;string scode, sItgu, sIttyp
...
if THIS.GetColumnName() = "fr_itnbr" THEN
	...
	SELECT ITTYP, ITGU INTO :sIttyp, :sItgu FROM ITEMAS WHERE ITNBR = :sCode;
	If IsNull(sItgu) Or Trim(sItgu) = '' Or sItgu <> '6' Then Return
...
event dw_ip::rbuttondown;call super::rbuttondown;string sittyp, sItgu
...
elseif this.GetColumnName() = 'fr_itnbr' then
	gs_gubun = '2'
	open(w_itemas_waiju_popup)
	
	if isnull(gs_code) or gs_code = "" then return
	
	this.SetItem(1,"fr_itnbr",gs_code)
	
	SELECT ITTYP, ITGU INTO :sIttyp, :sItgu FROM ITEMAS WHERE ITNBR = :gs_code;
	If IsNull(sItgu) Or Trim(sItgu) = '' Or sItgu <> '6' Then Return
...
type dw_list from w_standard_print`dw_list within w_pdm_11712w
string dataobject = "d_pdm_11712_2"
end type
...
event clicked;dw_list.dataobject = 'd_pdm_11712_2'
...
event clicked;dw_list.dataobject = 'd_pdm_11712_2_1'
dw_list.settransobject(sqlca)
end event
