$PBExportHeader$w_adt_00170.srw
$PBExportComments$불량 환입 요청
forward
global type w_adt_00170 from window
end type
type p_out from picture within w_adt_00170
end type
type p_exit from uo_picture within w_adt_00170
end type
type p_can from uo_picture within w_adt_00170
end type
type p_mod from uo_picture within w_adt_00170
end type
type p_inq from uo_picture within w_adt_00170
end type
type dw_imhist from datawindow within w_adt_00170
end type
type rb_delete from radiobutton within w_adt_00170
end type
type rb_insert from radiobutton within w_adt_00170
end type
type dw_detail from datawindow within w_adt_00170
end type
type rr_1 from roundrectangle within w_adt_00170
end type
type rr_2 from roundrectangle within w_adt_00170
end type
type dw_list from datawindow within w_adt_00170
end type
end forward
...
global w_adt_00170 w_adt_00170
...
string   siogbn, sIogbn1, sIogbn2, sIogbn3, sIogbn4, sIogbn5, sdepot
...
For Lrow = 1 to dw_list.rowcount()
	 ...
	 dw_imhist.setitem(Linsert, "Depot_no", sdepot)
	 ...
	 dw_imhist.setitem(Linsert, "ioreqty",  dw_list.getitemDecimal(Lrow, "imhist_ioreqty"))
	 dw_imhist.setitem(Linsert, "iosuqty",  dw_list.getitemDecimal(Lrow,'imhist_iosuqty'))
	 ...
	 if dw_list.getitemdecimal(Lrow, "imhist_cnvfat") = 1 then
		 dw_imhist.setitem(Linsert, "cnviore",  dw_list.getitemdecimal(lrow, "imhist_ioreqty"))
	 elseif dw_list.getitemstring(Lrow, "imhist_cnvart") = '*' then
		 dw_imhist.setitem(Linsert, "cnviore",  &
					round(dw_list.getitemdecimal(lrow, "imhist_cnvfat") / &
							dw_list.getitemdecimal(lrow, "imhist_ioreqty"), 3))
	 else
		 dw_imhist.setitem(Linsert, "cnviore",  &
					round(dw_list.getitemdecimal(lrow, "imhist_cnvfat") * &
							dw_list.getitemdecimal(lrow, "imhist_ioreqty"), 3))		
	 ...
	 dw_imhist.setitem(Linsert, "qcgub",	 '1')		// 특채는 필수적으로 무검사를 한다.
	 dw_imhist.setitem(Linsert, "io_confirm", 'N') 	// 불량환입은 필수적으로 수동승인
	 
	 dw_imhist.setitem(Linsert, "filsk",	 'Y' )	// 불량환입은 필수적으로 재고관리대상
	 ...
	 dw_imhist.setitem(Linsert, "jnpcrt",	 '043')
	 ...
	 dw_imhist.setitem(Linsert, "insdat",	 dw_list.getitemstring(Lrow, "imhist_insdat"))
...
public function integer wf_checkrequiredfield ();string	sReqDate,	&
			sEmpno,		&
			sNull
long		lRow
dec{3}	dBadQty, dQty, dreQty, dTukQty
SetNull(sNull)

if dw_list.accepttext() = -1 then return -1

FOR lRow = 1	TO		dw_list.RowCount()
    dreQty  = dw_list.GetItemDecimal(lRow, "reqty")
	 dBadQty = dw_list.GetItemDecimal(lRow, "imhist_iofaqty")
	 dTukQty = dw_list.GetItemDecimal(lRow, "imhist_tukqty")
	 dQty    = dw_list.GetItemDecimal(lRow, "badqty")
	
	 IF dQty = 0 THEN
		 MessageBox("확인", "환입수량은 0 이상이어야합니다.")		 
		 dw_list.setcolumn("badqty")
		 dw_list.setfocus()
		 RETURN -1
	 ELSE	
		 IF (dreQty + dTukQty) = 0 THEN //불량환입 + 특채 수량이 0 일경우 최초입력
			 IF dQty > dBadQty		THEN
					MessageBox("확인", "불량수량보다 클 수 없습니다.")
					dw_list.SetItem(lRow, "badqty", 0)
					dw_list.setcolumn("badqty")
					dw_list.setfocus()
					RETURN -1
			 END IF
		 ELSE
			 IF dQty > (dBadQty - dTukQty - dreQty) THEN
				 MessageBox("확인", "이미 환입수량이 존재합니다. ~n잔량만 요청가능합니다.(잔량 = 불량 - 특채 - 불량환입) ")
				 dw_list.SetItem(lRow, "badqty", 0)
				 dw_list.setcolumn("badqty")
				 dw_list.setfocus()
				 RETURN -1
	 ...
	 dw_list.Setitem(lRow,'imhist_iosuqty',dQty)
	 dw_list.Setitem(lRow,'imhist_insdat',f_today()) //불량환입은 무검사이므로  INSDAT를 입력한다.
NEXT
...
ELSE
	FOR i = 1 TO dw_list.RowCount()
		 dBadQty = dw_list.GetItemDecimal(i, "imhist_old_qty")
		 dQty    = dw_list.GetItemDecimal(i, "imhist_ioreqty") 
		 IF dQty > dBadQty		THEN
			 MessageBox("확인", "처음 의뢰수량보다 클 수 없습니다.")
			 dw_list.SetItem(i, "imhist_ioreqty", dBadQty)
			 RETURN 
		 END IF
	NEXT
...
sCustFrom = TRIM(dw_detail.GetItemString(1, "cust"))
sCustTo   = TRIM(dw_detail.GetItemString(1, "Cust2"))
...
IF ic_status = '1'	THEN	
	IF	dw_list.Retrieve(gs_sabu,ls_porgu,sDateFrom,sDateTo, sCustFrom, sCustTo) <	1		THEN
		f_message_chk(50, '[불량환입 요청의뢰내역]')
		dw_detail.setcolumn("sdate")
		dw_detail.setfocus()
		RETURN
	END IF
ELSE
	IF	dw_list.Retrieve(gs_sabu,ls_porgu,sReqDate, sCustFrom, sCustTo) <	1		THEN
		f_message_chk(50, '[불량환입 요청내역]')
		dw_detail.setcolumn("sdate")
		dw_detail.setfocus()
		RETURN
	END IF
...
dw_list.DataObject = 'd_adt_00170_2'
dw_list.SetTransObject(sqlca)
...
dw_list.DataObject = 'd_adt_00170_1'
dw_list.SetTransObject(sqlca)
...
string dataobject = "d_adt_00170_1"
...
IF this.GetColumnName() = 'badqty'	THEN
	...
	if ic_status = '1' then	// 요청시에는 불량수량을 기준으로 check
		If GetItemString(lRow, 'decisionyn') = 'Y' Then
			...
			dTukQty = this.GetItemDecimal(lRow, "imhist_tukqty")
			dQty    = this.GetItemDecimal(lRow, "badqty")
			
			IF dreQty = 0 THEN
				IF dQty > dBadQty		THEN
					MessageBox("확인", "불량수량보다 클 수 없습니다.")
					this.SetItem(lRow, "badqty", 0)
					RETURN 1
				END IF
			ELSE
				IF dQty > (dBadQty - dTukQty - dreQty) THEN
			...
			this.Setitem(lRow,'imhist_ioreqty',dQty)
		End If
	ELSE		
		dBadQty = this.GetItemDecimal(lRow, "imhist_old_qty")
		dQty    = this.GetItemDecimal(lRow, "imhist_ioreqty") 
		
		IF dQty > dBadQty		THEN
			MessageBox("확인", "처음 의뢰수량보다 클 수 없습니다.")
			this.SetItem(lRow, "imhist_ioreqty", 0)
			RETURN 1
		END IF			
	END IF	
END IF
