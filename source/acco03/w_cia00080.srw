$PBExportHeader$w_cia00080.srw
$PBExportComments$노무비 투입 명세서
forward
global type w_cia00080 from w_standard_print
end type
type rr_1 from roundrectangle within w_cia00080
end type
end forward

global type w_cia00080 from w_standard_print
string title = "노무비 투입 명세서"
rr_1 rr_1
end type
global w_cia00080 w_cia00080
...
sAcc_cd1  = dw_ip.GetITemString(1,"acc1_cd")
sAcc_cd2  = dw_ip.GetITemString(1,"acc2_cd")
sWon_Gubn = dw_ip.GetITemString(1,"won_gubn")  
sGubn     = dw_ip.GetITemString(1,"sgubn")  
//---------------------------------------------------
...
IF sAcc_cd1 = '' OR ISNULL(sAcc_cd1) AND sAcc_cd2 = '' OR ISNULL(sAcc_cd2) THEN
	sAcc_cd = '%'
ELSE	
	sAcc_cd = Trim(sAcc_cd1) + Trim(sAcc_cd2)  
END IF	


IF sGubn = '1' THEN
	dw_list.DataObject = "dw_cia00080_2"   /*계정과목별*/
	dw_print.DataObject = "dw_cia00080_2_p"   /*계정과목별*/
ELSEIF sGubn = '2' THEN	
	dw_list.DataObject = "dw_cia00080_3"   /*원가부문별*/
	dw_print.DataObject = "dw_cia00080_3_p"   /*원가부문별*/
END IF		
dw_list.SetTransObject(sqlca)
dw_print.SetTransObject(sqlca)

IF dw_print.Retrieve(scost_sabu,scost_nm,sFromym,sToym,sAcc_cd,sWon_Gubn)  <= 0 THEN
   f_messagechk(14,"") 
	Return -1 	
END IF	
...
type p_preview from w_standard_print`p_preview within w_cia00080
integer taborder = 30
end type

type p_exit from w_standard_print`p_exit within w_cia00080
integer taborder = 50
end type

type p_print from w_standard_print`p_print within w_cia00080
integer taborder = 40
end type

type p_retrieve from w_standard_print`p_retrieve within w_cia00080
end type
...
type st_10 from w_standard_print`st_10 within w_cia00080
end type



type dw_print from w_standard_print`dw_print within w_cia00080
string dataobject = "dw_cia00080_2_p"
end type

type dw_ip from w_standard_print`dw_ip within w_cia00080
integer x = 69
integer width = 3767
integer height = 204
string dataobject = "dw_cia00080_1"
end type
...
IF this.GetColumnName() = "acc1_cd" THEN
	lstr_account.acc1_cd = this.object.acc1_cd[this.getrow()]
	lstr_account.acc2_cd = this.object.acc2_cd[this.getrow()]
...
IF dwo.name = "sgubn" THEN
	CHOOSE CASE data
		CASE '1'
			dw_list.DataObject ="dw_cia00080_2"
			dw_list.SetTransObject(SQLCA)
			
			dw_print.DataObject ="dw_cia00080_2_p"
			dw_print.SetTransObject(SQLCA)

		CASE '2'
			dw_list.DataObject ="dw_cia00080_3"
			dw_list.SetTransObject(SQLCA)
			
			dw_print.DataObject ="dw_cia00080_3_p"
			dw_print.SetTransObject(SQLCA)	
	END CHOOSE
...
type dw_list from w_standard_print`dw_list within w_cia00080
integer x = 82
integer y = 228
integer width = 4512
integer height = 1980
string dataobject = "dw_cia00080_2"
