$PBExportHeader$w_kfid01.srw
$PBExportComments$받을어음 송부 전표 처리
forward
global type w_kfid01 from w_inherite
end type
type gb_1 from groupbox within w_kfid01
end type
type rb_1 from radiobutton within w_kfid01
end type
type rb_2 from radiobutton within w_kfid01
end type
type dw_junpoy from datawindow within w_kfid01
end type
type dw_sungin from datawindow within w_kfid01
end type
type dw_print from datawindow within w_kfid01
end type
type dw_bill from datawindow within w_kfid01
end type
type st_2 from statictext within w_kfid01
end type
type dw_find from u_key_enter within w_kfid01
end type
type dw_ip from u_key_enter within w_kfid01
end type
type rr_1 from roundrectangle within w_kfid01
end type
type dw_delete from datawindow within w_kfid01
end type
type dw_rtv from u_d_popup_sort within w_kfid01
end type
end forward

global type w_kfid01 from w_inherite
integer width = 4663
string title = "받을어음 송부 전표 처리"
gb_1 gb_1
rb_1 rb_1
rb_2 rb_2
dw_junpoy dw_junpoy
...
forward prototypes
public function integer wf_insert_kfz12otd (integer irow, string ssaupj, string sbaldate, long ljunno, integer llinno, string stosaupj)
public function integer wf_delete_kfz12ot0 ()
public function integer wf_insert_kfz12ot0_1 (string ssaupj, string sempno, string sbaldate, string stosaupj)
public function integer wf_insert_kfz12ot0_2 (string ssaupj, string sempno, string sbaldate, string stosaupj)
end prototypes

public function integer wf_insert_kfz12otd (integer irow, string ssaupj, string sbaldate, long ljunno, integer llinno, string stosaupj);Integer iCurRow
String  sFullJunNo
...
dw_bill.SetItem(iCurRow,"status",				'6')
...
dw_bill.SetItem(iCurRow,"owner_saupj",		sToSaupj)
...
public function integer wf_insert_kfz12ot0_1 (string ssaupj, string sempno, string sbaldate, string stosaupj);/************************************************************************************/
/* 받을어음 중 송부할 자료를 읽어서 송부 처리하고 전표 처리한다.(어음하나=전표하나) */
/* 1. 차변 : 본지점계정으로 발생.(환경파일 A-1-68)												*/
/* 2. 대변 : 대상사업장의 어음 계정으로 발생.													*/
/************************************************************************************/
String   sAcc1_Cha,sAcc2_Cha,sAcc1_Dae,sAcc2_Dae,sDcGbn,sYesanGbn,sDeptCode,&
			sCusGbn,sChaDae,sSangGbn,sAccDate,sAlcGbn,sGbn1,sRemark1,sSendSaupj
Integer  k,lLinNo,iCurRow
Long     lJunNo,lAccJunNo
Double   dAmount

dw_ip.AcceptText()
sSendSaupj = dw_ip.GetItemString(dw_ip.GetRow(),"to_saupj")

select substr(rfna3,1,5), substr(rfna3,6,2)
	into :sAcc1_Cha,		  :sAcc2_Cha
	from reffpf where rfcod = 'AD' and rfgub = :sSendSaupj;
IF SQLCA.SQLCODE <> 0 THEN
	F_MessageChk(56,'[본지점 계정]')
	Return -1
END IF
...
w_mdi_frame.sle_msg.text ="받을어음 송부 전표 처리 중 ..."
...
FOR k = 1 TO dw_rtv.RowCount()	
	IF dw_rtv.GetItemString(k,"chk") = '1' THEN			
		...
		dw_junpoy.SetItem(iCurRow,"descr",   '어음 송부')	
		...
		/*반제처리 여부,차대*/
		SELECT "CUS_GU",	"GBN1",		"DC_GU",  		"YESAN_GU",			"REMARK1"      
			INTO :sCusGbn,	:sGbn1,		:sChaDae,		:sYesanGbn,			:sRemark1		
			FROM "KFZ01OM0"  
		...
		dw_junpoy.SetItem(iCurRow,"amt",     dAmount)	
		dw_junpoy.SetItem(iCurRow,"descr",   '어음 송부['+F_Get_Refferance('AD',sToSaupj)+']')	
		...
		IF Wf_Insert_Kfz12otd(k,sSaupj,sBalDate,lJunNo,lLinNo,sToSaupj) = -1 THEN
...
w_mdi_frame.sle_msg.text ="받을어음 송부 전표 처리 완료!!"

Return 1
end function

public function integer wf_insert_kfz12ot0_2 (string ssaupj, string sempno, string sbaldate, string stosaupj);/************************************************************************************/
/* 받을어음 중 송부할 자료를 읽어서 송부 처리하고 전표 처리한다.(전표하나에 묶어서)	*/
/* 1. 차변 : 본지점계정으로 발생.(환경파일 A-1-68)												*/
/* 2. 대변 : 대상사업장의 어음 계정으로 발생.													*/
/************************************************************************************/
String   sAcc1_Cha,sAcc2_Cha,sAcc1_Dae,sAcc2_Dae,sDcGbn,sDeptCode,sSdept,sYesanGbn,&
			sCusGbn,sChaDae,sSangGbn,sAccDate,sAlcGbn,sGbn1,sRemark1,sSendSaupj
Integer  k,lLinNo,iCurRow
Long     lJunNo,lAccJunNo
Double   dAmount
...
this.Control[iCurrent+7]=this.dw_bill
this.Control[iCurrent+8]=this.st_2
this.Control[iCurrent+9]=this.dw_find
...
type dw_insert from w_inherite`dw_insert within w_kfid01
...
type p_delrow from w_inherite`p_delrow within w_kfid01
boolean visible = false
integer x = 3598
integer y = 2756
integer taborder = 0
...
type p_addrow from w_inherite`p_addrow within w_kfid01
boolean visible = false
integer x = 3415
integer y = 2752
integer taborder = 0
...
type p_search from w_inherite`p_search within w_kfid01
boolean visible = false
integer x = 4329
integer y = 2760
integer taborder = 0
...
type p_ins from w_inherite`p_ins within w_kfid01
boolean visible = false
integer x = 3232
integer y = 2752
integer taborder = 0
...
type p_exit from w_inherite`p_exit within w_kfid01
integer y = 0
integer taborder = 70
end type

type p_can from w_inherite`p_can within w_kfid01
boolean visible = false
integer x = 4142
integer y = 2760
integer taborder = 0
...
type p_print from w_inherite`p_print within w_kfid01
boolean visible = false
integer x = 3771
integer y = 2756
integer taborder = 0
...
type p_inq from w_inherite`p_inq within w_kfid01
integer x = 4091
integer y = 0
end type
...
IF rb_1.Checked =True THEN
	IF dw_rtv.Retrieve(sFromSaupj,'1','8') <= 0 THEN
		F_MessageChk(14,'')
		dw_ip.SetFocus()
	END IF
ELSEIF rb_2.Checked =True THEN
	IF dw_delete.Retrieve(sFromSaupj,'6','6') <= 0 THEN
		F_MessageChk(14,'')
		dw_ip.SetFocus()
	END IF
END IF
...
type p_del from w_inherite`p_del within w_kfid01
boolean visible = false
integer x = 3954
integer y = 2760
integer taborder = 0
...
type p_mod from w_inherite`p_mod within w_kfid01
integer x = 4270
integer y = 0
integer taborder = 60
...
IF rb_1.Checked =True THEN
	String sFromSaupj,sToSaupj,sEmpNo,sBalDate

	dw_ip.AcceptText()
	sFromSaupj = dw_ip.GetItemString(dw_ip.GetRow(),"from_saupj")
	sToSaupj   = dw_ip.GetItemString(dw_ip.GetRow(),"to_saupj")
	sEmpNo     = dw_ip.GetItemString(dw_ip.GetRow(),"empno")
	sBalDate   = Trim(dw_ip.GetItemString(dw_ip.GetRow(),"baldate"))
	
	IF sTosaupj ="" OR IsNull(sTosaupj) THEN
		F_MessageChk(1,'[인수할 사업장]')	
		dw_ip.SetColumn("to_saupj")
		dw_ip.SetFocus()
		Return 
	END IF
	...
	IF LsCreateGbn = '1' THEN											/*어음1개 = 전표번호 1개*/
		IF Wf_Insert_Kfz12ot0_1(sFromSaupj,sEmpNo,sBalDate,sToSaupj) = -1 THEN
			Rollback;
			Return
		END IF
	ELSE																		/*전표에 묶어서*/
		IF Wf_Insert_Kfz12ot0_2(sFromSaupj,sEmpNo,sBalDate,sToSaupj) = -1 THEN
			Rollback;
			Return
		END IF
	END IF
...
type cb_exit from w_inherite`cb_exit within w_kfid01
boolean visible = false
integer x = 2738
integer y = 2784
end type

type cb_mod from w_inherite`cb_mod within w_kfid01
boolean visible = false
integer x = 2377
integer y = 2784
string text = "처리(&P)"
...
type cb_ins from w_inherite`cb_ins within w_kfid01
boolean visible = false
integer x = 2464
integer y = 2588
...
type cb_del from w_inherite`cb_del within w_kfid01
boolean visible = false
integer x = 2871
integer y = 2584
end type

type cb_inq from w_inherite`cb_inq within w_kfid01
boolean visible = false
integer x = 2016
integer y = 2784
end type

type cb_print from w_inherite`cb_print within w_kfid01
boolean visible = false
integer x = 2702
integer y = 2428
end type
...
type st_1 from w_inherite`st_1 within w_kfid01
boolean visible = false
end type

type cb_can from w_inherite`cb_can within w_kfid01
boolean visible = false
integer x = 3218
integer y = 2580
end type

type cb_search from w_inherite`cb_search within w_kfid01
boolean visible = false
integer x = 1947
integer y = 2588
integer width = 498
...
type dw_datetime from w_inherite`dw_datetime within w_kfid01
boolean visible = false
end type

type sle_msg from w_inherite`sle_msg within w_kfid01
boolean visible = false
end type

type gb_10 from w_inherite`gb_10 within w_kfid01
boolean visible = false
end type

type gb_button1 from w_inherite`gb_button1 within w_kfid01
boolean visible = false
integer x = 2811
integer y = 2532
end type

type gb_button2 from w_inherite`gb_button2 within w_kfid01
boolean visible = false
integer x = 1984
integer y = 2732
integer width = 1134
...
event clicked;IF rb_1.Checked =True THEN
	...
	dw_ip.Modify("txt_saupj.visible = 1")
	dw_ip.Modify("to_saupj.visible = 1")
	dw_ip.Modify("t_saupj.visible = 1")
...
event clicked;IF rb_2.Checked =True THEN
	...
	IF LsCreateGbn = '1' THEN								/*어음1=전표1*/
		dw_delete.DataObject = 'd_kfid013'
		st_2.Visible = False
	ELSE
		dw_delete.DataObject = 'd_kfid014'
		st_2.Visible = True
	END IF
	dw_delete.SetTransObject(Sqlca)
	dw_delete.Reset()
	...
	dw_ip.Modify("txt_saupj.visible = 0")
	dw_ip.Modify("to_saupj.visible = 0")
	dw_ip.Modify("t_saupj.visible = 0")
