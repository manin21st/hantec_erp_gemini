$PBExportHeader$w_kfia66.srw
$PBExportComments$차입금 상환계획 등록2(상환횟수)
forward
global type w_kfia66 from w_inherite
end type
type dw_list from datawindow within w_kfia66
end type
type gb_5 from groupbox within w_kfia66
end type
type dw_mst from datawindow within w_kfia66
end type
type p_1 from picture within w_kfia66
end type
type p_2 from picture within w_kfia66
end type
type p_3 from picture within w_kfia66
end type
type p_4 from picture within w_kfia66
end type
type p_calc from uo_picture within w_kfia66
end type
type p_remain from uo_picture within w_kfia66
end type
type p_calc_rs from uo_picture within w_kfia66
end type
type p_calc_plan from uo_picture within w_kfia66
end type
type p_plan_pay from uo_picture within w_kfia66
end type
type cb_1 from commandbutton within w_kfia66
end type
type rr_1 from roundrectangle within w_kfia66
end type
type rr_2 from roundrectangle within w_kfia66
end type
end forward
...
public function integer wf_create_plan ();Integer iPassYear,iPayYear,iTermMnW,iCalcYear,iCalcMonth,iDay,iAddCnt = 0
...
//				end if
				...
				iAddCnt = iAddCnt + 1
				/*2003.12.10수정 : 분할년을 분할 횟수로 변경*/
				dCalcAmt  = Round(dChaIpAmtW / iPayYear ,0)		/*상환금액*/
				
				if iAddCnt = iPayYear then
					dCalcAmt  = dCalcAmt + (dRemainW - dCalcAmt)
				else
					dCalcAmt  = Round(dChaIpAmtW / iPayYear ,0)		/*상환금액*/
				end if
				
				dCalcAmtF = Round(dChaIpAmtF / iPayYear,2)		/*상환금액-외화*/	
				if iAddCnt = iPayYear then
					dCalcAmtF = dCalcAmtF + (dRemainF - dCalcAmtF)
				else
					dCalcAmtF = Round(dChaIpAmtF / iPayYear,2)		/*상환금액-외화*/	
...
Double  dYAmt,dYItAmt,dRate,dWAmt,dWItAmt,dLoExcRate,dWRAmt,dYRamt
String  sCurr,sRsDate,sRsYear,sBaseYear,sCDate,sBefYear

dw_mst.AcceptText()
sCurr      = dw_mst.GetItemString(1,"lo_curr")
dLoExcRate = dw_mst.GetItemNumber(1,"lo_excrat")
sCDate     = dw_mst.GetItemString(1,"lo_afdt")						/*차입일자*/
...
SetPointer(HourGlass!)
sle_msg.text = '외화 환산 중...'

FOR k = 1 TO dw_list.RowCount()
	IF dw_list.GetItemString(k,"rs_gbn") = 'Y' then Continue
	
	sRsDate = dw_list.GetItemString(k,"rs_date")
	sRsYear = Left(sRsDate,4)
	sBefYear = String(Long(Left(sRsDate,4)) - 1,'0000')
	
//	if Left(sCDate,4) = Left(sRsDate,4) then
//		dRate = dLoExcRate
//	elseif Left(sRsDate,4) > sBaseYear then
//		select nvl(y_rate,0)			into :dRate
//			from kfz34ot1
//			where closing_date = (select max(closing_date) from kfz34ot1
//											where closing_date <  :sRsYear||'0101' and y_curr = :sCurr) and
//					y_curr = :sCurr ;	
...
//			from kfz34ot1
//			where closing_date = (select max(closing_date) from kfz34ot1
//											where closing_date >= :sBefYear||'0101' and 
//													closing_date <  :sRsYear||'0101' and y_curr = :sCurr) and
//					y_curr = :sCurr ;	
		/*적용환율은 상환일자 이전 외화평가환율의 최근환율 적용:2004.01.03*/
		select nvl(y_rate,0)			into :dRate
			from kfz34ot1
			where closing_date = (select max(closing_date) from kfz34ot1
											where closing_date <  :sRsDate and y_curr = :sCurr) and
					y_curr = :sCurr ;	
		if sqlca.sqlcode <> 0 OR dRate = 0 then
...
//	ELSE
		...
		dYRamt = dw_list.GetItemNumber(k,"rs_yjamt")
		if IsNull(dYRamt) then dYrAmt = 0
		
		dWRAmt = Round((dYRamt * dRate) / iWeight,0)
		
		dw_list.SetItem(k,"rs_jamt", dWRAmt)
//	END IF
NEXT
SetPointer(Arrow!)
	...
	ELSE		
		...
		IF Left(sCalcFromF,6) = Left(sCurDateF,6) THEN
			...
			IF sBaseCalcDate <= sLoto then
				...
				ELSE
					dCalcIja  = 0
					dCalcIjaF = dRemainF * (iTotalDays / 365) * (dLoRate / 100)				/*이자 금액-외화*/ 
				END IF
...
type dw_insert from w_inherite`dw_insert within w_kfia66
boolean visible = false
integer taborder = 0
end type

type p_delrow from w_inherite`p_delrow within w_kfia66
boolean visible = false
integer x = 4389
integer y = 1172
integer taborder = 0
...
type p_addrow from w_inherite`p_addrow within w_kfia66
boolean visible = false
integer x = 4215
integer y = 1172
integer taborder = 0
...
type p_search from w_inherite`p_search within w_kfia66
boolean visible = false
integer x = 3854
integer y = 1176
integer taborder = 0
...
type p_ins from w_inherite`p_ins within w_kfia66
integer x = 3749
integer y = 20
integer taborder = 40
end type
...
type p_exit from w_inherite`p_exit within w_kfia66
integer taborder = 120
end type

type p_can from w_inherite`p_can within w_kfia66
end type
...
type p_print from w_inherite`p_print within w_kfia66
boolean visible = false
integer x = 4027
integer y = 1176
integer taborder = 0
...
type p_inq from w_inherite`p_inq within w_kfia66
integer x = 3575
integer y = 20
integer taborder = 20
end type
...
if dw_mst.GetItemString(1,"lo_curr") = 'WON' then
	...
	p_calc_rs.PictureName = 'C:\erpman\image\외화평가환산_d.gif'
else
	p_calc_rs.Enabled = True
	p_calc_rs.PictureName = 'C:\erpman\image\외화평가환산_up.gif'
end if
	
end event

type p_del from w_inherite`p_del within w_kfia66
end type
...
type p_mod from w_inherite`p_mod within w_kfia66
end type
...
type cb_exit from w_inherite`cb_exit within w_kfia66
boolean visible = false
integer x = 4315
integer y = 1940
integer width = 288
...
type cb_mod from w_inherite`cb_mod within w_kfia66
boolean visible = false
integer x = 3401
integer y = 1940
integer width = 288
...
type cb_ins from w_inherite`cb_ins within w_kfia66
boolean visible = false
integer x = 3045
integer y = 1932
integer width = 288
...
type cb_del from w_inherite`cb_del within w_kfia66
boolean visible = false
integer x = 3707
integer y = 1940
integer width = 288
...
type cb_inq from w_inherite`cb_inq within w_kfia66
boolean visible = false
integer x = 2743
integer y = 1932
integer width = 288
...
type cb_print from w_inherite`cb_print within w_kfia66
boolean visible = false
integer x = 1541
integer y = 2856
...
type st_1 from w_inherite`st_1 within w_kfia66
boolean visible = false
integer y = 2972
end type

type cb_can from w_inherite`cb_can within w_kfia66
boolean visible = false
integer x = 4009
integer y = 1940
integer width = 288
...
type cb_search from w_inherite`cb_search within w_kfia66
boolean visible = false
integer x = 1893
integer y = 2856
boolean enabled = false
...
type dw_datetime from w_inherite`dw_datetime within w_kfia66
boolean visible = false
integer x = 2848
end type

type sle_msg from w_inherite`sle_msg within w_kfia66
boolean visible = false
integer width = 2464
end type

type gb_10 from w_inherite`gb_10 within w_kfia66
boolean visible = false
integer width = 3602
end type

type gb_button1 from w_inherite`gb_button1 within w_kfia66
boolean visible = false
integer x = 2715
integer y = 1876
integer width = 645
...
type gb_button2 from w_inherite`gb_button2 within w_kfia66
boolean visible = false
integer x = 3369
integer y = 1884
integer width = 1262
...
OpenWithParm(w_kfia50a,dw_mst.GetItemString(1, 'lo_cd'))

sMsgParm = Message.StringParm

IF sMsgParm = '0' THEN Return

sFromYm = Left(sMsgParm,6)
dRate   = Double(Mid(sMsgParm,7,6))
...
FOR k = 1 TO dw_list.RowCount()
	...
	ELSE
		dIjaAmtF = Truncate((dCurAmtF * (iDays / 365) * (dRate / 100)) / 100,2) * 100
...
ELSE
	...
	if dw_mst.GetItemString(1,"lo_curr") = 'WON' then
		...
		update kfm03ot1
			set rs_ycamt = rs_ycamt  + (:dChaIpAmtY - :dPlanAmtY)
//			,
...
event clicked;//Integer k,iWeight
...
//			String sMsgParm
//			
//			OpenWithParm(w_kfia63a,sRsDate+sCurr)
//			
//			sMsgParm = Message.StringParm
//
//			IF sMsgParm = '0' THEN 
