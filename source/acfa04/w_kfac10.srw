$PBExportHeader$w_kfac10.srw
$PBExportComments$감가상각비조정명세서합계표 조회 출력
forward
global type w_kfac10 from w_standard_print
end type
type st_wait from statictext within w_kfac10
end type
type rr_1 from roundrectangle within w_kfac10
end type
end forward
...
integer x = 0
integer y = 0
string title = "감가상각비 조정명세서 합계표"
st_wait st_wait
rr_1 rr_1
end type
global w_kfac10 w_kfac10
...
SELECT "SYSCNFG"."DATANAME"      INTO :sangho  												/*상호*/
	FROM "SYSCNFG"  
   WHERE ( "SYSCNFG"."SYSGU" = 'C' ) AND  ( "SYSCNFG"."SERIAL" = 1 ) AND  ( "SYSCNFG"."LINENO" = '3' )   ;

SELECT "SYSCNFG"."DATANAME"      INTO :saupno  												/*사업자번호*/
	FROM "SYSCNFG"  
   WHERE ( "SYSCNFG"."SYSGU" = 'C' ) AND ( "SYSCNFG"."SERIAL" = 1 ) AND ( "SYSCNFG"."LINENO" = '2' )   ;
			
if ls_saupj <> '%' then	
	SELECT "REFFPF"."RFNA2"      INTO :ls_code
	   FROM "REFFPF"  
   	WHERE ( "REFFPF"."RFCOD" = 'AD' ) AND ( "REFFPF"."RFGUB" = :ls_saupj )   ;
			
	SELECT "VNDMST"."SANO","VNDMST"."CVNAS"      INTO :ls_sano, :ls_name  
	   FROM "VNDMST"  
	...
	if Not IsNull(ls_sano) then
		saupno = mid(ls_sano,1,3) +'-'+ mid(ls_sano,4,2) +'-'+ mid(ls_sano,6,5)
		saupno = ls_sano
	end if
end if
...
dw_ip.SetItem(dw_ip.GetRow(), "TFCOD1","G")
...
type p_xls from w_standard_print`p_xls within w_kfac10
end type

type p_sort from w_standard_print`p_sort within w_kfac10
end type

type p_preview from w_standard_print`p_preview within w_kfac10
boolean visible = false
integer x = 3282
integer y = 36
end type

type p_exit from w_standard_print`p_exit within w_kfac10
integer x = 4434
integer y = 8
end type

type p_print from w_standard_print`p_print within w_kfac10
integer x = 4256
integer y = 8
end type

type p_retrieve from w_standard_print`p_retrieve within w_kfac10
integer x = 4078
integer y = 8
end type
...
type st_10 from w_standard_print`st_10 within w_kfac10
end type



type dw_print from w_standard_print`dw_print within w_kfac10
string dataobject = "dw_kfac10_2"
end type

type dw_ip from w_standard_print`dw_ip within w_kfac10
integer x = 27
integer width = 2354
integer height = 144
string dataobject = "dw_kfac10_1"
end type
...
type dw_list from w_standard_print`dw_list within w_kfac10
integer x = 69
integer y = 248
integer width = 4521
integer height = 1964
