...
type dw_imhist_ins from u_key_enter within w_ssal_02050
end type
...
type rr_1 from roundrectangle within w_ssal_02050
end type
...
dw_imhist_ins.SetItem(1,"ioredept", gs_dept)
dw_imhist_ins.SetItem(1,"deptname", f_get_name('DPTNO',gs_dept))
...
event ue_open;call super::ue_open;f_child_saupj(dw_imhist_ins,'depot_no', gs_saupj)

dw_imhist_ins.SetTransObject(SQLCA)
dw_imhist_ins.Reset()
dw_imhist_ins.InsertRow(0)
...
//			iRtnValue =  sqlca.Fun_Erp100000016(gs_sabu,  dw_imhist_ins.GetItemString(1,"sudat"), dw_imhist_ins.GetItemString(1,"cvcod"), sItnbr, '.', &
...
sSudat = Trim(dw_imhist_ins.GetItemString(1,'sudat'))
...
sEmpno = Trim(dw_imhist_ins.GetItemString(1,'ioreemp'))
...
//ls_bumun = Trim(dw_imhist_ins.GetItemString(1,'yebi3'))
//If IsNull(ls_bumun) or ls_bumun = '' Then
//	f_message_chk(30,'[사업부문]')
//	dw_imhist_ins.SetColumn("yebi3")
//	Return 
//End If
//
...
ls_cvcod = dw_imhist_ins.GetItemString(1, 'cvcod')
SELECT SAREA
  INTO :sarea
  FROM VNDMST
...
p_preview.Picturename = 'C:\erpman\image\반품의뢰서출력_d.gif'
...
p_preview.Picturename = 'C:\erpman\image\반품의뢰서출력_up.gif'
...
Choose Case this.GetColumnName()
	Case "saupj"
		sIoCust = GetText()
		f_child_saupj(dw_imhist_ins,'depot_no', sIoCust)
//		f_child_saupj(dw_imhist_ins,'cust_area', sIoCust)
	/* 반품의뢰번호 */
	Case  "iojpno" 
...
string picturename = "C:\erpman\image\반품의뢰서출력_d.gif"
boolean focusrectangle = false
end type

event ue_lbuttondown;If this.Enabled = True then 
	p_preview.Picturename = 'C:\erpman\image\반품의뢰서출력_dn.gif'
End if
