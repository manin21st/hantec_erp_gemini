$PBExportHeader$w_qct_01020_kum.srw
$PBExportComments$사내 재검사의뢰 등록-KUM
forward
global type w_qct_01020_kum from window
end type
type st_1 from statictext within w_qct_01020_kum
end type
type pb_1 from u_pb_cal within w_qct_01020_kum
end type
type p_exit from uo_picture within w_qct_01020_kum
end type
type p_cancel from uo_picture within w_qct_01020_kum
end type
type p_select from uo_picture within w_qct_01020_kum
end type
type p_delete from uo_picture within w_qct_01020_kum
end type
type p_insert from uo_picture within w_qct_01020_kum
end type
type p_save from uo_picture within w_qct_01020_kum
end type
type p_retrieve from uo_picture within w_qct_01020_kum
end type
type rb_delete from radiobutton within w_qct_01020_kum
end type
type rb_insert from radiobutton within w_qct_01020_kum
end type
type dw_detail from datawindow within w_qct_01020_kum
end type
type gb_2 from groupbox within w_qct_01020_kum
end type
type rr_1 from roundrectangle within w_qct_01020_kum
end type
type dw_list from datawindow within w_qct_01020_kum
end type
end forward
...
global w_qct_01020_kum w_qct_01020_kum
...
string	sJpno,   sDate, 		&
			sQcEmp,	sQcitem, &
			sDept,   sNull, sitgu, sdepot, sitnbr, ssaupj, sCvcod
...
sCvcod = dw_detail.GetItemString(1, "depot")				

lcount = dw_list.RowCount()
lRowHist = 0

// 창고 사업장 - 2006.05.24 - 송병호
select ipjogun into :ssaupj from vndmst where cvcod = :sDepot;
...
//	dw_list.SetItem(lRow, "imhist_cvcod",	   sdept) 			// 거래처=의뢰부서
	...
	dw_list.SetItem(lRow, "imhist_cvcod",	   sCvcod) 			// 거래처=양품입고창고
...
IF ic_status = '1'	then
	...
	dw_detail.settaborder("depot",  30)
...
string	sJpno, sIOgubun,	sDate, sTagbn, sEmpno2, sDept, sDeptname, &
         sHouse, sEmpno, sEmpname, sRcvcod, sSaleyn, snull, sQcgub, sPspec, sCvcod, sProject
long		lRow
...
sEmpname  = dw_detail.GetItemString(1, "name")   	//	의뢰자
sEmpno2 = sEmpno
sDept   = dw_detail.GetItemString(1, "dept")    	//	의뢰부서
sDeptname = dw_detail.GetItemString(1, "deptname") 
sCvcod  = sDept   											// 	입고처-부서출고
...
FOR	lRow = 1		TO		dw_list.RowCount()
		...
		dw_list.SetItem(lRow, "imhist_sudat",	    sdate)			// 수불일자=출고일자
		dw_list.SetItem(lRow, "imhist_depot_no",sHouse) 			// 기준창고=출고창고
		dw_list.SetItem(lRow, "imhist_cvcod",	sDept) 			// 거래처창고=부서
		dw_list.SetItem(lRow, "imhist_opseq",	'9999')			// 공정코드
		dw_list.SetItem(lRow, "imhist_io_confirm",	'Y')			// 수불승인여부
		dw_list.SetItem(lRow, "imhist_io_date",	sDate)			// 수불승인일자=출고일자	
		dw_list.SetItem(lRow, "imhist_io_empno",sEmpno2)			// 수불승인자=담당자	
		dw_list.SetItem(lRow, "imhist_botimh",	'N')				// 동시출고여부
		dw_list.SetItem(lRow, "imhist_itgu",    dw_list.GetItemString(lRow, "itemas_itgu")) 	// 구입형태
		dw_list.SetItem(lRow, "imhist_ioredept",sDept)	   	// 수불의뢰부서=할당.부서
		dw_list.SetItem(lRow, "dptnm",sDeptname)	   	// 수불의뢰부서=할당.부서
		dw_list.SetItem(lRow, "imhist_ioreemp",	sEmpno)			// 수불의뢰담당자=담당자	
		dw_list.SetItem(lRow, "empnm",sEmpname)	   	// 수불의뢰부서=할당.부서
...
long backcolor = 32106727
string text = "※검사결과 양품에 대한 입고창고 지정"
...
/////////////////////////////////////////////////////////////////////////
string	sDate, sHouse, sEmpno, sEmpname, sDept, sDeptname, sFactory
long     il_currow
...
SELECT JUNGMOCK INTO :sFactory FROM VNDMST
 WHERE CVGU = '5' AND CVCOD = :sHouse AND JUMAECHUL = '3' ;
IF SQLCA.SQLCODE <> 0 THEN
	sFactory = '.'
END IF
...
dw_list.SetItem(il_currow,'imhist_ioredept',sDept)
dw_list.SetItem(il_currow,'dptnm',sDeptname)
dw_list.SetItem(il_currow,'imhist_ioreemp',sEmpno)
dw_list.SetItem(il_currow,'empnm',sEmpname)
dw_list.SetItem(il_currow,'imhist_facgbn',sFactory)
...
event itemchanged;string	sDept, sDeptName,		&
			sEmpno, sEmpname,		&
			sJpno, sHouse, sEdate, sCvcod,		&
			sNull
SetNull(sNull)


// 의뢰부서
IF this.GetColumnName() = 'house'		THEN
	sHouse = this.gettext()
	sCvcod = trim(this.getitemstring(1, "depot"))
	if IsNull(sCvcod) or sCvcod = '' then
		this.setitem(1, "depot", sHouse)
	end if
...
IF this.GetColumnName() = 'jpno'		THEN
	...
	SELECT SUDAT, DEPOT_NO, CVCOD, IOREDEPT, FUN_GET_DPTNO(IOREDEPT), IOREEMP, FUN_GET_EMPNO(IOREEMP)
	  INTO :sEdate, :sHouse, :sCvcod, :sDept, :sDeptName, :sEmpno, :sEmpname
	  FROM IMHIST
	 WHERE SABU = :gs_sabu
	   AND IOJPNO LIKE :sJpno||'%'
		AND IOGBN = 'O30' 
		AND ROWNUM = 1 ;
	...
	this.setitem(1, "edate", sEdate)
	this.setitem(1, "house", sHouse)
	this.setitem(1, "depot", sCvcod)
	this.setitem(1, "dept", sDept)
	this.setitem(1, "deptname", sDeptName)
...
integer height = 1912
integer taborder = 20
string dataobject = "d_qct_01021_kum"
