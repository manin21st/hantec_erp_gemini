$PBExportHeader$w_adt_00250.srw
$PBExportComments$재직업 의뢰등록
forward
global type w_adt_00250 from window
end type
type dw_hidden from datawindow within w_adt_00250
end type
type p_print from uo_picture within w_adt_00250
end type
type p_2 from uo_picture within w_adt_00250
end type
type p_1 from uo_picture within w_adt_00250
end type
type p_retrieve from uo_picture within w_adt_00250
end type
type p_save from uo_picture within w_adt_00250
end type
type p_delete from uo_picture within w_adt_00250
end type
type p_cancel from uo_picture within w_adt_00250
end type
type p_exit from uo_picture within w_adt_00250
end type
type p_addrow from uo_picture within w_adt_00250
end type
type p_delrow from uo_picture within w_adt_00250
end type
type dw_print from datawindow within w_adt_00250
end type
type cbx_1 from checkbox within w_adt_00250
end type
type dw_imhist_in from datawindow within w_adt_00250
end type
type rb_delete from radiobutton within w_adt_00250
end type
type rb_insert from radiobutton within w_adt_00250
end type
type dw_imhist from datawindow within w_adt_00250
end type
type dw_list from datawindow within w_adt_00250
end type
type gb_2 from groupbox within w_adt_00250
end type
type rr_1 from roundrectangle within w_adt_00250
end type
type dw_detail from datawindow within w_adt_00250
end type
end forward
...
global w_adt_00250 w_adt_00250
...
public function integer wf_item2 (string sitem, string sspec, long lrow)
end prototypes
...
FOR	lRow = 1		TO		dw_list.RowCount()
	...
	IF abs(dOutQty) > 0		THEN
		...
		dw_imhist.SetItem(lRowHist, "jnpcrt",	'044')			// 전표생성구분
		...
		dw_imhist.SetItem(lRowHist, "field_cd",   dw_list.GetItemString(lRow, "field_cd")) // 생산품번
// 	dw_imhist.SetItem(lRowHist, "outchk",  'Y') 			// 출고의뢰완료
		
		// 창고이동 입고내역 생성
		IF sHouseYn = 'Y' and sHouseGubun = 'Y'	and dOutQty > 0	THEN
...
FOR lrow = 1 TO lRowCount
   ...
   if sDelgub = 'N' then continue
	...
	// 작업지시가 내려간 경우 삭제 불가
	if not isnull(dw_list.GetItemString(lRow, "pordno")) then continue
...
//	
//////////////////////////////////////////////////////////////////
string	sHist_Jpno, sGubun, siodate, sioyn, sioconfirm
...
FOR	lRow = 1		TO	lcount
   ...
   k++
	...
	IF dOutQty <> dTemp_OutQty		THEN
		  ...
		  UPDATE "IMHIST"  
     		  SET "IOQTY"   = DECODE("IO_CONFIRM",'Y', :dOutQty, "IOQTY"), 
         		"IOREQTY" = :dOutQty,   
         		"IOSUQTY" = :dOutQty,   
         		"OUTCHK" = :sGubun,  
					"UPD_USER" = :gs_userid    
...
public function integer wf_item (string sitem, string sspec, long lrow);String  sname, sitgu, snull, sispec, sfilsk, sdepot, sjijil, sSpec_code, slotgub, sGrpno2, sGubun
...
If sGubun = 'OM4' Then // 재작업출고(동일품번 투입/생산)시
	dw_list.setitem(lrow, "field_cd", sItem)
End If
...
FOR	lRow = 1		TO		lCount
   ...
   sLotgub = Trim(dw_list.GetitemString(lRow, "lotgub"))
	...
	// 생산품번
	sItem = dw_list.GetitemString(lRow, "field_cd")
	IF IsNull(sItem)	or   trim(sItem) = ''	THEN
		f_message_chk(30,'[품번]')
		dw_list.ScrollToRow(lrow)
		dw_list.Setcolumn("field_cd")
		dw_list.setfocus()
		RETURN -1
	END IF
...
public function integer wf_item2 (string sitem, string sspec, long lrow);String  sname, sitgu, snull, sispec, sfilsk, sdepot, sjijil, sSpec_code, slotgub, sGrpno2, sGubun, sPinbr
Decimal {3} dJegoqty, dvalidqty, dqty
...
sPinbr = dw_list.GetItemString(lrow, 'itnbr')
If sGubun = 'OM3' Then
	If sItem = sPinbr Then
		MessageBox('확인', '재투입시는 품번이 동일할수 없습니다.!!')
		dw_list.SetItem(lrow, 'field_cd', sNull)
		Return 1
	End If
End If
If sGubun = 'OM4' Then
	If sItem <> sPinbr Then
		MessageBox('확인', '재작업시는 품번이 동일해야 합니다.!!')
		dw_list.SetItem(lrow, 'field_cd', sNull)
		Return 1
	End If
End If
...
SELECT A.ITDSC, A.ISPEC
  INTO :sName, :sIspec
  FROM ITEMAS A
 WHERE A.ITNBR = :sItem 	AND	 A.USEYN = '0'  ;                   
 
IF sqlca.sqlcode <> 0		THEN
	f_message_chk(33,'[품번]')
	dw_list.setitem(lRow, "field_cd", sNull)	
	dw_list.SetItem(lRow, "f_itdsc", sNull)
	dw_list.SetItem(lRow, "f_ispec", sNull)
	RETURN 1
END IF

dw_list.SetItem(lRow, "f_itdsc", sName)
dw_list.SetItem(lRow, "f_ispec", sIspec)
...
gs_codename = dw_list.GetItemString(nRow, 'itnbr')
...
IF scvcod = shouse 	THEN
	MessageBox('확인','입고처와 출고처는 동일할 수 없습니다.!!')
	dw_detail.SetColumn("cvcod")
	dw_detail.SetFocus()
	RETURN
END IF
...
if not isnull(dw_list.GetItemString(lRow, "cndate")) AND dw_list.GetItemString(lRow, "i_confirm") = 'N' then 
	messagebox("확 인", "입고자료가 승인처리 되어 있으므로 삭제 할 수 없습니다." + &
							  '~n' + "입고 자료를 확인하세요!")
	return 
end if

if not isnull(dw_list.GetItemString(lRow, "pordno"))  then 
	messagebox("확 인", "작업지시 되어 있으므로 삭제 할 수 없습니다." + &
							  '~n' + "입고 자료를 확인하세요!")
	return 
end if
...
string dataobject = "d_adt_00250_02_p"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
integer height = 1816
integer taborder = 30
string dataobject = "d_adt_00250_1"
boolean hscrollbar = true
boolean vscrollbar = true
...
//	END IF
	...
	dw_list.setitem(lrow, "jego_temp", dqty)
// 생산품번
ELSEIF this.GetColumnName() = 'field_cd'	THEN
	sItem  = trim(this.GetText())
	spspec = this.getitemstring(lrow, "pspec")
	ireturn = wf_item2(sitem, sPspec, lrow)
	
	RETURN ireturn
ELSEIF this.GetColumnName() = "f_itdsc"	THEN
	sNAME = trim(this.GetText())
	ireturn = f_get_name3('품명', 'Y', sItem, sName, sSpec, sJijil, sSpec_code)
	this.setitem(lrow, "field_cd", sitem)	
	this.setitem(lrow, "f_itdsc", sname)	
	this.setitem(lrow, "f_ispec", sspec)
	if sitem > '.' then
		spspec = this.getitemstring(lrow, "pspec")
		wf_item2(sitem, sPspec, lrow)
	end if
	RETURN ireturn
ELSEIF this.GetColumnName() = "f_ispec"	THEN
	sspec = trim(this.GetText())
	ireturn = f_get_name3('규격', 'Y', sItem, sName, sSpec, sJijil, sSpec_code)
	this.setitem(lrow, "field_cd", sitem)	
	this.setitem(lrow, "f_itdsc", sname)	
