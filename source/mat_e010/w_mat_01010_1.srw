$PBExportHeader$w_mat_01010_1.srw
$PBExportComments$기타 입고의뢰 등록
forward
global type w_mat_01010_1 from window
end type
type st_1 from statictext within w_mat_01010_1
end type
type dw_daeche from datawindow within w_mat_01010_1
end type
type pb_1 from u_pb_cal within w_mat_01010_1
end type
type p_print from uo_picture within w_mat_01010_1
end type
type p_2 from uo_picture within w_mat_01010_1
end type
type p_copy from uo_picture within w_mat_01010_1
end type
type p_exit from uo_picture within w_mat_01010_1
end type
type p_cancel from uo_picture within w_mat_01010_1
end type
type p_delete from uo_picture within w_mat_01010_1
end type
type p_save from uo_picture within w_mat_01010_1
end type
type p_inq from uo_picture within w_mat_01010_1
end type
type p_delrow from uo_picture within w_mat_01010_1
end type
type p_addrow from uo_picture within w_mat_01010_1
end type
type dw_print from datawindow within w_mat_01010_1
end type
type dw_imhist from datawindow within w_mat_01010_1
end type
type dw_copy from datawindow within w_mat_01010_1
end type
type rb_delete from radiobutton within w_mat_01010_1
end type
type rb_insert from radiobutton within w_mat_01010_1
end type
type dw_detail from datawindow within w_mat_01010_1
end type
type gb_2 from groupbox within w_mat_01010_1
end type
type rr_1 from roundrectangle within w_mat_01010_1
end type
type dw_list from datawindow within w_mat_01010_1
end type
end forward
...
global w_mat_01010_1 w_mat_01010_1
...
public function integer wf_imhist_returns ()
end prototypes
...
FOR  lRow = lRowCount 	TO		1		STEP  -1
	If dw_list.GetItemString(lrow, 'iogbn') = 'I96' Then
		sHist_jpno = dw_list.GetItemString(lrow, 'iojpno')
		
		DELETE FROM IMHIST_DAECHE WHERE SABU = :gs_sabu AND DAEJPNO = :sHist_jpno;
...
string	sJpno, 		&
			...
			sitgu, spdtgu, siosp , sDaeNo
int		iCount
long		lRow, lRowHist, dSeq, nRow
Dec      dAmt
...
////////////////////////////////////////////////////////////////////////
If sGubun = 'I96' Then
	dSeq = SQLCA.FUN_JUNPYO(gs_sabu, sDate, 'C1')
...
FOR	lRow = 1		TO		dw_list.RowCount()
   ...
   //생산실적이 Y 인 경우만 생산팀 입력
	...
	dw_imhist.SetItem(lRowHist, "depot_no",dw_detail.GetItemString(1, "depot_no"))	// 기준창고=입고처
	...
	dw_imhist.SetItem(lRowHist, "ioprc",	dw_list.GetItemDecimal(lRow, "ioprc")) 				// 수불단가=0
...
//	sSaleYn = 'N'
	...
	dw_imhist.SetItem(lRowHist, "gurdat",	sdate)		// 검사요청일
   ...
   // 입고 lot no
	...
	// 미검수내역을 환입할 경우 imhist_daecha에 내역을 기록한다
	If sgubun = 'I96' Then
		nRow = dw_daeche.InsertRow(0)
		
		dw_daeche.SetItem(nRow, 'sabu',    gs_sabu)
		dw_daeche.SetItem(nRow, 'repno',   sDaeno)
		dw_daeche.SetItem(nRow, 'iojpno',  dw_list.GetItemString(lRow, "imhist_iojpno"))
		dw_daeche.SetItem(nRow, 'daejpno', sJpno + string(lRowHist, "000"))
	End If
NEXT
...
FOR	lRow = 1		TO		dw_list.RowCount()
	...
	IF IsNull(sjpno)	OR sJpno = '' 	THEN
		...
		dw_list.SetItem(lRow, "jnpcrt",	'039')			// 전표생성구분
...
IF ic_status = '1'	then
	...
	dw_detail.settaborder("gubun", 20)
	dw_detail.settaborder("cvcod", 30)
	dw_detail.settaborder("depot_no",  40)
	dw_detail.settaborder("frdate", 50)
	dw_detail.settaborder("todate", 60)
	dw_detail.settaborder("itnbr", 70)
	
	dw_detail.settaborder("saupj", 1)
...
// 미검수 제품 환입시 SKIP
If dw_detail.GetItemString(1, 'gubun') = 'I96' Then Return 1
/////////////////////////////////////////////////////////////////
...
long textcolor = 255
long backcolor = 32106727
string text = "* 주의) 기타입고(미검수)는 미검수 출고내역을 환입처리합니다.따라서 환입내역은 검수처리 대상에서 제외됩니다.!! "
...
integer taborder = 30
string title = "none"
string dataobject = "d_imhist_daeche"
...
// 미검수 제품 환입시 제외
If rb_insert.Checked And dw_detail.getitemstring(1, 'gubun') = 'I96' Then Return
...
IF ic_status = '1'	THEN
	If dw_detail.GetItemString(1, 'gubun') = 'I96' Then
		nRow = dw_list.RowCount()
		For ix = nRow To 1 Step -1
			If dw_list.GetItemString(ix, 'chk') <> 'Y' Then
				dw_list.RowsDiscard(ix, ix, Primary!)
			End If
		Next
	End If
   ...
   if	wf_imhist_create(sdate, dseq) < 0 then return 
	...
	IF dw_daeche.Update(True, False) > 0		THEN
	ELSE
		ROLLBACK;
		f_Rollback()
		Return
	...
	IF dw_imhist.Update(True, False) > 0		THEN
		COMMIT;
	ELSE
		ROLLBACK;
		f_Rollback()
...
string  	sJpno,		&
			sNull, sgubun, sfrdate, stodate, scvcod, sitnbr
long		lRow 
SetNull(sNull)
...
// 미검수 제품 환입시
If rb_insert.Checked And sGubun = 'I96' Then
	sfrdate  = dw_detail.getitemstring(1, "frdate")
	stodate  = dw_detail.getitemstring(1, "todate")
	scvcod  = dw_detail.getitemstring(1, "cvcod")
	sitnbr  = dw_detail.getitemstring(1, "itnbr")
	...
	IF	dw_list.Retrieve(gs_sabu, sfrdate, stodate, scvcod, gs_saupj, sitnbr+'%') <	1	THEN
		dw_list.setredraw(true)
		f_message_chk(50, '[입고내역]')
		dw_detail.setcolumn("jpno")
		dw_detail.setfocus()
...
event clicked;call super::clicked;String sHist_jpno
long	lrow
...
dw_list.DataObject = 'd_mat_01011_1'
dw_list.SetTransObject(SQLCA)
...
integer height = 376
integer taborder = 10
string dataobject = "d_mat_01010_1"
boolean border = false
boolean livescroll = true
end type
...
integer height = 1704
integer taborder = 30
string dataobject = "d_mat_01011_1"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
boolean livescroll = true
