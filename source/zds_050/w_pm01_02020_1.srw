$PBExportHeader$w_pm01_02020_1.srw
$PBExportComments$월 생산능력 검토
forward
global type w_pm01_02020_1 from w_inherite_popup
end type
type st_2 from statictext within w_pm01_02020_1
end type
type st_3 from statictext within w_pm01_02020_1
end type
type rr_2 from roundrectangle within w_pm01_02020_1
end type
type rr_1 from roundrectangle within w_pm01_02020_1
end type
type rr_3 from roundrectangle within w_pm01_02020_1
end type
end forward
...
global w_pm01_02020_1 w_pm01_02020_1
...
If is_calgbn = '1' Then
	dw_jogun.DataObject = 'd_pm01_02020_1_11'
	dw_1.DataObject = 'd_pm01_02020_1_21'
	dw_jogun.SetTransObject(sqlca)
	dw_1.SetTransObject(sqlca)
	
	sWkctr = gs_codename2
Else
	dw_jogun.DataObject = 'd_pm01_02020_1_1'
	dw_1.DataObject = 'd_pm01_02020_1_2'
	dw_jogun.SetTransObject(sqlca)
	dw_1.SetTransObject(sqlca)
...
If dw_jogun.Retrieve(is_yymm, gs_codename, gs_codename2,sWkctr, ii_jucha ) <= 0 Then
	Close(this)
	Return
End If
	
/* 대체작업장 조회 */
stemp = f_afterday(is_yymm, ii_jucha -1)

dw_1.Retrieve(stemp, stemp, sWkctr)
...
type dw_jogun from w_inherite_popup`dw_jogun within w_pm01_02020_1
integer x = 59
integer y = 200
integer width = 2720
integer height = 432
integer taborder = 30
string dataobject = "d_pm01_02020_1_11"
end type

type p_exit from w_inherite_popup`p_exit within w_pm01_02020_1
integer x = 2619
integer taborder = 20
end type
...
type p_inq from w_inherite_popup`p_inq within w_pm01_02020_1
integer x = 2446
integer taborder = 10
...
UPDATE PM02_CAPA_DTL
   SET S1QTY = DECODE(:ii_jucha,1,:djQty,S1QTY),
		 ...
		 S7QTY = DECODE(:ii_jucha,7,:djQty,S7QTY),
		 ...
		 LOTQTY7 = DECODE(:ii_jucha,7,:djQty,LOTQTY7)
 WHERE YYMM = :is_yymm
   AND ITNBR = :sItnbr
	AND WKCTR = :sWkctr;
	...
	If nCnt > 0 Then
		UPDATE PM02_CAPA_DTL
			SET S1QTY = S1QTY + DECODE(:ii_jucha,1,:dQty,0),
			    ...
			    S7QTY = S7QTY + DECODE(:ii_jucha,7,:dQty,0),
			    LOTQTY1 = LOTQTY1 + DECODE(:ii_jucha,1,:dQty,0),
				 ...
				 LOTQTY7 = LOTQTY7 + DECODE(:ii_jucha,7,:dQty,0)
		 WHERE YYMM = :is_yymm
			AND ITNBR = :sItnbr
			AND WKCTR = :sWkctr;
	Else
		...
		VALUES ( :is_yymm, :sItnbr, :sWkctr, :dMchr,
		         DECODE(:ii_jucha,1,:dQty,0),			DECODE(:ii_jucha,2,:dQty,0),
					DECODE(:ii_jucha,3,:dQty,0),			DECODE(:ii_jucha,4,:dQty,0),
					DECODE(:ii_jucha,5,:dQty,0),			DECODE(:ii_jucha,6,:dQty,0), 	DECODE(:ii_jucha,7,:dQty,0),
		         DECODE(:ii_jucha,1,:dQty,0),			DECODE(:ii_jucha,2,:dQty,0),
					DECODE(:ii_jucha,3,:dQty,0),			DECODE(:ii_jucha,4,:dQty,0),
					DECODE(:ii_jucha,5,:dQty,0),			DECODE(:ii_jucha,6,:dQty,0), 	DECODE(:ii_jucha,7,:dQty,0));
	End If
		...
		If nCnt > 0 Then
			UPDATE PM02_CAPA_DTL_MCH
			 ...
			 WHERE YYMM = :is_yymm
				...
				AND MCHNO = :sMchno;
		Else
			INSERT INTO PM02_CAPA_DTL_MCH
				( YYMM, ITNBR, MCHNO, MCHR, S1QTY, S2QTY, S3QTY, S4QTY, S5QTY, S6QTY, S7QTY,
			                            LOTQTY1, LOTQTY2, LOTQTY3, LOTQTY4, LOTQTY5, LOTQTY6, LOTQTY7 )
...
// 설비 부하계산
If is_calgbn = '2' Then
	sqlca.PM02_CAPA_LOD_WEEK(gs_sabu, is_yymm, '4');
	If sqlca.sqlcode <> 0 Then
		MessageBox(string(sqlca.sqlcode), SQLCA.SQLERRTEXT)
		RETURN
	END iF
...
type p_choose from w_inherite_popup`p_choose within w_pm01_02020_1
...
type dw_1 from w_inherite_popup`dw_1 within w_pm01_02020_1
integer x = 55
integer y = 756
integer width = 2747
integer height = 916
integer taborder = 40
string dataobject = "d_pm01_02020_1_21"
end type
...
type sle_2 from w_inherite_popup`sle_2 within w_pm01_02020_1
end type

type cb_1 from w_inherite_popup`cb_1 within w_pm01_02020_1
end type

type cb_return from w_inherite_popup`cb_return within w_pm01_02020_1
end type

type cb_inq from w_inherite_popup`cb_inq within w_pm01_02020_1
end type

type sle_1 from w_inherite_popup`sle_1 within w_pm01_02020_1
end type

type st_1 from w_inherite_popup`st_1 within w_pm01_02020_1
end type
