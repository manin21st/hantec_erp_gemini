$PBExportHeader$w_sm04_00040.srw
$PBExportComments$월간 채무 확정 현황
forward
global type w_sm04_00040 from w_standard_print
end type
type rr_2 from roundrectangle within w_sm04_00040
end type
type p_mod from picture within w_sm04_00040
end type
type p_gwins from uo_picture within w_sm04_00040
end type
type rr_1 from roundrectangle within w_sm04_00040
end type
end forward

global type w_sm04_00040 from w_standard_print
string title = "월간 채무 확정 현황"
rr_2 rr_2
p_mod p_mod
p_gwins p_gwins
rr_1 rr_1
end type
global w_sm04_00040 w_sm04_00040
...
type p_preview from w_standard_print`p_preview within w_sm04_00040
end type

type p_exit from w_standard_print`p_exit within w_sm04_00040
end type

type p_print from w_standard_print`p_print within w_sm04_00040
end type

type p_retrieve from w_standard_print`p_retrieve within w_sm04_00040
end type
...
type st_10 from w_standard_print`st_10 within w_sm04_00040
end type



type dw_print from w_standard_print`dw_print within w_sm04_00040
integer x = 3666
integer y = 20
string dataobject = "d_sm04_00040_p2"
end type

type dw_ip from w_standard_print`dw_ip within w_sm04_00040
integer x = 105
integer y = 96
integer width = 3150
integer height = 96
string dataobject = "d_sm04_00040"
end type
...
type dw_list from w_standard_print`dw_list within w_sm04_00040
integer x = 73
integer y = 264
integer width = 4512
integer height = 2004
string dataobject = "d_sm04_00040_2"
...
event clicked;call super::clicked;String sEstNo, sDate,sgb, sgb_save,sch, sch_save
...
If nsPos > 0 And nePos > 0 Then
	...
	do 
		if ix = 1 then
			...
			sCh_save = ' '
		End if 
		sGb      = dw_list.GetItemString(ix,'gbnm')
		sCh      = dw_list.GetItemString(ix,'chasu')
		nPos = Pos(sRepeat, '(_kd_)')  
      if ix > 1 and sGb = sGb_save then
			If nPos > 0 Then sDetRow = Replace(sRepeat, nPos, 6, "    ")
		Else
			If nPos > 0 Then sDetRow = Replace(sRepeat, nPos, 6, dw_list.GetItemString(ix,'gbnm'))
			sGb_save = sGb
		End if
		
		nPos = Pos(sDetRow, '(_cha_)')  
      if ix > 1 and sCh = sCh_save then
			If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 7, "    ")
		Else
			If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 7, dw_list.GetItemString(ix,'chasu'))
			sCh_save = sCh
		End if
		nPos = Pos(sDetRow, '(_cvnas_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 9, dw_list.GetItemString(ix,'cvnas'))
		nPos = Pos(sDetRow, '(_v_ioamt_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 11, String(dw_list.GetItemNumber(ix,'v_io_amt'),'#,##0'))
		nPos = Pos(sDetRow, '(_v_iovat_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 11, String(dw_list.GetItemNumber(ix,'v_io_vat'),'#,##0'))
		nPos = Pos(sDetRow, '(_l_ioamt_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 11, String(dw_list.GetItemNumber(ix,'l_io_amt'),'#,##0'))
		nPos = Pos(sDetRow, '(_l_iovat_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 11, String(dw_list.GetItemNumber(ix,'l_io_vat'),'#,###'))
		nPos = Pos(sDetRow, '(_ioamt_)')  
		If nPos > 0 Then sDetRow = Replace(sDetRow, nPos, 9 , String(dw_list.GetItemNumber(ix,'io_amt'),'#,##0'))
