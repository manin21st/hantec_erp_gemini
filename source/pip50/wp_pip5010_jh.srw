$PBExportHeader$wp_pip5010_jh.srw
$PBExportComments$** 원천징수이행상황신고서(2)
forward
global type wp_pip5010_jh from w_standard_print
end type
type dw_1 from datawindow within wp_pip5010_jh
end type
type cb_2 from commandbutton within wp_pip5010_jh
end type
type rr_1 from roundrectangle within wp_pip5010_jh
end type
type dw_4 from datawindow within wp_pip5010_jh
end type
type p_mod from picture within wp_pip5010_jh
end type
end forward
...
global wp_pip5010_jh wp_pip5010_jh
...
/*원천징수의무자*/
//IF sSaupcd = '20' OR sSaupcd = '40' THEN
//   S_SAUPNO = ''			//사업자번호
//	S_SANAME = '세신전자'			//사업장명
//	S_PRESIDENT = ''		//대표자명
//	S_ADDR = ''				//사업장주소
...
IF sSaupcd = '10' THEN
   S_saname = '세신전자(주)서울'
ELSEIF sSaupcd = '30' THEN
	S_saname = '세신전자(주)장성공장'
ELSEIF sSaupcd = '50' THEN
	S_saname = '세신전자(주)구미공장'
ELSE
	S_sanAme = '세신전자(주)'
END IF


dw_list.setitem(1,"gi_ym",spym)           // 귀속년월
dw_list.setitem(1,"ji_ym",tym)            // 지급년월
dw_list.setitem(1,"sname",S_SANAME)       // 사업장명
dw_list.setitem(1,"spname",S_PRESIDENT)   // 대표자성명
dw_list.setitem(1,"ssno",S_SAUPNO)       // 사업장
dw_list.setitem(1,"saupj",sSaupcd)       // 사업장
dw_list.setitem(1,"saddr",S_ADDR)         // 사업장소재지
dw_list.setitem(1,"stel",S_TEL)           // 전화번호
dw_list.setitem(1,"je_ymd",sDate)         // 제출일자 
//dw_list.setitem(1,"spname1",S_PRESIDENT)  // 대표자성명
dw_list.setitem(1,"semus",sSemu)          // 세무서

if spbtag = '%' then
	dw_list.setitem(1,"a011",inwon11)      // 근로소득(인원)
else
	dw_list.setitem(1,"a011",inwon13)
end if	
dw_list.setitem(1,"a012",totamt11)        // 근로소득(총지급액)
dw_list.setitem(1,"a013",totamt13)        // 근로소득(소득세등)
...
dw_list.setitem(1,"a031",inwon11)         // 중도퇴사(인원)
dw_list.setitem(1,"a032",totamt11)        // 중도퇴사(총지급액)
...
IF sPbtag = '%' or sPbtag = 'P' THEN 
	IF sSaupcd = '20' OR sSaupcd = '40' THEN
		SELECT count(*) as inwon1,   
				 sum(a.WORKINCOMEGROSS) as totamt1,  
	...
	dw_list.setitem(1,"a021",inwon41)                 // 중도정산(인원)
	dw_list.setitem(1,"a022",totamt41)                // 중도정산(총지급액)
	dw_list.setitem(1,"a023",totamt43 + totamt44)     // 중도정산(소득세등)
...
dw_list.setitem(1,"a041",inwon21)                 // 연말정산(인원)
dw_list.setitem(1,"a042",totamt21)                // 연말정산(총지급액)
dw_list.setitem(1,"a043",totamt23 + totamt24)     // 연말정산(소득세등)
...
select count(empno),
       sum(retirementpay1),
       sum(balanceincometax)       
into  :inwon201, :totamt202, :totamt203		 
from p3_retirementpay
where substr(todate,1,6) = :StartYear1;
...
dw_list.setitem(1,"a201",inwon201)       // 퇴직소득(인원)
dw_list.setitem(1,"a202",totamt202)      // 퇴직소득(총지급액)
dw_list.setitem(1,"a203",totamt203)      // 퇴직소득(소득세등)
dw_list.setitem(1,"a207",totamt203)      // 퇴직소득(소득세등)
...
dw_list.setitem(1,"id",sId)			//e-mail id
	
	

dw_list.object.jj_1.visible = false
dw_list.object.jj_2.visible = false
dw_list.object.jj_3.visible = false
dw_list.object.jj_4.visible = false
...
choose case ls_jj
	case '1'
		dw_list.object.jj_1.visible = true
		dw_print.object.jj_1.visible = true
		dw_list.object.jj_2.visible = true
		dw_print.object.jj_2.visible = true
	case '2'
		dw_list.object.jj_3.visible = true
		dw_print.object.jj_3.visible = true
		dw_list.object.jj_4.visible = true
...
this.Control[iCurrent+5]=this.p_mod
end on

on wp_pip5010_jh.destroy
call super::destroy
...
type p_preview from w_standard_print`p_preview within wp_pip5010_jh
integer x = 4229
integer y = 16
...
type p_exit from w_standard_print`p_exit within wp_pip5010_jh
integer x = 4402
integer y = 160
end type

type p_print from w_standard_print`p_print within wp_pip5010_jh
integer x = 4402
integer y = 16
boolean enabled = true
end type
...
type p_retrieve from w_standard_print`p_retrieve within wp_pip5010_jh
integer x = 4055
integer y = 16
end type

event p_retrieve::clicked;String ls_saupcd, ls_gym
Int    iRowCnt
...
iRowCnt = dw_list.retrieve(ls_Saupcd,ls_GYm)
...
type st_window from w_standard_print`st_window within wp_pip5010_jh
integer x = 2565
integer y = 2844
end type

type sle_msg from w_standard_print`sle_msg within wp_pip5010_jh
integer x = 553
integer y = 3008
end type

type dw_datetime from w_standard_print`dw_datetime within wp_pip5010_jh
integer x = 3022
integer y = 3008
end type

type st_10 from w_standard_print`st_10 within wp_pip5010_jh
integer x = 192
integer y = 3008
end type

type gb_10 from w_standard_print`gb_10 within wp_pip5010_jh
integer x = 178
integer y = 2956
end type

type dw_print from w_standard_print`dw_print within wp_pip5010_jh
integer x = 4306
integer y = 692
string dataobject = "dp_pip5010_new_jh"
end type

type dw_ip from w_standard_print`dw_ip within wp_pip5010_jh
integer x = 215
integer width = 3785
integer height = 272
integer taborder = 70
string dataobject = "dp_pip5010_10_jh"
end type
...
type dw_list from w_standard_print`dw_list within wp_pip5010_jh
integer x = 434
integer y = 300
integer width = 3465
integer height = 2024
...
event rbuttondown;IF dw_ip.GetColumnName() = "empno" THEN 
	setnull(gs_code)
	setnull(gs_codename)
	SetNull(Gs_Gubun)
...
dw_list.setitem(iRow,"a107",dw_list.getitemnumber(iRow,"compute_11"))
dw_list.setitem(iRow,"a207",dw_list.getitemnumber(iRow,"a207"))
dw_list.setitem(iRow,"a997",dw_list.getitemnumber(iRow,"compute_25"))
dw_list.setitem(iRow,"a407",dw_list.getitemnumber(iRow,"compute_36"))
dw_list.setitem(iRow,"a457",dw_list.getitemnumber(iRow,"compute_37"))
dw_list.setitem(iRow,"a507",dw_list.getitemnumber(iRow,"compute_38"))
dw_list.setitem(iRow,"a607",dw_list.getitemnumber(iRow,"compute_39"))
dw_list.setitem(iRow,"a697",dw_list.getitemnumber(iRow,"compute_40"))
dw_list.setitem(iRow,"a807",dw_list.getitemnumber(iRow,"compute_41"))
dw_list.setitem(iRow,"a907",dw_list.getitemnumber(iRow,"compute_42"))
dw_list.setitem(iRow,"a706",dw_list.getitemnumber(iRow,"compute_10"))
dw_list.setitem(iRow,"a607",dw_list.getitemnumber(iRow,"compute_30"))
