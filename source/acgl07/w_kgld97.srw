$PBExportHeader$w_kgld97.srw
$PBExportComments$원가부문별 계정집계표
forward
global type w_kgld97 from w_standard_print
end type
type rr_1 from roundrectangle within w_kgld97
end type
end forward
...
integer x = 0
integer y = 0
string title = "원가부문별 계정집계표"
rr_1 rr_1
end type
global w_kgld97 w_kgld97
...
IF f_datechk(sacc_ymd+'01') = -1 THEN
	MessageBox("확인", "회계년월을 확인하세요.!!!")
	dw_ip.SetColumn("k_symd")
	dw_ip.SetFocus()
	Return 1
END IF
eacc_ymd = Trim(dw_ip.GetItemString(1,"k_eymd"))                
IF f_datechk(eacc_ymd+'01') = -1 THEN
	MessageBox("확인", "회계년월을 확인하세요.!!!")
...
dw_ip.SetItem(1,"k_symd",String(today(),"yyyy01"))
dw_ip.SetItem(1,"k_eymd",String(today(),"yyyymm"))
dw_ip.SetItem(1,"saupj", gs_saupj)
dw_ip.SetFocus()
end event

type p_preview from w_standard_print`p_preview within w_kgld97
end type

type p_exit from w_standard_print`p_exit within w_kgld97
end type

type p_print from w_standard_print`p_print within w_kgld97
end type

type p_retrieve from w_standard_print`p_retrieve within w_kgld97
end type
...
type st_10 from w_standard_print`st_10 within w_kgld97
end type



type dw_print from w_standard_print`dw_print within w_kgld97
integer x = 4023
integer y = 164
string dataobject = "dw_kgld972_P"
end type

type dw_ip from w_standard_print`dw_ip within w_kgld97
integer y = 28
integer width = 3840
integer height = 220
string dataobject = "dw_kgld971"
end type
...
IF this.GetColumnName() = 'gbn' then
	if this.GetText() = '1' then
		dw_list.DataObject ="dw_kgld972"
		dw_print.DataObject ="dw_kgld972_p"
	else
		dw_list.DataObject ="dw_kgld973"
		dw_print.DataObject ="dw_kgld973_p"
	end if
...
type dw_list from w_standard_print`dw_list within w_kgld97
integer x = 59
integer y = 264
integer width = 4517
integer height = 1928
string title = "원가부문별 계정집계표"
string dataobject = "dw_kgld972"
