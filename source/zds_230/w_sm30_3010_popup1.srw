$PBExportHeader$w_sm30_3010_popup1.srw
$PBExportComments$주간 판매계획 접수
forward
global type w_sm30_3010_popup1 from w_inherite_popup
end type
type dw_insert from datawindow within w_sm30_3010_popup1
end type
type rr_2 from roundrectangle within w_sm30_3010_popup1
end type
end forward
...
global w_sm30_3010_popup1 w_sm30_3010_popup1
...
dw_1.SetItem(1, 'empno', gs_codename )

dw_jogun.Retrieve(1)
Long i
For i = 1 To dw_jogun.RowCount()
	dw_jogun.Object.sdate[i] = gs_code
Next
...
type dw_jogun from w_inherite_popup`dw_jogun within w_sm30_3010_popup1
integer x = 37
integer y = 212
integer width = 1774
integer height = 688
string dataobject = "d_sm30_3010_popup1_2"
end type

type p_exit from w_inherite_popup`p_exit within w_sm30_3010_popup1
integer x = 1733
integer y = 32
...
type p_inq from w_inherite_popup`p_inq within w_sm30_3010_popup1
boolean visible = false
integer x = 27
integer y = 1148
end type

type p_choose from w_inherite_popup`p_choose within w_sm30_3010_popup1
integer x = 1559
integer y = 32
...
/* 위아광주(WK)는 VAN 자료 미 접수로 주간판매계획 생성이 되지 않음. - 생성 시 위아광주는 별도 품목 LIST만 생성(수량 = 0) */
/* 2006.12.13 BY SHINGOON */
wf_wk(ls_saupj, ls_ymd, ls_empno)

/* 위아포승(WP), 위아서산(WS)도 VAN자료 미 접수로 주간판매계획 생성이 되지 않음. - 생성 시 위아포승, 서산은 별도 품목 LIST만 생성(수량 = 0) */
/* 2007.06.04 BY SHINGOON */
wf_wp(ls_saupj, ls_ymd, ls_empno)
wf_ws(ls_saupj, ls_ymd, ls_empno)

/*수출 자료 P/O SHEET 등록에서 주간판매 계획으로 생성
=> PO sheet D+1의 일자가.. 생성일 + 5 +3주 (26일~31일)에 들어 오면 주간판매계획에 포함
2010.06.21 by WON */
wf_exp(ls_saupj, ls_ymd, ls_empno)

/* 파워텍 생산코드 접수로 인해 계획자료 산출함. - 2009.10.14 by shingoon  임시 보류(다시복원:2009.10.27)*********/
/* 파워텍 자료 생성 - VAN자료 수신되면 실행되지 않음 */
/* 파워텍 VAN자료가 매일 수신 되지 않기 때문에 VAN자료 접수가 되지 않으면 임의 자료 생성 
   VAN이 접수되지 않으면 주/판 생성않됨. VAN자료 없이 "행추가"작업을 하면 사용자가 작업할 양이 많음.
	by shingoon 2007.05.12 -영업팀 이석기DR요청 - 주/판 생성 시 파워텍 VAN자료 없으면 일괄 임의생성 */
Long   ll_chk
SELECT COUNT('X')
  INTO :ll_chk
 ...
 WHERE GUBUN  <> 'HAN'
   AND YYMMDD =  :ls_ymd
	...
	AND CVCOD  =  '201012' ;
If ll_chk < 1 Then
	wf_ptec(ls_saupj, ls_ymd, ls_empno)
End If
/********************************************************************************************************/
end event
...
type dw_1 from w_inherite_popup`dw_1 within w_sm30_3010_popup1
integer x = 23
integer y = 24
integer width = 1545
integer height = 172
...
type sle_2 from w_inherite_popup`sle_2 within w_sm30_3010_popup1
end type

type cb_1 from w_inherite_popup`cb_1 within w_sm30_3010_popup1
end type

type cb_return from w_inherite_popup`cb_return within w_sm30_3010_popup1
end type

type cb_inq from w_inherite_popup`cb_inq within w_sm30_3010_popup1
end type

type sle_1 from w_inherite_popup`sle_1 within w_sm30_3010_popup1
end type

type st_1 from w_inherite_popup`st_1 within w_sm30_3010_popup1
end type
